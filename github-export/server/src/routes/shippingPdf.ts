...
async function getUPSAccessToken() {
  ...
  if (
    ...
    throw new Error(
      'UPS credentials missing: username, password, or access key not provided'
    );
  }
...
router.get('/qc-checklist/:orderId', async (req: Request, res: Response) => {
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="QC-Checklist-${orderId}.pdf"`
    );
...
router.get('/sales-order/:orderId', async (req: Request, res: Response) => {
  try {
    ...
    if (customerAddresses.length > 0) {
      ...
      shipCurrentY -= 15;
      const cityStateZip =
        `${primaryAddress.city || ''}, ${primaryAddress.state || ''} ${primaryAddress.zipCode || ''}`.trim();
      if (cityStateZip !== ', ') {
    ...
    if (order.features && Object.keys(order.features).length > 0) {
      Object.entries(order.features).forEach(([featureKey, featureValue]) => {
        if (featureValue && featureValue !== false && featureValue !== '') {
          ...
          const featureName = featureDetail
            ? featureDetail.displayName || featureDetail.name
            : featureKey;
    ...
    currentY -= 20;
    page.drawText(
      'Questions? Contact us at sales@agatcomposite.com or (XXX) XXX-XXXX',
      {
        x: margin,
        y: currentY,
        size: 9,
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="Sales-Order-${orderId}.pdf"`
    );
    res.setHeader('Content-Length', pdfBytes.length);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        res.setHeader(
          'Content-Disposition',
          `attachment; filename="UPS-Label-${orderId}-${trackingNumber}.pdf"`
        );
        res.setHeader('Content-Length', pdfBytes.length);
        ...
        res.setHeader(
          'Content-Disposition',
          `attachment; filename="Shipping-Label-Fallback-${orderId}.pdf"`
        );
...
router.post('/ups-shipping-label/bulk', async (req: Request, res: Response) => {
  try {
    const { orderIds, shippingAddress, packageDetails, trackingNumber } =
      req.body;
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="Bulk-Shipping-Label-${trackingNumber || 'BULK'}.pdf"`
    );
    res.setHeader('Content-Length', pdfBytes.length);
