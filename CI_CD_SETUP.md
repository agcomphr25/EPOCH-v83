...
- ✅ ESLint code quality checks (BLOCKS merges)
- ✅ Prettier formatting validation (BLOCKS merges)
- ✅ Build verification (BLOCKS merges - this is the real validation)
- ✅ Security audit (moderate level - advisory only)

**Required checks must pass before merging. Build check catches real breaking issues.**

**Why TypeScript check is advisory:**
- The codebase has 379 existing type warnings (Drizzle ORM compatibility issues)
- These are technical debt that don't affect runtime or builds
- Making it blocking would fail every PR even when code is perfect
- The build step is the real validation - if build passes, code works
