...
export default function WasteManagementForm() {
  ...
  const [availableContainerSizes, setAvailableContainerSizes] = useState<
    string[]
  >([
    '5 gal',
    '1 gal',
  ...
  const [newChemicalInput, setNewChemicalInput] = useState<{
    [key: string]: string;
  }>({});
  const [showAddChemical, setShowAddChemical] = useState<{
    [key: string]: boolean;
  }>({});
  const [newContainerSizeInput, setNewContainerSizeInput] = useState<{
    [key: string]: string;
  }>({});
  const [showAddContainerSize, setShowAddContainerSize] = useState<{
    [key: string]: boolean;
  }>({});
  ...
  useEffect(() => {
    ...
    const savedContainerSizes = localStorage.getItem(
      'wasteManagementContainerSizes'
    );
  ...
  const saveChemicalsToStorage = (chemicals: string[]) => {
    const customChemicals = chemicals.filter(
      (chem) =>
        ![
          'Acetone',
          'Benzene',
    ...
    localStorage.setItem(
      'wasteManagementChemicals',
      JSON.stringify(customChemicals)
    );
  };
  ...
  const handleChemicalEntryChange = (
    id: string,
    field: keyof ChemicalEntry,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const submissionData = {
      ...formData,
      chemicals: chemicalEntries.filter(
        (entry) => entry.chemicalName.trim() !== ''
      ),
  ...
  const handleSave = () => {
    const draftData = {
      formData,
      chemicalEntries: chemicalEntries.filter(
        (entry) => entry.chemicalName || entry.quantity || entry.containerSize
      ),
    ...
    const existingDrafts = JSON.parse(
      localStorage.getItem('wasteManagementDrafts') || '[]'
    );
    existingDrafts.push(draftData);
    localStorage.setItem(
      'wasteManagementDrafts',
  ...
  const loadDraft = (draft: any) => {
    ...
    const processedEntries = loadedEntries.map(
      (entry: ChemicalEntry, index: number) => ({
        ...entry,
        id: (index + 1).toString(),
      })
    );
        ...
        <CardContent className="space-y-6">
                          ...
                          <div className="flex items-start justify-between">
                            <div>
                              ...
                              <p className="text-xs text-gray-600 mt-1">
                                {draft.chemicalEntries?.length || 0} chemical
                                entries
                              </p>
                            </div>
                          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                <Label
                  htmlFor="generatorPickupAddress"
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleFormDataChange('customerNameAndPhone', e.target.value)
                  }
                  className="mt-1 border-b-2 border-l-0 border-r-0 border-t-0 rounded-none focus:border-blue-500 bg-transparent"
                              ...
                              <Select
                                ...
                                onValueChange={(value) => {
                                  if (value === '__add_new__') {
                                    setShowAddChemical((prev) => ({
                                      ...prev,
                                      [entry.id]: true,
                                    }));
                                  } else {
                                    handleChemicalEntryChange(
                                      entry.id,
                                      'chemicalName',
                                      value
                                    );
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  handleFormDataChange('additionalComments', e.target.value)
                }
