...
| May 7, 2026 | Section 10 (Phase 2) added: Routing-step enforcement on material issues per Task #144. The active routing step on a traveler is now the **single source of truth** for what may be issued / consumed: `server/src/services/routingStepService.ts#getActiveRoutingStep(travelerId)` derives it from `traveler_steps` (lowest-numbered IN_PROGRESS / STARTED step, otherwise the lowest-numbered NOT_STARTED step whose every prior step is COMPLETED, otherwise null). Migration `0111_routing_step_enforcement.sql` adds `intended_routing_step_id` to both `material_lot_reservations` and `cutting_built_packets` (FK → `traveler_steps.id`, nullable for legacy rows). `MaterialIssueService` now: (a) auto-detects the active step when callers omit `travelerStepId`, (b) passes both the active step AND the packet/reservation’s pinned `intendedRoutingStepId` into `validateRoutingStep`, (c) rejects mismatches with a new `WRONG_ROUTING_STEP` blocker code (also new: `NO_ACTIVE_ROUTING_STEP`), and (d) accepts a typed `override` payload validated against the new reason-code catalog `server/src/services/materialIssueOverridePolicy.ts` — `ROUTING_STEP_BYPASS` (allowed: Production Supervisor / Manufacturing Manager / ADMIN / OWNER, written reason ≥ 10 chars), `WAD_LATE_RELEASE`, `LOT_QUARANTINE_DEVIATION`, `EMERGENCY_PRODUCTION`. Override evidence (reason, approver id, approver display name, approver role, written reason) is stamped on the inventory ledger row’s `metadata.override` for permanent DCAA traceability. Step-completion atomicity is provided by `server/src/services/routingStepCompletionService.ts#completeRoutingStep`, which under one `db.transaction` row-locks the step (`FOR UPDATE`), verifies operator (and inspector when `qcRequired`) signoffs are present, transitions to COMPLETED with `completedAt` / `completedBy`, and returns the next eligible NOT_STARTED step id without auto-starting it. Operator-UI auto-detection ships at the service layer — the existing routes that consume `MaterialIssueService` no longer need to send `travelerStepId` on a draw, since the service derives it from the active step. Phase-2 builds strictly on Phase 1; the only changes to gate semantics are additive (new blocker codes) and existing tests for Phase-1 invariants pass unchanged. Tests: `server/__tests__/routingStepService.test.ts` (active-step derivation incl. cache + HOLD edge), `server/__tests__/materialIssueOverridePolicy.test.ts` (catalog + role/reason-code matrix), `server/__tests__/materialIssueServiceRoutingStep.test.ts` (in-order draw succeeds, wrong-step rejected, override clears wrong-step, unauthorized override does not), and extended `server/__tests__/materialIssueGates.test.ts` cases for `WRONG_ROUTING_STEP` / `NO_ACTIVE_ROUTING_STEP` / packet-pin mismatch. |
| May 7, 2026 | Section 10 (Phase 2) added: Operator badge authentication for material issues per Task #143. New `operator_auth_sessions` table holds short-lived shop-floor sessions issued by `OperatorAuthService` (`server/src/services/operatorAuthService.ts`) via badge scan or PIN entry. Tokens are HMAC-signed (`OPERATOR_TOKEN_SECRET`, falls back to `PORTAL_TOKEN_SECRET`) and presented as `MaterialIssueRequest.operatorAuth.sessionToken`; `MaterialIssueService.executeMaterialIssue` resolves the token to a session row and uses it as the authoritative source for operator identity stamped on the ledger (overriding any caller-supplied `req.operator`). New gate `validateOperatorSession` in `materialIssueGates.ts` enforces revocation, absolute timeout (default 8h), idle timeout (default 15min), and — when `operatorAuth.requireFreshReauth` is set — a `STALE_OPERATOR_AUTH` blocker if `lastReauthAt` is older than `HIGH_RISK_REAUTH_MAX_AGE_SECONDS` (default 60s). High-risk callers (overrides, expired-lot release, quarantine release, scrap above `HIGH_RISK_SCRAP_USD_THRESHOLD`) MUST set `requireFreshReauth: true`. Routes mounted at `/api/operator-auth` (`/badge`, `/pin`, `/refresh`, `/reauth`, `/revoke`, `/me`, `/sessions`); admin viewer at `/admin/operator-sessions`; client hook `useOperatorAuth` + `OperatorAuthDialog` capture badge/PIN at the workstation. Sessions are stored in `sessionStorage` (not `localStorage`) so they die with the tab on top of server-side timeouts. Idle/absolute timeouts and the high-risk reauth window are env-tunable. |
| May 11, 2026 | Task #190 — WAD Backfill & Authoring Streamlining. New endpoints `GET /api/work-orders/production/wad-status` (per-project WAD aggregation across `p2_release` + `production` stages) and `POST /api/work-orders/production/ensure-for-project/:projectId` (idempotent auto-create of a Production Work Order so a WAD can be authored even when one was never seeded from the quote). The wizard's approval route (`POST /api/work-orders/production/:id/wizard/approve`) now (a) flips `production_work_orders.status` from `PLANNED`/`READY` → `RELEASED` once all four required roles have signed off, satisfying the project's WAD release gate without re-running `/release-to-p2`, and (b) emits an immutable audit-ledger event via `recordAuditEvent()` — `WAD_APPROVED` for normal cases, **`WAD_BACKFILL_APPROVED`** (with `payload.tag = 'wad_backfill'`) when the parent project is already in stage `production` at the moment of approval. Backfill approval intentionally does **not** mutate `projects.current_stage` (no stage regression, no stage advance) — the WAD gate flips ✓ purely via the work-order status change. New permitted workflow: PM Control Center → WAD Status Dashboard (`/wad-status`) → "Author WAD" → wizard runs in BACKFILL banner mode → approve → ledger records `wad_backfill`. Cross-reference: see `docs/audit-evidence-policy.md` for the audit ledger contract and `client/src/pages/WADStatusDashboard.tsx` for the operator surface. |
| May 14, 2026 | Task #229 — Three remaining receiving entry points wired into `inventory_transaction_ledger` so all RECEIVE events surface in the Material Traceability Viewer / Inventory Ledger page. (1) `POST /api/vendor-pos/items/:itemId/receive` (`storage.recordVendorPOReceipt`) writes a `RECEIVE` ITL row inside the same `db.transaction` that updates `vendor_po_items.receivedQuantity`; the prior cumulative qty is read with `FOR UPDATE` so `quantityDelta = newCumulative − prevCumulative` is consistent under concurrent receipts; idempotency key `sourceModule='receiving:vendor-po'` + `sourceRecordId='${poLineItemId}:${newCumulative}'` makes partial-receipt retries collapse to a single ledger row. (2) `POST /api/inventory/parts-requests/receive` is wrapped in a `db.transaction` covering the parent receipt row, line updates, allocation propagation, request status history, batch status, AND one `RECEIVE` ITL row per receipt line keyed by the stable composite `sourceRecordId='${createdReceipt.id}:${orderLineId}'` (anchored to the parent receipt id and the business order-line id rather than the surrogate receipt-line PK). AG-part lookup against `inventory_items`; ad-hoc lines (no `agPartNumber`) and lines whose AG number has no matching inventory item log a structured warning and skip the ITL write (the FK is NOT NULL). (3) `POST /api/material-lots` now wraps `materialLots` insert + initial `material_lot_transactions` RECEIVE row + ITL RECEIVE row in a single `db.transaction` (`sourceModule='material-lots:create'`, `sourceRecordId=lot.id`). All three paths fail atomically: an ITL failure rolls back the inventory mutations. Historical rows are reconstructed by `server/scripts/backfillInventoryTransactionLedger.ts` with two new sources: `parts-request-receipts` (joins `parts_request_receipt_lines` → `parts_request_receipts` → `parts_request_order_lines`, one row per receipt line, `createdAtOverride = receipts.received_at`) and `vendor-po-items` (one cumulative RECEIVE row per `vendor_po_items.received_quantity > 0`, key `${poLineItemId}:${cumulative}` so live writer + backfill cannot double-write). `recordVendorPOReceipt` skips the ITL write — with a clear console warning — when `inventory_items` has no row for the line's `agPartNumber`, since the ITL `inventory_item_id` FK is NOT NULL; this preserves the legacy "ad-hoc PO line" flow. Reference pattern is `handleAcceptedUnit` in `server/src/routes/receiving.ts` (Task #216). |
