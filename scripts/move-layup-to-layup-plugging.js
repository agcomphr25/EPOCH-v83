...
async function moveLayupToLayupPlugging() {
  try {
    ...
    layupOrders.forEach((order) => {
      console.log(
        `  - Order ID: ${order.orderId}, FB Order #: ${order.fbOrderNumber || 'N/A'}, Current Dept: ${order.currentDepartment}`
      );
    });
    ...
    if (updateResponse.ok) {
      console.log(
        '✅ Successfully moved orders from Layup to Layup/Plugging department!'
      );
      console.log(
