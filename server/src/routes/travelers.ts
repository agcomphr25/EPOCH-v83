...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const traveler = await storage.generateTravelerFromRouting(
        partRoutingId,
        {
          workOrderId,
          salesOrderId,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (unsignedSigTasks.length > 0) {
        return res.status(400).json({
          error:
            'All tasks requiring signatures must be signed before completing the step',
      ...
      if (taskId) {
        ...
        matchedGateTask = pendingGateTasks.find(
          (t) =>
            ...
            (t as any).signatureRole === signatureRole
        );
      }
