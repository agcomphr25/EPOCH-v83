...
- **Order Urgency/Priority System** (Oct 24, 2025): ✅ COMPLETE & VERIFIED - Implemented end-to-end urgency/priority system for rush orders across all application layers:
  - **Frontend**: Staff can mark orders as urgent via "Set Urgent/Rush" dropdown in All Orders list. Urgent orders display animated orange "RUSH!!!" badge with lightning bolt icon in both All Orders and Production Queue views
  - **Production Queue Sorting**: Rush orders automatically move to top of production queue (priority_score=1000), sorted ahead of all normal orders (priority_score=9999)
  - **Dashboard Metrics**: Production Queue and department dashboards show dedicated "Rush Orders" count card with pulsing orange badge highlighting urgent orders requiring immediate attention
  - **Backend**: API endpoint PUT /api/orders/:orderId/urgency updates urgency levels with automatic priority scoring. Production queue endpoint /api/production-queue/prioritized returns orders sorted by priority_score → due_date → created_at
