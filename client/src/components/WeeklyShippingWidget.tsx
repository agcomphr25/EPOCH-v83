...
export default function WeeklyShippingWidget() {
  ...
  const currentWeekShipped = orders?.filter(order => {
    if (order.status !== 'FULFILLED' && order.currentDepartment !== 'Fulfilled') {
      return false;
    }
  ...
  return (
    <Card className="bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200" data-testid="card-weekly-shipping-tracker">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-2xl font-bold text-blue-600" data-testid="text-current-week">Week {currentWeek}</div>
              </div>
              <div className="text-right">
                <div className="text-sm text-gray-600">Stocks Shipped</div>
                <div className="text-3xl font-bold text-green-600" data-testid="text-stocks-shipped">{currentWeekShipped}</div>
              </div>
            </div>
            <div className="flex items-center gap-2 text-xs text-gray-500" data-testid="text-week-range">
              <Calendar className="h-3 w-3" />
              <span>{formatWeekRange(currentWeek, currentYear)}</span>
            </div>
            <Link href="/shipping-tracker">
              <div className="mt-3 text-center">
                <button className="text-sm text-blue-600 hover:text-blue-800 hover:underline flex items-center gap-1 mx-auto" data-testid="button-view-full-tracker">
