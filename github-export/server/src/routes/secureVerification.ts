  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!isValidPassword) {
        ...
        console.warn(
          `Failed admin verification attempt by user ${req.user.id} (${req.user.username})`
        );
      ...
      console.log(
        `Admin verification successful for user ${req.user.id} (${req.user.username})`
      );
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `SECURE OPERATION: ${req.user?.username} performed ${operationType} on ${itemType}: ${itemName} - ${operation}`
      );
