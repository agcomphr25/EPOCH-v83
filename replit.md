...
## Recent Changes
- **Order Urgency/Priority System** (Oct 24, 2025): ✅ COMPLETE - Implemented comprehensive urgency/priority system for rush orders. Staff can mark orders as urgent/high priority via the "Set Urgent/Rush" dropdown option in the All Orders list. Urgent orders display an animated orange "RUSH!!!" badge with a lightning bolt icon on order cards. Department dashboard cards (starting with CNC Queue) now show a dedicated "Rush Orders" count card highlighting the number of high-priority orders requiring immediate attention. Backend API endpoint (PUT /api/orders/:orderId/urgency) updates order urgency levels with automatic priority scoring (score of 1000 for rush orders moves them to top of production queue). System includes proper cache invalidation to ensure urgent counts and badges update immediately across all department views. Database schema extended with urgency, priorityScore, and isManualUrgency fields for tracking manual priority overrides.
