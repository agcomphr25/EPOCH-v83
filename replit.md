...
Authentication: Production-ready backend authentication system with HTTP-only session cookies, bcrypt password hashing, and server-side session validation. Authentication is REQUIRED FOR ALL ENVIRONMENTS (development and production) for testing purposes. All dashboard routes validate sessions against backend on mount, preventing unauthorized access. Session tokens are stored securely in HTTP-only cookies (not accessible to JavaScript). Database-first authentication: Login system checks database for user credentials first, then falls back to hardcoded users for backward compatibility. Password updates via user management immediately take effect on next login, with database password_hash taking precedence over hardcoded passwords.
...
Navigation enhancement: Role-based navbar system fully implemented where each user sees only the navigation items they have access to based on their dashboard permissions. Admins (glennj, tasham, staciw) have full navigation access to all system functions. Non-logged-in users see no navigation items (except Communications). All users see welcome message and logout button when logged in. User permissions are validated against backend session data to prevent client-side manipulation. Navigation items are filtered using `hasRouteAccess()` and `hasFullAccess()` functions from `userPermissions.ts` configuration. All main nav links and dropdown menus (Forms & Reports, Training, Inventory, QC & Maintenance, Employees, Finance, Purchase Orders, Production Scheduling, P1 Department Manager, Verified Modules) conditionally render based on filtered permissions. Implemented with useMemo hooks for performance optimization. Home button added to main navigation near branding and on personalized dashboards for easy access to user's dashboard.
All Orders Page Filtering: Changed from server-side to client-side pagination for proper filtering across entire dataset. AllOrdersPage.tsx now fetches all orders via `/api/orders/with-payment-status` endpoint and filters the complete dataset before paginating results. This ensures search and department filters work across all 803+ orders, not just the current page. Pagination resets to page 1 when filters change. Performance optimized with useMemo for filtering logic.
...
-   **Core Features**:
    ...
    -   **Department Manager**: Enhanced navigation with department-specific views and comprehensive order details via tooltips. **CRITICAL WORKFLOW (NEVER CHANGES)**: Orders entered → P1 Production Queue → (Layup Scheduler) → Layup/Plugging → Barcode → CNC → Gunsmith → Finish → Finish QC → Paint → Shipping QC → Shipping (final department). **STATUS PROGRESSION**: When orders complete the Shipping department, their status changes to "FULFILLED" and currentDepartment is cleared. "Fulfilled" is a STATUS, not a department. **EXCEPTIONS**: Flat tops, bottom metal only items, or miscellaneous items only may skip departments. CNC Department features dual-queue system with automatic routing based on order features. Gunsmith Department features due date categorization with color-coded priority levels while maintaining original card grid layout and multi-select functionality. Finish Department features technician assignment with routing to next stage. Finish QC Department organizes orders by assigned technician with alphabetical/numerical sorting, displays texture and paint color information on cards, and provides multi-select progression. Paint Department features multi-select functionality and progression to Shipping QC. Shipping QC progresses to Shipping. Shipping features "Complete Shipping" button that sets order status to FULFILLED and clears currentDepartment. Orders with "no_rail" bypass gunsmith work entirely, routing directly from CNC to Finish department.
    ...
    -   **Shipping Integration**: UPS OAuth 2.0 API integration with dedicated shipping label page. Features package details form, billing options (sender/receiver), customer address auto-population, real UPS label generation with tracking numbers, and comprehensive error handling. Enhanced Shipping Tracker with search functionality for order number and customer name lookups, displays detailed shipping information including tracking numbers, shipped dates, carrier details, and customer notification status with timestamps.
