...
export default function CuttingTableProductionQueue() {
  ...
  const { data: queueData = [], isLoading } = useQuery<POGroup[]>({
    queryKey: ['/api/p2-production-queue', 'Cutting Table', selectedStatus],
    queryFn: async ({ queryKey }) => {
      const [baseUrl, department, status] = queryKey;
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch cutting table production queue');
      }
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      ...
      IN_PROGRESS: { variant: 'default' as const, icon: Scissors },
    ...
    return (
      <div className="container mx-auto p-6">
        <div className="flex items-center justify-center h-64">
          <div className="text-muted-foreground">Loading cutting table queue...</div>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight flex items-center gap-2">
            ...
            Cutting Table Production Queue
          </h1>
          <p className="text-muted-foreground mt-1">
            Manage cutting table production orders. Schedule layup dates and track manufacturing progress.
          </p>
        </div>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center h-64">
            ...
            <p className="text-muted-foreground text-lg">
              No cutting table production orders found
            </p>
            <p className="text-sm text-muted-foreground mt-2">
              Production orders will appear here when generated from P2 Purchase Orders
            </p>
          </CardContent>
        </Card>
      ) : (
