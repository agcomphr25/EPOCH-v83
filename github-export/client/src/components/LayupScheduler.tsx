...
function DroppableCell({
  ...
  console.log(
    `🔍 DroppableCell [${moldId}]: ${orders.length} orders`,
...
export default function LayupScheduler() {
  ...
  // Debug molds data
  console.log('🔧 LayupScheduler: Molds data:', {
    molds,
    moldsLength: molds.length,
    moldsLoading,
  });
  const {
    employees,
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        '📅 Loading existing schedule assignments:',
  ...
  useEffect(() => {
    ...
    const filteredOutP1Orders = allOrders.filter(
    ...
    console.log(
      '🏭 LayupScheduler: P1 PO orders filtered out:',
      filteredOutP1Orders.length
    );
    ...
    if (orders.length > 0 && molds.length > 0 && employees.length > 0) {
      console.log(
        '🚀 LayupScheduler: All data loaded, auto-schedule should run'
      );
      ...
      if (!hasAssignments && orders.length > 0) {
        console.log(
          '🎯 Auto-triggering initial schedule generation will be available after component initialization'
        );
      }
    } else {
  ...
  const generateAutoSchedule = useCallback(() => {
    ...
    const sortedOrders = [...orders].sort((a, b) => {
      ...
      const aMesaUniversal =
        a.stockModelId === 'mesa_universal' || a.product === 'Mesa - Universal';
      const bMesaUniversal =
        b.stockModelId === 'mesa_universal' || b.product === 'Mesa - Universal';
    ...
    const mesaUniversalOrders = sortedOrders.filter(
      (o) =>
    ...
    const getCompatibleMolds = (order: any) => {
      ...
      if (
        ...
        console.log('🏭 DETAILED PRODUCTION/P1 ORDER COMPATIBILITY CHECK:', {
          ...
          moldsWithModelId: molds
            .filter((m) => m.enabled && m.stockModels?.includes(modelId))
            .map((m) => m.moldId),
          allEnabledMolds: molds
            .filter((m) => m.enabled)
      ...
      const compatibleMolds = molds.filter((mold) => {
        ...
        if (!mold.stockModels || mold.stockModels.length === 0) {
          console.log(
            `🔧 Mold ${mold.moldId} has no stock model restrictions - compatible with all`
          );
        ...
        if (
          ...
          console.log(
            `🏭 MOLD CHECK: Order ${order.orderId} (${modelId}) vs Mold ${mold.moldId} (${mold.stockModels?.join(', ')}) = ${isCompatible ? '✅ COMPATIBLE' : '❌ NOT COMPATIBLE'}`
          );
        }
        if (
          !isCompatible &&
          ...
          console.log(
            `❌ Order ${order.orderId} (${modelId}) not compatible with mold ${mold.moldId} (has: ${mold.stockModels?.slice(0, 3).join(', ')}...)`
          );
        }
        return isCompatible;
      });
    ...
    sortedOrders.forEach((order, index) => {
      ...
      if (bestMold && bestDateIndex < allWorkDays.length) {
        ...
        if (isMesaUniversal && currentMesaCount >= 8) {
          console.log(
            `⏸️ Mesa Universal limit reached for ${dateKey}: ${currentMesaCount}/8 - skipping to next day`
          );
        ...
        if (isMesaUniversal) {
          mesaUniversalDailyCount[dateKey] =
            (mesaUniversalDailyCount[dateKey] || 0) + 1;
        }
        ...
        const logPrefix = isMesaUniversal
          ...
          : order.source === 'production_order'
            ...
            : '✅ Assigned';
        const mesaStatus = isMesaUniversal
          ? ` (Mesa: ${mesaUniversalDailyCount[dateKey]}/8)`
          : '';
        console.log(
          `${logPrefix} ${order.orderId} to ${bestMold.moldId} on ${format(targetDate, 'MM/dd')} (${dailyAssignments[dateKey]}/${maxOrdersPerDay} daily capacity)${mesaStatus}`
        );
      }
    ...
    Object.entries(mesaUniversalDailyCount).forEach(([date, count]) => {
      console.log(
        `  ${format(new Date(date), 'MM/dd')}: ${count}/8 Mesa Universal orders`
      );
    });
              ...
              const moldRows = activeMolds
                .filter((mold) => {
                  ...
                  return datesWithOrders.some((date) => {
                    ...
                    return Object.entries(orderAssignments).some(
                      ([orderId, assignment]) => {
  ...
  useEffect(() => {
    ...
    const unassignedProductionOrders = productionOrders.filter(
      (o) => !orderAssignments[o.orderId]
    );

    const shouldRunAutoSchedule =
      orders.length > 0 &&
      molds.length > 0 &&
      employees.length > 0 &&
  ...
  React.useEffect(() => {
    if (schedule.length > 0 && Object.keys(orderAssignments).length === 0) {
      ...
      console.log(
        '🏭 Production/P1 orders in schedule:',
        productionScheduleItems.length
      );
      ...
      schedule.forEach((item) => {
        ...
        if (isProduction) {
          console.log(
            `🏭 PRODUCTION/P1 ORDER ASSIGNMENT: ${item.orderId} (${order?.source}) → mold ${item.moldId} on ${item.scheduledDate.toDateString()}`
          );
        }
      ...
      console.log(
        ...
        Object.keys(autoAssignments).length
      );
      console.log(
        '✅ Production/P1 orders assigned:',
        Object.keys(autoAssignments).filter((orderId) => {
            ...
            <div className="flex items-center space-x-4 text-sm">
              ...
              <div className="bg-green-50 dark:bg-green-900/20 px-3 py-2 rounded-lg">
                <span className="text-green-700 dark:text-green-300 font-medium">
                  {
                    orders.filter((o) => o.source === 'p1_purchase_order')
                      .length
                  }{' '}
                  P1 Purchase Orders
                </span>
              ...
              <Dialog>
                ...
                <DialogContent className="max-w-2xl">
                  ...
                  <div className="space-y-4 max-h-96 overflow-y-auto">
                    ...
                    {employees.length === 0 ? (
                      <div className="text-center py-8 text-gray-500">
                        No employees configured yet. Use the form above to add
                        your first employee.
                      </div>
                    ) : (
                              ...
                              <div className="flex items-center justify-between mb-3">
                                <div>
                                  ...
                                  <div className="text-sm text-gray-600 dark:text-gray-400">
                                    Employee ID: {emp.employeeId} | Department:{' '}
                                    {emp.department}
                                  </div>
                                </div>
                                <div className="flex items-center space-x-2">
                    ...
                    {employees.length > 0 && (
                      <div className="mt-4 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                        <p className="text-sm text-green-700 dark:text-green-300">
                          <strong>Tip:</strong> Set realistic production rates
                          and daily hours for accurate scheduling. The system
                          will automatically distribute work based on these
                          settings.
                        </p>
                      </div>
                    )}
            ...
            {viewType === 'week' || viewType === 'day' ? (
              ...
              >
                ...
                {(() => {
                  ...
                  console.log(
                    `  • Total enabled molds: ${molds.filter((m) => m.enabled).length}`
                  );
                  console.log(
                    `  • Total order assignments: ${Object.keys(orderAssignments).length}`
                  );
                  ...
                  orders.forEach((order) => {
                    const compatible = getCompatibleMolds(order);
                    compatible.forEach((mold) =>
                      compatibleMoldIds.add(mold.moldId)
                    );
                  });
                  ...
                  const relevantMolds = molds.filter((m) => {
                    ...
                    const isCompatibleWithQueue = compatibleMoldIds.has(
                      m.moldId
                    );
                  ...
                  console.log(
                    `  • Relevant molds (with orders or compatible): ${relevantMolds.length}/${molds.filter((m) => m.enabled).length}`
                  );
                  console.log(
                    `  • Mold order counts:`,
                    sortedMolds.map(
