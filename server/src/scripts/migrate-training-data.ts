...
async function migrateTrainingData() {
  ...
  try {
    ...
    console.log(
      '📦 Step 3: Exporting training question options from DEV database...'
    );
    const devOptions = await devDb
      .select()
      .from(schema.trainingQuestionOptions);
    console.log(`   Found ${devOptions.length} training question options`);

    // Step 4: Export employee training records from DEV
    console.log(
      '📦 Step 4: Exporting employee training records from DEV database...'
    );
    const devRecords = await devDb
      .select()
      .from(schema.employeeTrainingRecords);
    console.log(`   Found ${devRecords.length} employee training records`);

    // Step 5: Export employee quiz attempts from DEV
    console.log(
      '📦 Step 5: Exporting employee quiz attempts from DEV database...'
    );
    ...
    for (const module of devModules) {
      ...
      console.log(
        `   Migrated module ID ${id} → ${inserted.id}: ${module.title}`
      );
    }
    ...
    for (const question of devQuestions) {
      ...
      if (!newModuleId) {
        console.error(
          `   ⚠️  Skipping question ${id} - module ${moduleId} not found in mapping`
        );
        continue;
      }
      ...
      console.log(
        `   Migrated question ID ${id} → ${inserted.id} (module ${moduleId} → ${newModuleId})`
      );
    }
    ...
    for (const option of devOptions) {
      ...
      if (!newQuestionId) {
        console.error(
          `   ⚠️  Skipping option ${id} - question ${questionId} not found in mapping`
        );
        continue;
      }

      await prodDb
        .insert(schema.trainingQuestionOptions)
        .values({ ...optionData, questionId: newQuestionId });
      console.log(
        `   Migrated option ID ${id} (question ${questionId} → ${newQuestionId})`
      );
    }
    ...
    for (const record of devRecords) {
      ...
      if (!newModuleId) {
        console.error(
          `   ⚠️  Skipping record ${id} - module ${moduleId} not found in mapping`
        );
        continue;
      }

      const [inserted] = await prodDb
        .insert(schema.employeeTrainingRecords)
        .values({ ...recordData, moduleId: newModuleId })
        .returning({ id: schema.employeeTrainingRecords.id });
      recordIdMap[id] = inserted.id;
      console.log(
        `   Migrated record ID ${id} → ${inserted.id} (module ${moduleId} → ${newModuleId})`
      );
    }
    ...
    for (const attempt of devAttempts) {
      ...
      if (!newRecordId || !newModuleId) {
        console.error(
          `   ⚠️  Skipping attempt ${id} - record ${trainingRecordId} or module ${moduleId} not found`
        );
        continue;
      }
    ...
    for (const entry of devMatrix) {
      ...
      await prodDb.insert(schema.trainingMatrix).values(matrixData);
      migratedMatrixCount++;
    ...
    if (skippedMatrixCount > 0) {
      console.log(
        `   ⚠️  Skipped ${skippedMatrixCount} entries with employee references (employees not in production)`
      );
    }
    console.log(
      `   ✅ Migrated ${migratedMatrixCount} training matrix entries`
    );
    ...
    const prodQuestionsCount = await prodDb
      .select()
      .from(schema.trainingQuestions);
    const prodOptionsCount = await prodDb
      .select()
      .from(schema.trainingQuestionOptions);
    const prodRecordsCount = await prodDb
      .select()
      .from(schema.employeeTrainingRecords);
    const prodAttemptsCount = await prodDb
      .select()
      .from(schema.employeeQuizAttempts);
    const prodMatrixCount = await prodDb.select().from(schema.trainingMatrix);

    console.log('\n📊 Migration Summary:');
    console.log('═'.repeat(60));
    console.log(
      `Training Modules:        ${devModules.length} → ${prodModulesCount.length}`
    );
    console.log(
      `Training Questions:      ${devQuestions.length} → ${prodQuestionsCount.length}`
    );
    console.log(
      `Question Options:        ${devOptions.length} → ${prodOptionsCount.length}`
    );
    console.log(
      `Employee Records:        ${devRecords.length} → ${prodRecordsCount.length}`
    );
    console.log(
      `Quiz Attempts:           ${devAttempts.length} → ${prodAttemptsCount.length}`
    );
    console.log(
      `Training Matrix Entries: ${devMatrix.length} → ${prodMatrixCount.length}`
    );
    console.log('═'.repeat(60));
