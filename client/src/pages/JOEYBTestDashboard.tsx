...
export default function JOEYBTestDashboard() {
  ...
  const queueCounts = useMemo(() => {
    const cnc = allOrders.filter((o: any) => isOrderInDepartment(o, 'CNC')).length;
    const gunsmith = allOrders.filter((o: any) => isOrderInDepartment(o, 'Gunsmith')).length;
    return { cnc, gunsmith };
  }, [allOrders]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {navCards.map((card) => (
          <Link key={card.href} href={card.href}>
            <Card className={`hover:shadow-lg transition-shadow cursor-pointer border-2 ${card.hoverBorder} h-full`}>
              <CardContent className="p-4 text-center">
                <div className="relative inline-block">
                  <card.icon className={`w-8 h-8 ${card.iconColor} mx-auto mb-3`} />
                  {card.badge !== null && (
                    <Badge className={`absolute -top-2 -right-4 text-xs px-1.5 py-0.5 ${card.badgeColor}`}>
                      {card.badge}
                    </Badge>
                  )}
                </div>
