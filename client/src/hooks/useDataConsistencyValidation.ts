...
export function useDataConsistencyValidation(
  ...
  useEffect(() => {
    ...
    if (features.bottom_metal !== separateStateVars.bottomMetal) {
      if (features.bottom_metal && !separateStateVars.bottomMetal) {
        inconsistencies.push(
          `❌ Bottom Metal: features.bottom_metal="${features.bottom_metal}" but bottomMetal state is empty`
        );
      } else if (!features.bottom_metal && separateStateVars.bottomMetal) {
        inconsistencies.push(
          `❌ Bottom Metal: bottomMetal state="${separateStateVars.bottomMetal}" but features.bottom_metal is empty`
        );
      } else if (features.bottom_metal && separateStateVars.bottomMetal) {
        inconsistencies.push(
          `❌ Bottom Metal: Mismatch - features.bottom_metal="${features.bottom_metal}" vs bottomMetal="${separateStateVars.bottomMetal}"`
        );
      }
    }
    ...
    if (
      JSON.stringify(featuresOther.sort()) !== JSON.stringify(stateOther.sort())
    ) {
      inconsistencies.push(
        `❌ Other Options: Mismatch - features.other_options=[${featuresOther.join(',')}] vs otherOptions=[${stateOther.join(',')}]`
      );
    }
    ...
    if (
      JSON.stringify(featuresRail.sort()) !== JSON.stringify(stateRail.sort())
    ) {
      inconsistencies.push(
        `❌ Rail Accessory: Mismatch - features.rail_accessory=[${featuresRail.join(',')}] vs railAccessory=[${stateRail.join(',')}]`
      );
    }
    ...
    if (featuresPaint !== separateStateVars.paintOptions) {
      if (featuresPaint && !separateStateVars.paintOptions) {
        inconsistencies.push(
          `❌ Paint Options: features has paint="${featuresPaint}" but paintOptions state is empty`
        );
      } else if (!featuresPaint && separateStateVars.paintOptions) {
        inconsistencies.push(
          `❌ Paint Options: paintOptions state="${separateStateVars.paintOptions}" but features has no paint value`
        );
      } else if (featuresPaint && separateStateVars.paintOptions) {
        inconsistencies.push(
          `❌ Paint Options: Mismatch - features paint="${featuresPaint}" vs paintOptions="${separateStateVars.paintOptions}"`
        );
      }
    }
    ...
    features,
    separateStateVars.bottomMetal,
    separateStateVars.otherOptions,
    separateStateVars.railAccessory,
    separateStateVars.paintOptions,
  ]);
}
