...
export default function MonthlyShippedReport() {
  const [selectedMonth, setSelectedMonth] = useState('9');
  const [selectedYear, setSelectedYear] = useState('2025');

  const { data: reportData, isLoading } = useQuery<ReportSummary>({
    queryKey: ['/api/reports/monthly-shipped', selectedMonth, selectedYear],
    queryFn: async () => {
      const response = await fetch(
        `/api/reports/monthly-shipped?month=${selectedMonth}&year=${selectedYear}`
      );
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl font-bold tracking-tight">
          {monthName} {selectedYear} SHIPPED Orders
        </h1>
        <p className="text-muted-foreground">
          Financial report showing all orders that were shipped in {monthName}{' '}
          {selectedYear}
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Detailed breakdown of all orders shipped in {monthName}{' '}
            {selectedYear}
          </CardDescription>
        </CardHeader>
        <CardContent>
