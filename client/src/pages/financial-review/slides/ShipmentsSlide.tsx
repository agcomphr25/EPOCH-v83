...
export default function ShipmentsSlide() {
  const { data = [], isLoading } = useQuery<ShipmentRow[]>({
  ...
  return (
    <div className="h-full flex flex-col px-10 py-8">
      <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-1">Products Shipped</h2>
        ...
        <>
          <div className="flex gap-8 mb-6">
            <div className="bg-blue-50 dark:bg-blue-900/30 rounded-lg px-6 py-3">
              ...
              <div className="text-3xl font-bold text-blue-600 dark:text-blue-300">{total.toLocaleString()}</div>
            </div>
            {latest && (
              <div className="bg-green-50 dark:bg-green-900/30 rounded-lg px-6 py-3">
                <div className="text-sm text-gray-500 dark:text-gray-400">Latest Month ({latest.month})</div>
                <div className="text-3xl font-bold text-green-600 dark:text-green-300">{latest.shipments.toLocaleString()}</div>
              </div>
            )}
          </div>
