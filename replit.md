...
## Overview
EPOCH v8 is a comprehensive Manufacturing ERP system designed for small manufacturing companies specializing in customizable products. It aims to streamline operations, enhance efficiency, and improve scalability through end-to-end order management, inventory tracking, employee portal functionality, and quality control workflows. The project's vision is to become the leading ERP solution for small-to-medium customizable product manufacturers. It is a full-stack TypeScript application with a React frontend and Express backend, featuring Progressive Web App (PWA) capabilities for deployment to web and mobile platforms via Capacitor.
...
- **Capability-Based Permissions**: Simplified 3-role system (ADMIN, EMPLOYEE, OWNER) with individual capability assignments.
...
- **CI/CD**: Implemented pre-commit hooks and GitHub Actions for automated quality checks.
...
- **Key Features**:
    - **Order Management**: Dynamic product configuration, editing, and streamlined order-to-production.
    - **Production Scheduling**: Layup Scheduler with auto-scheduling, priority scoring, and mold matching.
    - **Department Management**: Department-specific views, detailed order information, and defined workflow progression.
    ...
    - **Metal Accessories Tracker**: Production demand forecasting.
    ...
    - **Quality Control**: Workflows for digital signature capture, validation, and checklist submissions.
    - **Reporting**: Sales order PDF generation.
    - **Payment Tracking**: Integrated 'PAID' badge functionality.
    - **Shipping Integration**: UPS API for label generation and a Shipping Tracker with manual entry for various carriers.
    ...
    - **Training Management System**: Comprehensive platform for module creation (with quizzes and evaluations), employee training records, certification management, legacy training matrix import (CSV, PDF, Google Sheets), automatic certificate generation, and a visual Training Matrix View with completion tracking. Includes quiz question types like MULTIPLE_CHOICE, TRUE_FALSE, SHORT_ANSWER.
    - **AI-Powered Smart Sorting**: Intelligent dropdown sorting for Action Inlet options based on user behavior.
    - **Calendar Integration**: Displays Google Calendar events via OAuth.
    - **Azure Document Intelligence**: AI-powered document analysis for extracting data from various document types.
...
- Google Sheets (Training Matrix Import)