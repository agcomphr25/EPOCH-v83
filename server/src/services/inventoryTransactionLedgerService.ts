...
function eventHash(input: InventoryLedgerEntryInput, txNumber: string): string {
  return sha256Hex(
    canonicalize({
      ...
      sourceRecordId:
        input.sourceRecordId != null ? String(input.sourceRecordId) : null,
...
export async function recordInventoryLedgerEntry(
  ...
  if (input.transactionType === 'REVERSAL' && !input.reversedTransactionId) {
    throw new Error(
      'INVENTORY LEDGER: reversal entries must link reversedTransactionId'
    );
  }
...
export async function recordInventoryBalanceLedgerChange(
  ...
  if (!item) {
    throw new Error(
      `INVENTORY LEDGER: part number ${input.agPartNumber} not found`
    );
  }
  ...
  return recordInventoryLedgerEntry(
    {
      ...
      unitOfMeasure:
        input.unitOfMeasure ?? item.usageUnit ?? item.purchaseUnit ?? 'EA',
...
export async function reverseInventoryLedgerEntry(params: {
  ...
  return db.transaction(async (tx) => {
    ...
    if (!original) {
      throw new Error(
        `INVENTORY LEDGER: transaction ${params.transactionId} not found`
      );
    }
    if (original.transactionType === 'REVERSAL') {
      throw new Error(
        'INVENTORY LEDGER: reversal entries cannot be reversed directly'
      );
    }
    ...
    if (existingReversal) {
      throw new Error(
        `INVENTORY LEDGER: transaction ${params.transactionId} already has a reversal`
      );
    }
    ...
    const [balance] = await tx
      ...
      .where(
        sql`${inventoryBalances.agPartNumber} = ${original.agPartNumber} AND ${inventoryBalances.locationId} IS NOT DISTINCT FROM ${original.locationId}`
      )
      .limit(1);

    const currentQuantity =
      balance?.quantityOnHand ?? Number(original.quantityAfter);
    ...
    if (params.restoreMaterialCustody) {
      ...
      if (
        ...
        throw new Error(
          'INVENTORY LEDGER: material custody reversal does not match the original transaction'
        );
      ...
      if (!lot)
        throw new Error(
          'INVENTORY LEDGER: material lot for reversal was not found'
        );
      ...
      if (!unit)
        throw new Error(
          'INVENTORY LEDGER: Receiving custody unit for reversal was not found'
        );
      ...
      if (
        !requirement ||
        Number(requirement.issued_quantity) < custody.quantity
      )
        throw new Error(
          'INVENTORY LEDGER: released BOM demand cannot accept this reversal'
        );
      await tx.execute(sql`
...
export async function listInventoryLedgerEntries(
  ...
  if (params.agPartNumber) {
    conditions.push(
      eq(inventoryTransactionLedger.agPartNumber, params.agPartNumber)
    );
  }
  if (params.lotId) {
  ...
  if (params.sourceModule) {
    conditions.push(
      eq(inventoryTransactionLedger.sourceModule, params.sourceModule)
    );
  }
  ...
  return db
    ...
    .orderBy(
      desc(inventoryTransactionLedger.createdAt),
      asc(inventoryTransactionLedger.transactionNumber)
    )
    .limit(limit);
}

export async function verifyInventoryLedgerHashes(
  params: {
    limit?: number;
  } = {}
): Promise<{
  ...
  const rows = await db
    ...
    .from(inventoryTransactionLedger)
    .orderBy(
      asc(inventoryTransactionLedger.createdAt),
      asc(inventoryTransactionLedger.transactionNumber)
    )
    .limit(limit);
 ...
 */
export async function verifyInventoryLedgerHashesByIds(ids: string[]): Promise<{
  checked: number;
  mismatches: Array<{
    id: string;
    transactionNumber: string;
