...
Authentication: Production-ready backend authentication system with HTTP-only session cookies, bcrypt password hashing, and server-side session validation. Authentication is REQUIRED FOR ALL ENVIRONMENTS (development and production) for testing purposes. All dashboard routes validate sessions against backend on mount, preventing unauthorized access. Session tokens are stored securely in HTTP-only cookies (not accessible to JavaScript). Fixed October 1, 2025: Completed transition from localStorage to secure backend authentication with proper session management, cleanup, and modular routing integration.
Personalized Dashboards: Implemented secure auto-redirect system where each user is automatically routed to their specific dashboard upon login based on username mapping. All 21 active users have personalized dashboard routes with backend-validated sessions and consistent logout functionality that clears server-side sessions. Dashboard mapping is centralized in `client/src/config/dashboardMapping.ts` with `getDashboardRoute()` function returning user-specific routes. Login flow in `Login.tsx` automatically redirects to personalized dashboards post-authentication. Default `Dashboard.tsx` checks for personalized routes and redirects accordingly. All dashboard routes are registered in `App.tsx` with proper routing (e.g., `/agrace-dashboard`, `/tims-dashboard`, etc.). React Fragment issues fixed in PipelineVisualization component (October 6, 2025) by replacing fragments with div elements to prevent data-replit-metadata prop errors.
...
-   **Core Features**:
    ...
    -   **Training System**: Complete certification and quiz system for employee training modules with 9 aerospace/manufacturing courses. Features PDF viewer for training materials, 10-question quizzes with 80% passing score requirement, automatic certificate generation, and completion tracking. Training modules include: Preservation & FOD Training, Chemical Handling, Fire Safety, ITAR Compliance, AS9100 Orientation, Counterfeit Prevention, Ethics in Quality, Nonconforming Items (Leader), and Shut Down Procedures (Leader). PDFs served via dedicated `/attached_assets` route with environment-aware path resolution (development: root folder, production: dist/attached_assets). Build process automatically copies attached_assets folder to dist for deployment (October 2, 2025).
