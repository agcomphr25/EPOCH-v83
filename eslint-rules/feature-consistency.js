...
module.exports = {
  meta: {
    ...
    messages: {
      useFeatureObject:
        'Use "features.{{field}}" instead of "{{field}}" for consistent data access',
      avoidDirectAccess:
        'Direct field access "{{field}}" should use features object: "features.{{field}}"',
    },
  },
  ...
  create(context) {
    return {
      Identifier(node) {
        ...
        const camelCase = fieldName.replace(/_([a-z])/g, (g) =>
          g[1].toUpperCase()
        );
