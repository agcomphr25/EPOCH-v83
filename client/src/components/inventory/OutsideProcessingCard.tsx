        ...
        <CardContent>
          {locations.length > 0 ? (
            ...
            <p className="text-sm text-gray-500">
              No processing locations configured
            </p>
          )}
        </CardContent>
      </Card>
              ...
              {jobs.map((job: any) => (
                <div key={job.jobId} className="border rounded p-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      className={`${job.status === 'COMPLETED' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}
                    >
                      {job.status}
                    </Badge>
                  </div>
                  <div className="flex items-center gap-4 text-sm text-gray-600">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {job.expectedReturnDate
                        ? new Date(job.expectedReturnDate).toLocaleDateString()
                        : 'TBD'}
                    </span>
                  </div>
                </div>
