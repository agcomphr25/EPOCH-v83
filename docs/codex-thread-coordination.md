...
| ready | P2 invoice customer part carryover | `codex/p2-invoice-customer-part` | `/finance/invoices/:id`, `/p2/packing-slip/:id` | `server/src/services/invoiceFromPackingSlip.ts` | P2 invoice lines carry the same customer-facing part number/SKU shown on the packing slip. Coordinate with the broader P2 packing slip invoice workflow branch before merging. |
