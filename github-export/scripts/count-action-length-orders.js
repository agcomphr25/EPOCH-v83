...
async function countOrdersWithActionLength() {
  try {
    ...
    orders.forEach((order) => {
      ...
      if (order.features) {
        ...
        if (
          ...
          order.features.action_length !== 'none'
        ) {
    ...
    console.log(
      `Percentage: ${((ordersWithActionLength / orders.length) * 100).toFixed(1)}%`
    );
    ...
    ordersWithActionData.forEach((order) => {
      console.log(
        `  ${order.orderId}: ${order.value} (from ${order.source}) - Model: ${order.modelId}`
      );
    });
  } catch (error) {
