...
async function countUnverifiedOrders() {
  try {
    ...
    const unverifiedOrders = orders.filter(
      (order) =>
        !order.isVerified ||
        order.isVerified === false ||
        order.isVerified === null
    );
    ...
    console.log(
      `Orders NOT verified against previous ERP: ${unverifiedOrders.length}`
    );
    console.log(
      `Orders verified against previous ERP: ${orders.length - unverifiedOrders.length}`
    );
    console.log(
      `Verification rate: ${(((orders.length - unverifiedOrders.length) / orders.length) * 100).toFixed(1)}%`
    );
    ...
    if (unverifiedOrders.length > 0) {
      ...
      unverifiedOrders.slice(0, 20).forEach((order, index) => {
        console.log(
          `${index + 1}. Order ID: ${order.orderId} | Status: ${order.status} | Department: ${order.currentDepartment || 'Not Set'} | Customer: ${order.customerName || order.customer || 'N/A'}`
        );
      });
