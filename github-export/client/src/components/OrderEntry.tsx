...
export default function OrderEntry() {
  ...
  const calculateTotalPrice = useCallback(() => {
    ...
    if (selectedModel) {
      const basePrice =
        priceOverride !== null ? priceOverride : selectedModel.price || 0;
      total += basePrice;
                          ...
                          if (
                            ...
                            return (
                              <div className="text-gray-500 text-sm">
                                No Other Options available (Features loaded:{' '}
                                {featureDefs.length}, Looking for:
                                other_options)
                                {featureDefs.length > 0 && (
                                  <div className="text-xs mt-1">
                                    Available feature IDs:{' '}
                                    {featureDefs.map((f) => f.id).join(', ')}
                                  </div>
                                )}
                              </div>
                            );
                                ...
                                <Checkbox
                                  ...
                                  onCheckedChange={(checked) => {
                                    ...
                                    if (checked) {
                                      ...
                                      setFeatures((prev) => ({
                                        ...prev,
                                        other_options: currentOther.filter(
                                          (item: string) =>
                                            item !== option.value
                                        ),
                                      }));
                      ...
                      <div className="space-y-2 max-h-48 overflow-y-auto border rounded-md p-3">
                        {(() => {
                          ...
                          if (!railsFeature || !railsFeature.options) {
                            return (
                              <div className="text-gray-500 text-sm">
                                No Rails options available (Features loaded:{' '}
                                {featureDefs.length}, Looking for:
                                rail_accessory)
                                {featureDefs.length > 0 && (
                                  <div className="text-xs mt-1">
                                ...
                                <Checkbox
                                  ...
                                  onCheckedChange={(checked) => {
                                    ...
                                    if (checked) {
                                      ...
                                      setFeatures((prev) => ({
                                        ...prev,
                                        rail_accessory: currentRails.filter(
                                          (item: string) =>
                                            item !== option.value
                                        ),
                                      }));
                    ...
                    <span className="text-blue-600 font-bold">
                      ...
                      {(() => {
                        ...
                        if (currentRails && currentRails.length > 0) {
                          ...
                          const totalPrice = currentRails.reduce(
