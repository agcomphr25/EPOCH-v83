...
- **Widget System**: Extensible dashboard widget architecture with `widgetRegistry.ts` supporting types: `metric_stat`, `metric_stat_group`, `hero_metric`, `shipment_trend`, `bubble_chart`, `kit_progress`, `signal_card`, `swim_lane_preview`. Config-driven layouts via `dashboardLayouts.ts` and `DashboardGrid`/`WidgetRenderer`. Includes `FlippableCard` component (`client/src/components/widgets/FlippableCard.tsx`) for 3D flip animations using Framer Motion. `DashboardFilterContext` (`client/src/contexts/DashboardFilterContext.tsx`) provides `timeRange` and `businessContext` state via `useDashboardFilters()` hook — PCC page is wrapped with the provider. `GET /api/shipping/weekly-history` endpoint returns last 12 operational weeks with real shipment counts (using `shared/weekUtils.ts` for Wed-Tue week calculation). HeroMetricWidget supports color-coded goal progress bars, pace indicators, and enhanced trend badges.
