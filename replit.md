...
Tikka compatibility guardrails: On the Order Entry page, if the stock model does NOT contain "Tikka" in the name, action inlet, barrel inlet, and bottom metal options with "Tikka" in the name are hidden. If a user changes the stock model from a Tikka model to a non-Tikka model with Tikka options already selected, those selections are automatically cleared with a toast notification. Visual indicators ("Tikka options hidden" badges) are shown on affected dropdowns.
