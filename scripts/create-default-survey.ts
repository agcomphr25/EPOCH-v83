...
async function createDefaultSurvey() {
  try {
    const surveyData = {
      ...
      description: "Six question customer satisfaction survey to evaluate product quality, delivery, service, and gather feedback",
      isActive: true,
      questions: [
        {
          id: "question_1",
        ...
        },
        {
          id: "question_5_nps",
          type: "nps",
          question: "How likely are you to recommend our company and products to others?",
          required: true,
          scale: {
            min: 1,
            max: 10,
    ...
    const result = await db.insert(customerSatisfactionSurveys).values(surveyData).returning();
