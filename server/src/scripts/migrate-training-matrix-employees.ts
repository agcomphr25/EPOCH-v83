  ...
  try {
    ...
    for (const devUser of devUsers) {
      ...
      if (prodUser && prodUser.employeeId) {
        ...
        console.log(
          `   Mapped ${devUser.username}: employee ${devUser.employeeId} → ${prodUser.employeeId}`
        );
      } else {
        console.log(
          `   ⚠️  User ${devUser.username} not found in PROD or has no employeeId`
        );
      }
    }
    ...
    for (const entry of employeeMatrixEntries) {
      ...
      if (!newEmployeeId) {
        console.log(
          `   ⚠️  Skipping entry ${id} - employee ${employeeId} not found in mapping`
        );
        skippedCount++;
        continue;
      }
