...
export default function LayupPluggingQueuePage() {
  ...
  const processedOrders = useMemo(() => {
    ...
    const scheduledOrdersWithDetails = currentSchedule
      .map((scheduleEntry: any) => {
        const orderDetails = availableOrders.find(
          (order: any) => order.orderId === scheduleEntry.orderId
        );
        if (!orderDetails) {
          console.warn(
            `⚠️ Scheduled order ${scheduleEntry.orderId} not found in available orders`
          );
          return null;
        }
    ...
    console.log(
      `📋 Department Manager: Found ${scheduledOrdersWithDetails.length} scheduled orders out of ${currentSchedule.length} schedule entries`
    );
    ...
    try {
      ...
      const nextWeekOrders = currentSchedule.filter((scheduleItem: any) => {
        ...
        try {
          ...
          console.warn(
            '🔍 Date parsing error for next week calculation:',
            scheduleItem,
            dateError
          );
          return false;
        ...
        <Card className="bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800">
          ...
          <CardContent>
            ...
            <p className="text-sm text-blue-600 dark:text-blue-400 mt-1">
              Orders scheduled for {format(nextWeekDates[0], 'MMM d')} -{' '}
              {format(nextWeekDates[4], 'MMM d')}
            </p>
          ...
          <CardTitle className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {currentWeekOrders.length > 0 && (
                <div className="flex items-center gap-2 mr-4">
                  <Button
                    ...
                    disabled={
                      selectedOrders.length === currentWeekOrders.length
                    }
                  >
              ...
              <Badge variant="outline" className="text-sm">
                Week: {format(currentWeekDates[0], 'MMM d')} -{' '}
                {format(currentWeekDates[4], 'MMM d')}
              </Badge>
              {selectedOrders.length > 0 && (
                <Badge
                  variant="secondary"
