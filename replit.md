...
-   **P1 PO Shipping QC Management**: Comprehensive tracking system for P1 purchase orders with department status tracking across the entire production pipeline. Non-stock PO items automatically bypass production and go directly to Shipping QC. Implemented with authentication middleware, optimized raw SQL queries to avoid Drizzle ORM LEFT JOIN issues, and real-time status badges for all PO items showing current department and production status. Features accordion-wrapped UI for customers and orders with cross-PO item selection capability - users can select individual items from different POs of the same customer to ship together. Includes fulfillment tracking system for items shipped through external systems - users can mark items as fulfilled to prevent duplicate shipping, with audit trail (who marked it, when) and automatic exclusion from selection/progression.
-   **P1 PO Shipping Workflow**: Complete UPS integration with multi-step shipping dialog for creating labels with configurable service levels (Ground, 2nd Day Air, Next Day Air, etc.) and billing options (shipper/third-party/consignee). Floating "Ship Selected" button enables shipping items from different POs together with single shipping label but separate packing slips per PO. Database schema includes shipment_records and shipment_items tables with UUID primary keys for tracking all shipments. OEM Shipments history page provides comprehensive search/filtering by customer name, tracking number, reference, and date range with pagination (20 per page). Features collapsible shipment cards displaying tracking numbers (copy-to-clipboard), UPS service type, weight, shipped date/time, and shipper identity. Individual document downloads for shipping labels (GIF) and packing slips (PDF) through authenticated endpoints. Custom queryFn implementation ensures all filter parameters properly serialize to backend query strings.
