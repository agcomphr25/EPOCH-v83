...
router.get(
  '/verify-certification/:employeeCode/:barcode',
  async (req: Request, res: Response) => {
    try {
      const { employeeCode } = req.params;
      ...
      if (!routing) {
        ...
        const basePartMatch =
          serializedItem.partNumber.match(/^(.+?)\s*Rev\s*\w+$/i);
        if (basePartMatch) {
      ...
      if (!routing) {
        return res
          .status(404)
          .json({ error: 'No routing configuration found for this part' });
      }
      ...
      if (currentIndex >= departmentSequence.length) {
        return res
          .status(400)
          .json({ error: 'Part has completed all departments' });
      }
      ...
      const certification =
        await db.query.p2EmployeePartCertifications.findFirst({
          where: and(
            eq(p2EmployeePartCertifications.employeeId, employee.id),
            eq(
              p2EmployeePartCertifications.partNumber,
      ...
      return res.json({
        ...
        traceabilityRequirements:
          (routing.traceabilityConfig as any)?.[nextDepartment] || [],
      });
    } catch (error: any) {
...
router.post('/start-task', async (req: Request, res: Response) => {
  try {
    ...
    if (!allowMultipleTasks) {
      ...
      if (employeeActiveTasks.length > 0) {
        const activePartName =
          employeeActiveTasks[0]?.partName ||
          employeeActiveTasks[0]?.partNumber ||
          'another part';
        return res.status(400).json({
    ...
    return res
      .status(500)
      .json({ error: error.message || 'Failed to start task' });
  }
});
...
router.post('/complete-task', async (req: Request, res: Response) => {
  try {
    const { taskId, employeeCode, barcode, notes } = req.body;
    ...
    if (workTask.employeeCode.toLowerCase() !== employeeCode.toLowerCase()) {
      return res
        .status(403)
        .json({ error: 'Only the assigned technician can complete this task' });
    }
    ...
    if (!barcodeMatch) {
      ...
      const serializedItemForBarcode =
      ...
      if (serializedItemForBarcode) {
        const travelerBarcode =
          serializedItemForBarcode.travelerBarcode?.toLowerCase();
        const systemBarcode = serializedItemForBarcode.barcode?.toLowerCase();
        barcodeMatch =
          scannedBarcode === travelerBarcode ||
          scannedBarcode === systemBarcode;
      }
    }
    ...
    if (!barcodeMatch) {
      return res
        .status(400)
        .json({ error: 'Barcode does not match the started task' });
    }
    ...
    if (currentDepartment === 'Final QC') {
      ...
      const hasFinalQCFailures =
        itemMetadata?.finalQcFailures?.length > 0 ||
      ...
      if (hasFinalQCFailures) {
        ...
        const hasToleranceAuthorization =
          itemMetadata?.toleranceDeviationApproved === true;
        ...
        if (!hasToleranceAuthorization) {
          ...
          if (serializedItem.poItemId) {
            const { p2PurchaseOrders, p2PurchaseOrderItems } = await import(
              '../../schema'
            );
          ...
          if (!poHasAuthorizer) {
            return res.status(403).json({
              ...
              gatingFailed: true,
              message:
                'This item has failed Final QC inspections. Tolerance authorizer signature is required before the item can proceed. ' +
    ...
    await db.insert(p2SerializedItemEvents).values({
      ...
      notes:
        notes ||
        `Completed ${currentDepartment} - Duration: ${durationMinutes} minutes`,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const startedAt = workTask.startedAt
        ? new Date(workTask.startedAt)
        : new Date();
      ...
      console.log(
        `[ADMIN] Force-completed task ${taskId} for ${workTask.employeeName} in ${workTask.department}`
      );
      ...
      return res
        .status(500)
        .json({ error: error.message || 'Failed to force-complete task' });
    }
  }
);
    ...
    try {
      ...
      const traceabilityRecords =
        await db.query.p2SerializedItemTraceability.findMany({
          where: eq(
            p2SerializedItemTraceability.serializedItemId,
            serializedItemId
          ),
      ...
      const customDataRecords =
        await db.query.p2SerializedItemCustomData.findMany({
          where: eq(
            p2SerializedItemCustomData.serializedItemId,
            serializedItemId
          ),
      ...
      return res
        .status(500)
        .json({ error: 'Failed to get traceability report' });
    }
  }
);
