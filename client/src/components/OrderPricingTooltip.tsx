...
export default function OrderPricingTooltip({
  orderId,
  children,
}: OrderPricingTooltipProps) {
                  ...
                  <div className="text-sm ml-6">
                    <div className="mb-1">
                      ...
                      <span className="ml-2 text-gray-600">
                        {stockModels?.find((m) => m.id === order.modelId)
                          ?.displayName || order.modelId}
                      </span>
                    </div>
                    {order.handedness && (
                      <div className="mb-1">
                ...
                <div className="border-t pt-3">
                  ...
                  <div className="text-sm ml-6 space-y-1">
                    {Object.entries(order.features).map(([key, value]) => {
                      ...
                      if (
                        key === 'paint_options_combined' &&
                        typeof value === 'string' &&
                        value.includes(':')
                      ) {
                        const [paintFeatureId, optionValue] = value.split(':');
                        const paintFeature = features?.find(
                          (f) => f.id === paintFeatureId
                        );
                        if (
                          paintFeature?.options &&
                          Array.isArray(paintFeature.options)
                        ) {
