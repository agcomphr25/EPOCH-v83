...
| ready | P2 invoice customer part carryover and edit persistence | `codex/p2-invoice-customer-part` | `/finance/invoices/:id`, `/finance/invoices/:id/edit`, `/p2/packing-slip/:id`, `/p2-control-center` | `server/src/services/invoiceFromPackingSlip.ts`, `client/src/pages/InvoiceFormPage.tsx`, `client/src/pages/InvoiceDetailPage.tsx`, `client/src/components/MediaAttachmentPicker.tsx`, `client/src/components/p2/P2ShippingTab.tsx`, `client/src/components/p2/ShipmentSummaryModal.tsx`, `server/utils/pdf/arInvoicePdf.ts`, `server/src/routes/arInvoices.ts`, `server/src/routes/p2Shipping.ts` | P2 invoice lines/PDF carry the same customer-facing part number/SKU shown on the packing slip; invoice edit keeps current customer/PO/line data visible; invoice edit can split a P2 line quantity into same-part lines with different prices; shipment creation now requires manual billing bucket/CLIN assignment for selected finalized serials, and P2 invoices group lines by billing bucket with the bucket label in the description; invoice sends CC accounting; attachment uploads fall back when presigned storage URLs are unavailable; invoice attachments can be opened by media ID and selected for send; P2 send recipients fall back through linked PO, packing slip, lot, and customer-name sources; `/api/ar-invoices/:id/email-recipients?debug=1` provides a no-send preflight; SENT invoices without an AR journal can still be posted. Coordinate with the broader P2 packing slip invoice workflow branch before merging. |
