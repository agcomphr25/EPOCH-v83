...
-   **Core Features**:
    ...
    -   **Department Manager**: Enhanced navigation with department-specific views and comprehensive order details via tooltips. **CRITICAL WORKFLOW (NEVER CHANGES)**: Orders entered → P1 Production Queue → (Layup Scheduler) → Layup/Plugging → Barcode → CNC → Finish → Paint → Finish QC → Gunsmith → Shipping QC → Shipping → Fulfilled. **EXCEPTIONS**: Flat tops, bottom metal only items, or miscellaneous items only may skip departments. CNC Department features dual-queue system with automatic routing based on order features. Finish Department features technician assignment with routing to Paint. Paint Department features multi-select functionality and progression to Finish QC. Finish QC Department organizes orders by assigned technician with alphabetical/numerical sorting, displays texture and paint color information on cards, and provides multi-select progression to Gunsmith department. Gunsmith Department features due date categorization with color-coded priority levels while maintaining original card grid layout and multi-select functionality, progresses to Shipping QC. Shipping QC progresses to Shipping. Shipping progresses to Fulfilled (final department). Orders with "no_rail" bypass gunsmith work entirely, routing directly from CNC to Finish department.
