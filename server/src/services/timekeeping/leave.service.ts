import { db } from "../../../db";
import { leaveEntriesTable, timesheetsTable } from "../../schema/timekeeping";
import { eq, and, gte, lte } from "drizzle-orm";
import type { LeaveEntry } from "../../schema/timekeeping";
...
async function validateLeaveDateMutable(employeeId: number, date: string): Promise<string | null> {
  ...
  const FINALIZED = new Set(["certified", "locked", "correction_requested", "correction_approved"]);
  const hasFinalized = timesheets.some(t => FINALIZED.has(t.status));
  if (hasFinalized) {
    return "Cannot modify leave in a finalized (certified or locked) timesheet period.";
  }
...
async function validateLeaveDate(employeeId: number, date: string): Promise<string | null> {
  ...
  const hasFinalized = timesheets.some(t => FINALIZED.has(t.status));
  if (hasFinalized) {
    return "Cannot add leave to a finalized timesheet period. Use the correction workflow instead.";
  }
