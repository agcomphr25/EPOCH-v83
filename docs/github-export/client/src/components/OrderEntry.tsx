...
export default function OrderEntry() {
    ...
    const loadInitialData = async () => {
      try {
        ...
        const paintFeatures = allFeatures.filter(
          (f: any) => f.category === 'paint_options'
        );
        const nonPaintFeatures = allFeatures.filter(
          (f: any) => f.category !== 'paint_options'
        );
        ...
        const allPaintOptions = paintFeatures.reduce(
          (acc: any[], feature: any) => {
            if (feature.options && Array.isArray(feature.options)) {
              const optionsWithFeatureId = feature.options.map(
                (option: any) => ({
  ...
  const calculateTotal = useCallback(() => {
    ...
    for (const [featureId, value] of Object.entries(features)) {
      ...
      if (feature && feature.options && Array.isArray(feature.options)) {
        if (feature.type === 'checkbox') {
          ...
          // Debug logging for "Other Options"
          if (featureId === 'other_options') {
            console.log('Processing multiselect other_options:', {
              selectedValues,
              options: feature.options,
            });
          }
    ...
    const additionalItemsCost = additionalItems.reduce(
    ...
    if (customer && customer.customerType) {
      const customerType = customerTypes.find(
        (ct) => ct.id === customer.customerType
      );
      if (customerType && customerType.discountPercentage) {
        customerTypeDiscount =
          (basePrice + featureCost) * (customerType.discountPercentage / 100);
      }
    }
    ...
    if (customer) {
      const discount = persistentDiscounts.find(
        (pd) => pd.customerId === customer.id
      );
      if (discount) {
        if (discount.type === 'percentage') {
          persistentDiscount =
            (basePrice + featureCost) * (discount.value / 100);
        } else {
    ...
    const totalDiscount =
      discountAmount + customerTypeDiscount + persistentDiscount;
    const subtotal =
      basePrice + featureCost + additionalItemsCost - totalDiscount;
  ...
  const filteredPaintOptions = paintFeatures.filter((feature) => {
    ...
    return (
      feature.displayName?.toLowerCase().includes(searchTerm) ||
      feature.name?.toLowerCase().includes(searchTerm) ||
      feature.options?.some((option: any) =>
            ...
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                {orderStatus !== 'DRAFT' && (
                  <Badge
                    variant={
                      orderStatus === 'CONFIRMED' ? 'default' : 'secondary'
                    }
                  >
                    {orderStatus}
                  </Badge>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  />
                  {errors.orderId && (
                    <p className="text-red-500 text-sm">{errors.orderId}</p>
                  )}
                </div>
                <div className="space-y-2">
              ...
              {featureDefs.map((featureDef) => (
                <div key={featureDef.id} className="space-y-2">
                  ...
                  {featureDef.type === 'search' &&
                    featureDef.id === 'paint_options_combined' && (
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button
                            variant="outline"
                          ...
                          >
                            {features[featureDef.id]
                              ? allPaintOptions.find(
                                  (option) =>
                                    option.value === features[featureDef.id]
                                )?.label
                              : 'Select paint option...'}
                            <ChevronDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                          </Button>
                      ...
                      {featureDef.options?.map((option) => (
                        ...
                        >
                          <input
                            ...
                            checked={
                              Array.isArray(features[featureDef.id])
                ...
                <div className="border-t pt-2">
                  ...
                  <div className="space-y-1 text-sm">
                    {Object.entries(features).map(([featureId, value]) => {
                      if (!value || featureId === 'specialInstructions')
                        return null;
                      ...
                      if (featureId === 'paint_options_combined') {
                        ...
                        const paintFeature = paintFeatures.find(
                          (f) => f.id === paintFeatureId
                        );
                        if (paintFeature && paintFeature.subCategory) {
                          const subCategory = subCategories.find(
                            (sc) => sc.id === paintFeature.subCategory
                          );
                          const option = paintFeature.options?.find(
                            (opt: any) => opt.value === optionValue
                          );
