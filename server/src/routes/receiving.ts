...
import {
  ...
  generateReceivingUnitBarcodeValue,
...
function uploadReceiptDocument(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
const RECEIVING_MEDIA_STORAGE_DIR = path.join(
  'uploads',
  'media-library',
  'receiving'
);
...
function sanitizeUploadFilename(filename: string): string {
  ...
  const baseName =
    path.basename(filename, ext).replace(/[^a-zA-Z0-9.-]/g, '_') || 'document';
  return `${Date.now()}-${randomUUID()}-${baseName}${ext}`;
}

async function saveReceivingMediaFileLocally(
...
async function saveReceivingMediaFile(
  file: Express.Multer.File,
  receiptId: number
): Promise<string> {
  try {
    ...
    console.warn(
      'Receiving document durable upload unavailable outside production; using local fallback',
      {
        receiptId,
        filename: file.originalname,
        reason,
...
async function getOrCreateReceivingMediaFolder(
  user: AuthUser,
  displayName: string
): Promise<string | null> {
  try {
    const [existing] = await db
      ...
      .where(
        and(
          eq(mediaFolders.name, RECEIVING_MEDIA_FOLDER_NAME),
...
router.use(async (_req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res
      .status(500)
      .json({ error: 'Receiving project material schema is not ready' });
  }
});
...
const inspectionPlanContextSchema = z.object({
  ...
  supplierStatus: z
    .enum(['APPROVED', 'PROBATION', 'CONDITIONAL', 'BLOCKED'])
    .optional()
    .nullable(),
...
async function assertUnitOwnership(
  receiptId: number,
  unitId: number
): Promise<ReceivedUnit | null> {
  const [unit] = await db
    .select()
    .from(receivedUnits)
    .where(
...
async function resolveDefaultTargetProjectId(
  receipt: Receipt,
  line: ReceiptLine
): Promise<string | null> {
...
async function syncProjectReceivedMaterial(
  unitId: number,
  user: AuthUser,
  notes?: string | null
): Promise<void> {
  ...
  async (_req: Request, res: Response) => {
    try {
      const plans = await db
        ...
        .orderBy(
          desc(receivingInspectionPlans.priority),
          desc(receivingInspectionPlans.createdAt)
        );
      res.json(plans);
    } catch (err: any) {
      ...
      res
        .status(500)
        .json({ error: 'Failed to fetch receiving inspection plans' });
    }
  }
);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [plan] = await db
        .insert(receivingInspectionPlans)
        .values(parsed)
        .returning();
      ...
      res
        .status(500)
        .json({ error: 'Failed to create receiving inspection plan' });
    }
  }
);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!plan)
        return res
          .status(404)
          .json({ error: 'Receiving inspection plan not found' });
      res.json(plan);
    } catch (err: any) {
      if (err instanceof z.ZodError) {
        return res
      ...
      res
        .status(500)
        .json({ error: 'Failed to update receiving inspection plan' });
    }
  }
);

router.get(
  '/inspection-plans/evaluate',
  requireReceivingAccess,
  async (req: Request, res: Response) => {
    try {
      const context = inspectionPlanContextSchema.parse({
        ...
        riskLevel: req.query.riskLevel
          ? String(req.query.riskLevel).toUpperCase()
          : undefined,
        supplierName: req.query.supplierName,
        supplierStatus: req.query.supplierStatus
          ? String(req.query.supplierStatus).toUpperCase()
          : undefined,
      ...
      res
        .status(500)
        .json({ error: 'Failed to evaluate receiving inspection plan' });
    }
  }
);
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    `);
      ...
      res
        .status(500)
        .json({ error: 'Failed to fetch department receiving actions' });
    }
  }
);
...
router.get('/', requireReceivingAccess, async (req: Request, res: Response) => {
  try {
    ...
    if (status && status !== 'all')
      conditions.push(eq(receipts.status, status as string));
    if (vendorId)
      conditions.push(eq(receipts.vendorId, parseInt(vendorId as string)));
    if (vendorPoId)
      conditions.push(eq(receipts.vendorPoId, parseInt(vendorPoId as string)));
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (vendorPoId) {
        ...
        if (existRows.length > 0) {
          ...
          const [existingReceipt] = await db
            .select()
            .from(receipts)
            .where(eq(receipts.id, existingId));
          const lines = await db
            .select()
            .from(receiptLines)
            .where(eq(receiptLines.receiptId, existingId));
          const units = await db
            .select()
            .from(receivedUnits)
            .where(eq(receivedUnits.receiptId, existingId));
          const documents = await db
            .select()
            .from(receiptDocuments)
            .where(eq(receiptDocuments.receiptId, existingId));
          const auditLogs = await db
            .select()
            .from(receiptAuditLog)
            .where(eq(receiptAuditLog.receiptId, existingId))
      ...
      await logAudit(
        ...
        'receipt_created',
        user?.employeeId,
        body.receiverDisplayName ?? 'Unknown',
        {
          receiptNumber,
      ...
      res
        .status(201)
        .json({ ...receipt, lines, units: [], documents: [], auditLog: [] });
    } catch (err: any) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const units = await db
        .select()
        .from(receivedUnits)
        .where(eq(receivedUnits.receiptId, id));
      const documents = await db
        .select()
        .from(receiptDocuments)
        .where(eq(receiptDocuments.receiptId, id));
      const auditLogs = await db
        .select()
        .from(receiptAuditLog)
        .where(eq(receiptAuditLog.receiptId, id))
        .orderBy(desc(receiptAuditLog.createdAt));
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.status(status).json({
        error: err.message ?? 'Failed to preview receipt reversal',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (result.vendorPoStatus && result.vendorPoStatus !== 'Fully Received') {
        const [receipt] = await db
          .select({ vendorPoId: receipts.vendorPoId })
          .from(receipts)
          .where(eq(receipts.id, parseInt(req.params.id, 10)));
        if (receipt?.vendorPoId)
          await syncLinkedPartsRequestsReceivedForVendorPo(
            receipt.vendorPoId,
            actorName(user)
          );
      }
      ...
      const status = err instanceof ReceiptReversalError ? err.status : 500;
      res.status(status).json({
        error: err.message ?? 'Failed to reverse receipt',
        blockers: err.blockers ?? [],
      });
    }
  }
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const reopenReason =
        typeof rawPatch.reopenReason === 'string'
          ? rawPatch.reopenReason.trim()
          : '';
      ...
      const result = await db.transaction(async (tx) => {
        ...
        if (!existing)
          return {
            updated: null as Receipt | null,
            parentPoStatus: null as string | null,
            reopened: false,
          };
        ...
        if (updates.status === 'complete') {
          ...
          if (blockingUnits.length > 0) {
            const blockers = blockingUnits.map((unit) => ({
              ...
              reason:
                unit.disposition === 'document_hold'
            ...
            const error: any = new Error(
              'Receipt cannot be completed until all units are inspected, released from document hold, and put away.'
            );
            error.status = 422;
        ...
        const reopened =
          existing.status === 'complete' && updates.status === 'in_progress';
        ...
        if (updates.status === 'complete' && updated.vendorPoId) {
          ...
          const recvRows = sqlRows<{
            vendor_po_item_id: number | null;
          ...
          for (const row of recvRows) {
            if (row.vendor_po_item_id != null) {
              receivedByItem.set(
                row.vendor_po_item_id,
                parseFloat(row.total_received ?? '0') || 0
              );
            }
          }
      ...
      if (result.reopened) {
        await logAudit(
          id,
          'receipt_reopened_for_adjustment',
          user?.employeeId,
          actorName(user),
          {
      ...
      if (
        result.parentPoStatus === 'Fully Received' &&
        result.updated.vendorPoId
      ) {
        await logAudit(
          id,
          'po_auto_closed',
        ...
        result.parentPoStatus === 'Partially Received' &&
        result.updated.vendorPoId
      ) {
        await logAudit(
          id,
          'po_reopened_for_receipt_adjustment',
          user?.employeeId,
          actorName(user),
          {
            vendorPoId: result.updated.vendorPoId,
      ...
      if (err?.status === 422) {
        return res
          .status(422)
          .json({ error: err.message, blockers: err.blockers ?? [] });
      }
    ...
    try {
      ...
      if (!line)
        return res.status(404).json({
          error: 'Receipt line not found or does not belong to this receipt',
        });
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const ALWAYS_OPTIONAL_TRACE_FIELDS = new Set([
        'manufactureDate',
        'expirationDate',
        'batchNumber',
        'lotNumber',
      ...
      if (line.agPartNumber) {
        const [invItem] = await db
          .select({
            traceabilityFieldConfig: inventoryItems.traceabilityFieldConfig,
          })
          .from(inventoryItems)
        ...
        if (fieldConfig && Object.keys(fieldConfig).length > 0) {
          ...
          const missingRequired = TRACE_FIELDS.filter(
            (f) =>
              !ALWAYS_OPTIONAL_TRACE_FIELDS.has(f) &&
              (fieldConfig[f] ?? 'optional') === 'required' &&
              !req.body[f]?.toString().trim()
          );
          if (missingRequired.length > 0) {
            return res.status(422).json({
              error: `Required traceability fields missing: ${missingRequired.join(', ')}`,
            });
          }
      ...
      if (req.body.targetProjectId && req.body.targetRdProjectId) {
        return res.status(400).json({
          error:
            'Select either a production project or an R&D project, not both.',
        });
      }
      const targetProjectId =
        req.body.targetProjectId !== undefined
      ...
      if (body.disposition === 'accepted') {
        if (line.agPartNumber) {
          const missingDocs = await checkRequiredDocs(
            line.agPartNumber,
            receiptId
          );
          if (missingDocs.length > 0) {
            ...
            return res.status(422).json({
              error:
                'Unit placed on document hold because required documents are missing for this part.',
              missingDocuments: missingDocs,
              disposition: 'document_hold',
            });
          }
        ...
        try {
          ...
          await syncProjectReceivedMaterial(
            unit.id,
            user,
            'Material accepted in receiving and queued for PM review'
          );
        } catch (lotErr: any) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!unit)
        return res
          .status(404)
          .json({ error: 'Unit not found or does not belong to this receipt' });
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const finalTargetProjectId =
        'targetProjectId' in updates
          ? updates.targetProjectId
          : unit.targetProjectId;
      const finalTargetRdProjectId =
        'targetRdProjectId' in updates
          ? updates.targetRdProjectId
      ...
      const [updated] = await db.transaction(async (tx) => {
        ...
        if (unit.materialLotId) {
          const [lot] = await tx
            .select()
            .from(materialLots)
            .where(eq(materialLots.id, unit.materialLotId))
            .limit(1);
          if (lot) {
            const lotUpdates: Partial<typeof materialLots.$inferInsert> = {
              updatedAt: new Date(),
            };
            ...
            if ('quantity' in updates && updates.quantity != null) {
              ...
              if (nextRemaining < 0) {
                const error: any = new Error(
                  'Cannot reduce this accepted unit below material already issued or consumed.'
                );
                error.status = 422;
                throw error;
              }
            ...
            if ('uom' in updates && updates.uom != null)
              lotUpdates.unitOfMeasure = updates.uom;
            if ('lotNumber' in updates)
              lotUpdates.supplierLotNumber = updates.lotNumber ?? null;
            if ('manufactureDate' in updates)
              lotUpdates.manufactureDate = toTimestamp(updates.manufactureDate);
            if ('expirationDate' in updates)
              lotUpdates.expirationDate = toTimestamp(updates.expirationDate);
            if ('location' in updates)
              lotUpdates.storageLocation = updates.location ?? null;
            ...
            if (fabricRow) {
              const cuttingUpdates: Partial<
                typeof cuttingFabricInventory.$inferInsert
              > = { updatedAt: new Date() };
              if ('quantity' in updates && updates.quantity != null) {
              ...
              if ('lotNumber' in updates)
                cuttingUpdates.lotNumber = updates.lotNumber ?? null;
              if ('batchNumber' in updates || 'heatLot' in updates)
                cuttingUpdates.batchNumber =
                  updates.batchNumber ?? updates.heatLot ?? null;
              if ('rollNumber' in updates)
                cuttingUpdates.rollNumber = updates.rollNumber ?? null;
              if ('manufactureDate' in updates)
                cuttingUpdates.manufactureDate =
                  updates.manufactureDate ?? null;
              if ('expirationDate' in updates)
                cuttingUpdates.expirationDate = updates.expirationDate ?? null;
              if ('location' in updates)
                cuttingUpdates.location = updates.location ?? null;
              if ('freezerNumber' in updates)
                cuttingUpdates.freezerNumber = updates.freezerNumber ?? null;

              if (Object.keys(cuttingUpdates).length > 1) {
                await tx
                  .update(cuttingFabricInventory)
                  .set(cuttingUpdates)
                  .where(eq(cuttingFabricInventory.id, fabricRow.id));
              }
            }
          }
        }
          ...
          if (lotRow?.materialPartNumber) {
            ...
            const [existingLedger] = await tx
              ...
              .where(
                and(
                  eq(
                    inventoryTransactionLedger.sourceModule,
                    ledgerSourceModule
                  ),
                  eq(
                    inventoryTransactionLedger.sourceRecordId,
              ...
              await recordInventoryLedgerEntry(
                {
                  ...
                  metadata: {
                    ...
                    materialLotId: quantityAdjustment.materialLotId,
                    internalControlNumber:
                      quantityAdjustment.internalControlNumber,
                    quantityBefore: quantityAdjustment.before,
                    quantityAfter: quantityAdjustment.after,
                  },
                },
      ...
      if (Object.keys(auditableChanges).length > 0) {
        await logAudit(
          ...
          user?.employeeId,
          actorName(user),
          { unitId, changes: auditableChanges }
        );
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        return res.status(422).json({
          error: `Notes are required when setting disposition to "${disposition}"`,
        });
      }
      ...
      if (disposition === 'accepted') {
        ...
        if (expStatus === 'expired') {
          return res.status(422).json({
            error:
              'Cannot accept an expired unit. Update the expiration date or reject/quarantine this unit.',
        ...
        const [line] = await db
          .select()
          .from(receiptLines)
          .where(eq(receiptLines.id, unitCheck.receiptLineId));
        if (line?.agPartNumber) {
          const missingDocs = await checkRequiredDocs(
            line.agPartNumber,
            receiptId
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const user = req.user;
      const displayName = actorName(user);
      const { docType, notes, receivedUnitId } = req.body as {
        docType?: string;
        notes?: string;
      ...
      if (receivedUnitId) {
        const unitCheck = await assertUnitOwnership(
          receiptId,
          parseInt(receivedUnitId)
        );
        if (!unitCheck)
          return res
            .status(400)
            .json({ error: 'Unit does not belong to this receipt' });
      }
      ...
      if (req.file) {
        ...
        const receivingFolderId = await getOrCreateReceivingMediaFolder(
          user,
          displayName
        );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [doc] = await db
        ...
        .where(
          and(
            eq(receiptDocuments.id, docId),
            eq(receiptDocuments.receiptId, receiptId)
          )
        );
      if (!doc)
        return res.status(404).json({
          error: 'Document not found or does not belong to this receipt',
        });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [line] = await db
        .select()
        .from(receiptLines)
        .where(eq(receiptLines.id, unit.receiptLineId));
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (error instanceof P2ReceivingBarcodeError)
        return res
          .status(error.status)
      ...
      if (error instanceof z.ZodError)
        return res
          .status(400)
          .json({ error: 'INVALID_PRINT_EVIDENCE', details: error.flatten() });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const units = await db
        .select()
        .from(receivedUnits)
        .where(eq(receivedUnits.receiptId, receiptId));
      const lines = await db
        .select()
        .from(receiptLines)
      ...
      const labels = await Promise.all(
        units.map(async (unit) => {
          ...
          let barcodeImage: string | null = null;
          try {
            barcodeImage = await generateBarcodeImage(unit.barcode, {
              format: 'CODE128',
              width: 3,
              height: 12,
            });
...
async function checkRequiredDocs(
  ...
  const uploaded = new Set(
    docRows.map((d) => normalizeReceiptDocType(d.doc_type))
  );
  ...
  return requiredTypes
    .filter(
      (required) =>
        !required.itemLinked &&
        !required.receiptDocAliases.some((alias) => uploaded.has(alias))
    )
...
function receivedUnitExpirationStatus(
...
async function handleAcceptedUnit(
  unit: ReceivedUnit,
  receipt: Receipt,
  user: AuthUser
): Promise<void> {
  try {
    ...
    if (!line?.agPartNumber) {
      throw new Error(
        `Receipt line ${unit.receiptLineId} has no AG part number — cannot create material lot`
      );
    }
    ...
    if (!invRows.length) {
      ...
      if (!reRows.length) {
        throw new Error(
          `Failed to ensure inventory_items row for ag_part_number="${line.agPartNumber}"`
        );
      }
    ...
    if (!Number.isFinite(receivedQty) || receivedQty <= 0) {
      throw new Error(
        `Received unit ${unit.id} has invalid quantity "${unit.quantity}"`
      );
    }
    ...
    const prefilledMaxOutTime: number | null =
      invItem.default_max_out_time_minutes ?? null;
    if (invItem.shelf_life_controlled && !prefilledExpiration) {
      const days =
        invItem.frozen_shelf_life_days ?? invItem.room_temp_shelf_life_days;
      if (days != null && days > 0) {
        const base = unit.manufactureDate
    ...
    if (unit.materialLotId) {
      ...
      if (!existingLot) {
        throw new Error(
          `Received unit ${unit.id} references missing material lot ${unit.materialLotId}`
        );
      }
      lot = existingLot;
      ...
      const created = await db.transaction(async (tx) => {
        ...
        const lastSeq =
          icnRows.length > 0
            ? parseInt(
                icnRows[0].internal_control_number.split('-').pop() ?? '0',
                10
              )
            : 0;
    ...
    const [existingInventoryEvent] = await db
    ...
    if (isCuttingFabric) {
      ...
      const receivedDate = toDateOnly(
        receipt.receivedAt ?? receipt.receiptDate ?? new Date()
      );
      const [existingFabricInventory] = await db
        .select({ id: cuttingFabricInventory.id })
        .from(cuttingFabricInventory)
        .where(eq(cuttingFabricInventory.internalControlNumber, icn))
        .limit(1);

      if (!existingFabricInventory)
        await db.transaction(async (tx) => {
          const [fabricInventory] = await tx
            ...
            .values({
              ...
              receivedDate,
              expirationDate:
                toDateOnly(prefilledExpiration ?? unit.expirationDate) ?? null,
...
async function lineRequiresStrictSplit(
  line: ReceiptLine
): Promise<{ requires: boolean; fields: string[] }> {
  if (!line.agPartNumber) return { requires: false, fields: [] };
  const [invItem] = await db
    .select({ traceabilityFieldConfig: inventoryItems.traceabilityFieldConfig })
    .from(inventoryItems)
    .where(eq(inventoryItems.agPartNumber, line.agPartNumber))
    .limit(1);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      for (const line of lines) {
        ...
        const unitSequence = await getNextUnitSequence(receiptId);
        const barcode = generateUnitBarcode(
          receipt.receiptNumber,
          unitSequence
        );
        const targetProjectId = await resolveDefaultTargetProjectId(
          receipt,
          line
        );
        const custodyError = await assertP2ReceiptCustody(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!count || count < 2 || count > 200) {
        return res
          .status(400)
          .json({ error: 'count must be between 2 and 200' });
      }

      let normalizedRollNumbers: string[] | undefined;
      if (sqmPerRollArray !== undefined) {
        if (
          !Array.isArray(sqmPerRollArray) ||
          sqmPerRollArray.length !== count
        ) {
          return res.status(400).json({
            error: `sqmPerRollArray must have exactly ${count} entries`,
          });
        }
        if (
          sqmPerRollArray.some(
            (v) => typeof v !== 'number' || !Number.isFinite(v) || v <= 0
          )
        ) {
          return res.status(400).json({
            error:
              'Every entry in sqmPerRollArray must be a finite positive number',
          });
        }
        if (!Array.isArray(rollNumbers) || rollNumbers.length !== count) {
          return res
            .status(400)
            .json({ error: `rollNumbers must have exactly ${count} entries` });
        }
        if (
          rollNumbers.some(
          ...
          return res.status(400).json({
            error: 'Every roll number must be provided for roll-based splits',
          });
        }
        normalizedRollNumbers = rollNumbers.map((v) => v.trim());
      } else if (
        sqmPerRoll !== undefined &&
        (typeof sqmPerRoll !== 'number' ||
          !Number.isFinite(sqmPerRoll) ||
          sqmPerRoll <= 0)
      ) {
        return res
          .status(400)
          .json({ error: 'sqmPerRoll must be a positive number' });
      }
      ...
      const defaultTargetProjectId =
        templateFields &&
        Object.prototype.hasOwnProperty.call(templateFields, 'targetProjectId')
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (isRollArray) {
          ...
          const totalQty =
            parseFloat(String(line.receivedQty ?? '0')) ||
            parseFloat(String(line.orderedQty ?? '1'));
        ...
        const body = insertReceivedUnitSchema.parse({
          ...
          uom: line.uom ?? 'EA',
          ...(isRollArray && normalizedRollNumbers
            ? { rollNumber: normalizedRollNumbers[i] }
            : {}),
...
router.post(
  '/:id/units/batch-update',
  requireReceivingAccess,
  async (req: Request, res: Response) => {
    try {
      const receiptId = parseInt(req.params.id);
      const user = req.user;
      const { unitIds, updates } = req.body as {
        unitIds: number[];
      ...
      if (!Array.isArray(unitIds) || unitIds.length === 0) {
        return res
          .status(400)
          .json({ error: 'unitIds must be a non-empty array' });
      }
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const unitGenealogy = await Promise.all(
        units.map(async (unit) => {
          const expStatus = receivedUnitExpirationStatus(
          ...
          const travelerUsage =
            sqlRows<Record<string, unknown>>(travelerResult);
      ...
      res.json({
        ...
        summary: {
          ...
          quarantined: units.filter((u) => u.disposition === 'quarantine')
            .length,
          rejected: units.filter((u) => u.disposition === 'rejected').length,
          pendingInspection: units.filter(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [unit] = await db
        ...
        .where(eq(receivedUnits.id, unitId));
      if (!unit)
        return res.status(404).json({ error: 'Received unit not found' });

      const [receipt] = await db
        .select()
        .from(receipts)
        .where(eq(receipts.id, unit.receiptId));
      const [line] = await db
        .select()
        .from(receiptLines)
        .where(eq(receiptLines.id, unit.receiptLineId));
      ...
      const travelerUsage: Record<string, unknown>[] = [];
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        receiptId,
        missingByPartNumber: result,
        hasMissing: Object.keys(result).length > 0,
      });
    } catch (err: any) {
