...
-   **P1 PO Department Queue Visibility**: Updated `isOrderInDepartment()` utility function to properly handle production orders by checking for `productionStatus` field instead of `status` field. This ensures P1 PO items appear in all department queues (CNC, Finish, Finish QC, Paint, etc.) when filtered by department. Cache invalidation added to FinishQueuePage mutations to refresh current department view after order progression.
