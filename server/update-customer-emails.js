...
function normalizeCustomerName(name) {
  return name
    ...
    .replace(/[^\w\s&()-]/g, '')
    .replace(
      /\b(llc|inc|ltd|corporation|corp|company|co|custom|precision|firearms|gunworks|arms|tactical|outdoors)\b/gi,
      ''
    )
    .trim();
}
...
async function updateCustomerEmails() {
  try {
    ...
    const csvPath = path.join(
      process.cwd(),
      'attached_assets',
      'Customers export emails_1754623675320.csv'
    );
    ...
    console.log(
      `Parsed ${csvCustomers.length} valid customer records from CSV`
    );
    ...
    console.log(
      `Found ${customersWithoutEmails.length} customers without emails in database`
    );
    ...
    for (const dbCustomer of customersWithoutEmails) {
      ...
      const exactMatch = csvCustomers.find(
        (csvCustomer) => csvCustomer.normalizedName === normalizedDbName
      );
    ...
    for (const match of matches.slice(0, 10)) {
      ...
      console.log(
        `DB: "${match.dbName}" -> CSV: "${match.csvName}" (${match.email}) - Similarity: ${(match.similarity * 100).toFixed(1)}%`
      );
    }
    ...
    console.log(
      `- Customers still without emails: ${customersWithoutEmails.length - updatedCount}`
    );
