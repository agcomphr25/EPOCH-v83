...
export function inferStockModelFromFeatures(order: any): {
  stockModelId: string;
  product: string;
} {
  ...
  if (!stockModelId && order.features && typeof order.features === 'object') {
    ...
    if (features.action_inlet === 'mesa_precision_summit') {
      console.log(
        `🏔️ MESA PRECISION SUMMIT DETECTED (action_inlet): ${order.orderId || order.order_id} → Needs manual stock model selection`
      );
    }

    // Log special instructions for Mesa Precision Summit but don't auto-assign
    if (
      features.specialInstructions &&
      typeof features.specialInstructions === 'string'
    ) {
      const instructions = features.specialInstructions.toLowerCase();
      if (
        instructions.includes('mesa precision summit') ||
        instructions.includes('mesa_precision_summit')
      ) {
        console.log(
          `🏔️ MESA PRECISION SUMMIT DETECTED (specialInstructions): ${order.orderId || order.order_id} → Needs manual stock model selection`
        );
      }
    }
  }
  ...
  if (
    stockModelId &&
    stockModelId !== 'universal' &&
    stockModelId !== 'UNPROCESSED'
  ) {
  ...
  if (
    order.source === 'mesa_production_order' &&
    order.itemName?.toLowerCase().includes('mesa')
  ) {
    console.log(
      `🏔️ MESA PRODUCTION ORDER DETECTED: ${order.orderId || order.order_id} (itemName: ${order.itemName}) → Needs manual stock model selection`
    );
  }
  ...
  if (order.features && typeof order.features === 'object') {
    ...
    if (features.action_inlet || features.action) {
      ...
      if (action && typeof action === 'string') {
        ...
        if (
          actionLower.includes('terminus') ||
          actionLower.includes('defiance') ||
          actionLower.includes('impact') ||
          actionLower.includes('big_horn')
        ) {
        ...
        // Traditional Remington 700 actions often go with FG or wood stocks
          ...
          actionLower.includes('remington') ||
          actionLower.includes('rem')
        ) {
    ...
    if (
      ...
      if (barrel && typeof barrel === 'string') {
        ...
        if (
          barrelLower.includes('sendero') ||
          barrelLower.includes('heavy') ||
          barrelLower.includes('varmint') ||
          barrelLower.includes('carbon')
        ) {
        ...
        // Standard/sporter barrels
          ...
          barrelLower.includes('sporter') ||
          barrelLower.includes('standard')
        ) {
          stockModelId = 'fg_alpine_hunter';
          product = 'FG Alpine Hunter';
    ...
    if (
      ...
      if (
        ...
        product = 'CF Alpine Hunter';
        console.log(
          `🎯 FEATURE INFERENCE: Modern features detected → CF Alpine Hunter`
        );
      }
      // Paint options containing "carbon" suggest CF
      else if (
        features.paint_options &&
        features.paint_options.toLowerCase().includes('carbon')
      ) {
        stockModelId = 'cf_alpine_hunter';
        product = 'CF Alpine Hunter';
        console.log(
          `🎯 PAINT INFERENCE: ${features.paint_options} → CF Alpine Hunter`
        );
      }
      // REMOVED: No automatic defaults - let orders with unclear stock models be flagged
        ...
        console.log(
          `❓ UNCLEAR STOCK MODEL: ${order.orderId || order.order_id} → No clear inference possible`
        );
      }
    }
  }
  ...
  if (
    ...
    console.log(
      `🔧 FIXED: Setting stockModelId from product: "${product}" → "${stockModelId}"`
    );
  }
  ...
  if (!stockModelId || stockModelId === 'UNPROCESSED') {
    console.log(
      `❓ NEEDS INFORMATION: ${order.orderId || order.order_id} → No clear stock model identified`
    );
