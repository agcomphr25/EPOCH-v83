...
export interface IStorage {
  ...
  getBOMDetails(
    bomId: number
  ): Promise<(BomDefinition & { items: BomItem[] }) | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async generateNextOrderId(): Promise<string> {
    try {
      ...
      const fallbackId =
        currentPrefix + String(Date.now() % 10000).padStart(4, '0');
      console.warn(
        `All Order ID generation attempts failed, using fallback: ${fallbackId}`
      );
      return fallbackId;
    } catch (error) {
      ...
      const fallbackId =
        yearLetter + monthLetter + String(Date.now() % 1000).padStart(3, '0');
      return fallbackId;
    }
  }
  ...
  async createEvaluation(data: InsertEvaluation): Promise<Evaluation> {
    ...
    if (insertData.evaluationPeriodStart instanceof Date) {
      insertData.evaluationPeriodStart =
        insertData.evaluationPeriodStart.toISOString();
    }
    if (insertData.evaluationPeriodEnd instanceof Date) {
      insertData.evaluationPeriodEnd =
        insertData.evaluationPeriodEnd.toISOString();
    }
  ...
  async updateEvaluation(
    ...
    if (updateData.evaluationPeriodStart instanceof Date) {
      updateData.evaluationPeriodStart =
        updateData.evaluationPeriodStart.toISOString();
    }
    if (updateData.evaluationPeriodEnd instanceof Date) {
      updateData.evaluationPeriodEnd =
        updateData.evaluationPeriodEnd.toISOString();
    }
  ...
  async generateProductionOrders(poId: number): Promise<ProductionOrder[]> {
    ...
    if (existingOrders.length > 0) {
      console.log(
        `🏭 Found ${existingOrders.length} existing production orders for PO ${poId}`
      );
      console.log(
        `🏭 Returning existing production orders instead of creating duplicates`
      );
  ...
  // Layup Scheduler: Orders CRUD
  async getAllLayupOrders(filters?: {
    status?: string;
    department?: string;
  }): Promise<LayupOrder[]> {
    ...
    return await query.orderBy(
      layupOrders.priorityScore,
      layupOrders.orderDate
    );
  }
  ...
  > {
    try {
      ...
      const orders = await db
        ...
        .from(orderDrafts)
        .where(
          and(ne(orderDrafts.status, 'SCRAPPED'), isNull(orderDrafts.scrapDate))
        );
  ...
  private calculateScheduleStatus(
    ...
    if (cannotMeetDueDate) {
      console.log(
        `🟠 Order ${order.orderId}: CANNOT-MEET-DUE - Cannot meet due date (needs ${remainingProcessingDays} days, has ${daysUntilDue}) ${isDeptOverdue ? '[Also over dept time]' : ''}`
      );
