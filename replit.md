...
-   **Core Features**:
    ...
    -   **Training System**: Complete certification and quiz system for employee training modules. Features PDF viewer for training materials, 10-question quizzes with 80% passing score requirement, automatic certificate generation, and completion tracking. Includes comprehensive Preservation & FOD Training module with questions covering all 5 steps of both Preservation and FOD procedures from the company's training document. PDFs served via dedicated `/attached_assets` route configured to bypass Vite middleware for proper content-type handling.
