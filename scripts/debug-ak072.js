...
async function debugOrder() {
  try {
    ...
    const ak072Order = orders.find((order) => order.fbOrderNumber === 'AK072');
    ...
    if (!ak072Order) {
      ...
      if (similarOrders.length > 0) {
        ...
        similarOrders.forEach((order) => {
          console.log(
            `  - Order ID: ${order.orderId}, FB Order #: ${order.fbOrderNumber}, Current Dept: ${order.currentDepartment}`
          );
        });
      }
    ...
    try {
      const draftResponse = await fetch(
        `http://localhost:5000/api/orders/draft/${ak072Order.orderId}`
      );
      if (draftResponse.ok) {
    ...
    try {
      const finalizedResponse = await fetch(
        `http://localhost:5000/api/orders/finalized/${ak072Order.orderId}`
      );
    ...
    if (ak072Order.currentDepartment === 'Paint') {
      ...
      console.log(
        `❌ Order is still in ${ak072Order.currentDepartment} department`
      );
      console.log('💡 Possible reasons:');
      console.log(
        '  1. Order might be in draft status and needs to be finalized first'
      );
      console.log(
        '  2. Order might not have the exact FB Order Number "AK072"'
      );
