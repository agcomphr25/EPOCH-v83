...
router.post('/add-regular-orders', async (req, res) => {
  try {
    ...
    console.log(
      `📅 WEEK-SPECIFIC SCHEDULING: Selected week ${selectedWeekStart} to ${selectedWeekEnd}`
    );

    // Get unified P1 layup queue to find regular orders (excluding OEM orders)
    const fetch = (await import('node-fetch')).default;
    const p1QueueResponse = await fetch(
      'http://localhost:5000/api/p1-layup-queue'
    );
    ...
    const regularOrdersToSchedule = p1QueueData.filter((order: any) => {
      ...
      if (excludeOEMOrders) {
        ...
        if (isOemOrder) {
          oemFilteredCount++;
          console.log(
            `📋 FILTERED OUT OEM: ${order.orderId} (source: ${order.source})`
          );
          return false;
        }
      }
      ...
      if (!needsScheduling) {
        console.log(
          `📋 FILTERED OUT DEPT: ${order.orderId} (dept: ${order.currentDepartment})`
        );
        return false;
      }
    ...
    console.log(
      `   - Final regular orders to schedule: ${regularOrdersToSchedule.length}`
    );
    ...
    const findExactMatchingMolds = (stockModelId: string) => {
      ...
      return (activeMolds || []).filter((mold: any) => {
        ...
        return moldStockModels.some((moldModel: string) => {
          const normalizedMoldModel = moldModel
            .toLowerCase()
            .replace(/[\s\-]/g, '_');
    ...
    const existingAssignments = new Set(
      (existingSchedule || []).map((row: any) => {
        ...
        const normalizedMoldId = (row.mold_id || '')
          .toString()
          .trim()
          .toLowerCase();
    ...
    console.log(
      `📋 Found ${existingSchedule.length} existing schedule assignments to avoid collisions`
    );
    ...
    console.log(
      `📋 COLLISION FIX: Pre-populated currentAllocations with ${existingAssignments.size} existing assignments`
    );
    ...
    console.log(
      `👥 Found ${employees.length} active employees with production rates`
    );
    ...
    console.log(
      `📅 WEEK-SPECIFIC: Scheduling ONLY for week ${startDate.toDateString()} to ${endDate.toDateString()}`
    );
    ...
    while (currentDateIterator <= endDate) {
      if (workDays.includes(currentDateIterator.getDay())) {
        ...
        for (const emp of employees) {
          ...
          console.log(
            `📊 Employee ${emp.name} capacity on ${dateStr}: ${emp.production_rate} orders/day`
          );
        }
      }
    ...
    if (existingSchedule && existingSchedule.length > 0) {
      console.log(
        '📊 ADD-REGULAR: Reducing employee capacity based on existing database assignments...'
      );
        ...
        if (employeeCapacity.has(dateStr)) {
          ...
          for (const [empName, units] of Object.entries(assignments)) {
            ...
            const emp = employees.find((e) => {
              ...
              return (
                employeeName.includes(empNameCleaned) ||
                empNameCleaned.includes(employeeName) ||
                (empNameCleaned === 'jessica_pena' &&
                  employeeName.includes('jessica')) ||
                (empNameCleaned === 'theresa_flores' &&
                  employeeName.includes('theresa'))
              );
            });
            ...
            if (emp) {
              ...
              console.log(
                `📊 ADD-REGULAR: Reduced employee ${empId} (${emp.name}) capacity on ${dateStr} from ${capacity} to ${Math.max(0, capacity - reduction)} (existing: ${row.order_id})`
              );
          ...
          if (!reducedCapacity) {
            for (const [empId, capacity] of Array.from(dayCapacity.entries())) {
              if (capacity > 0) {
                ...
                console.log(
                  `📊 ADD-REGULAR: Reduced employee ${empId} capacity on ${dateStr} from ${capacity} to ${Math.max(0, capacity - 1)} (existing: ${row.order_id} - fallback)`
                );
                break;
              }
            }
      ...
      if (employeeCapacity.has(mondayStr)) {
        ...
        mondayCapacity.forEach((capacity, empId) => {
          ...
          console.log(
            `   Employee ${empId} (${emp?.name}): ${capacity} capacity remaining`
          );
        });
      }
    }
    ...
    for (const order of regularOrdersToSchedule) {
      // FIXED: Process all orders, no limit
      // Find compatible molds
      const compatibleMolds = findExactMatchingMolds(
        order.stockModelId || order.modelId || 'unknown'
      );
      ...
      while (!orderScheduled && attemptCount < maxAttempts) {
        ...
        while (
          ...
          if (currentDate > endDate) {
            console.log(
              `📅 WEEK-SPECIFIC: Reached end of selected week, stopping schedule for order ${order.orderId}`
            );
            break;
          }
          ...
          if (workDays.includes(currentDate.getDay())) {
            ...
            if (!employeeCapacity.has(dateStr)) {
              if (currentDate <= endDate) {
                ...
                for (const emp of employees) {
                  employeeCapacity
                    .get(dateStr)!
                    .set(emp.id, emp.production_rate);
                }
                console.log(
                  `📅 WEEK-SPECIFIC: Initialized capacity for ${dateStr} within selected week`
                );
              } else {
                // Beyond selected week - don't schedule here
                console.log(
                  `📅 WEEK-SPECIFIC: Skipping ${dateStr} - beyond selected week`
                );
                break;
              }
            }
            ...
            const availableEmployee = Array.from(dayCapacity.entries()).find(
              ([empId, capacity]) => capacity >= 1
            );

            if (availableEmployee) {
              foundSlot = true;
              console.log(
                `📋 Found available slot on ${dateStr} with employee ${availableEmployee[0]} (${availableEmployee[1]} capacity remaining)`
              );
            } else {
              // Check if ALL employees have zero capacity (deadlock protection)
              const totalCapacity = Array.from(dayCapacity.values()).reduce(
                (sum: number, cap: number) => sum + cap,
                0
              );
              if (totalCapacity === 0) {
                console.log(
                  `📋 ⚠️ All employees at zero capacity on ${dateStr}, skipping to next day`
                );
              }
        ...
        if (daysSearched >= maxDaysToSearch) {
          console.log(
            `📋 ⚠️ Could not find employee capacity within ${maxDaysToSearch} days for order ${order.orderId}`
          );
          break;
        }
        ...
        if (foundSlot) {
          ...
          for (const mold of compatibleMolds) {
            ...
            const normalizedMoldId = (mold.mold_id || '')
              .toString()
              .trim()
              .toLowerCase();
            ...
            const collisionCount =
              rows.length > 0 ? parseInt(rows[0]?.count) || 0 : 0;
            const poCount =
              rows.length > 0 ? parseInt(rows[0]?.po_count) || 0 : 0;
            const moldCapacity =
              rows.length > 0 ? parseInt(rows[0]?.capacity) || 1 : 1;
            ...
            const hasDbCollision =
              collisionCount > 0 &&
              (collisionCount >= moldCapacity || hasPOExclusivity);

            if (!hasDbCollision && !hasMemoryCollision) {
              selectedMold = mold;
              currentAllocations.add(moldDateKey);
              console.log(
                `📋 ✅ Found available mold: ${mold.mold_id} for ${order.orderId} on ${scheduledDateStr} (Capacity: ${moldCapacity}, Used: ${collisionCount}, PO: ${poCount}, PO Exclusive: ${hasPOExclusivity})`
              );
              break;
            } else {
              const reason = hasPOExclusivity
                ? 'PO exclusive access'
                : hasMemoryCollision
                  ? 'memory collision'
                  : `capacity full (${collisionCount}/${moldCapacity})`;
              console.log(
                `📋 ❌ Mold collision detected: ${mold.mold_id} blocked on ${scheduledDateStr} - ${reason}`
              );
            }
          }
          ...
          if (selectedMold) {
            ...
            const [employeeId, remainingCapacity] = Array.from(
              dayCapacity.entries()
            ).find(([empId, capacity]) => capacity >= 1) || [null, 0];
            ...
            if (employeeId && remainingCapacity >= 1) {
              ...
              orderScheduled = true;
              console.log(
                `📋 Scheduled regular order ${order.orderId} → ${selectedMold.mold_id} on ${currentDate.toDateString()} with employee ${employeeId}`
              );
            } else {
              console.log(
                `📋 ⚠️ No employee capacity available for ${order.orderId} on ${scheduledDateStr}`
              );
              foundSlot = false;
              currentDate.setDate(currentDate.getDate() + 1);
            }
            ...
            console.log(
              `📋 ⚠️ No available molds for order ${order.orderId} on ${scheduledDateStr} - trying next day (attempt ${attemptCount})`
            );
            foundSlot = false;
            currentDate.setDate(currentDate.getDate() + 1);
          }
      ...
      if (!orderScheduled) {
        console.log(
          `📋 ❌ Could not schedule order ${order.orderId} after ${maxAttempts} attempts`
        );
      }
    }
    ...
    try {
      console.log(
        `💾 Saving ${allocations.length} regular order assignments to layup_schedule table...`
      );
      ...
      for (const allocation of allocations) {
        ...
        const employeeAssignment = allocation.employeeId
          ...
          : {}; // Assign 1 unit of work to the employee
      ...
      console.log(
        `✅ Successfully saved ${allocations.length} regular order assignments to database`
      );
    } catch (saveError) {
      console.error(
        '⚠️ Error saving regular order schedule to database:',
        saveError
      );
    ...
    return res
      .status(500)
      .json({ error: 'Failed to add regular orders to schedule' });
  }
});
...
router.post('/oem-priority-only', async (req, res) => {
  try {
    ...
    console.log(
      `🟢 Scheduling OEM stock items from PO ${poId} (mode: ${selectionMode})`
    );
    ...
    const oemOrdersToSchedule = p1QueueData.filter((order: any) => {
      ...
      if (selectionMode === 'entire_po') {
        ...
        isSelectedItem =
          stockItemIds.includes(order.poItemId?.toString()) ||
          ...
          (order.orderId &&
            stockItemIds.some((id: string) =>
              order.orderId.includes(`-${id}-`)
            ));
      }

      console.log(
        `🟢 OEM Order ${order.orderId}: isOEM=${isOemOrder}, needsScheduling=${needsScheduling}, isSelected=${isSelectedItem}`
      );
      console.log(
        `🟢   → Selection Mode: ${selectionMode}, PO ID: ${poId}, Stock IDs: ${JSON.stringify(stockItemIds)}`
      );
      console.log(
        `🟢   → Order Props: poItemId=${order.poItemId}, stockItemId=${order.stockItemId}, id=${order.id}`
      );
    ...
    console.log(
      `🟢 Found ${oemOrdersToSchedule.length} OEM orders to schedule`
    );
    console.log('🟢 Debug - P1 queue data sample:', p1QueueData.slice(0, 3));
    console.log('🟢 Debug - Stock item IDs:', stockItemIds);
    console.log(
      '🟢 Debug - OEM orders to schedule:',
      oemOrdersToSchedule.map((o) => ({
        orderId: o.orderId,
        source: o.source,
        poItemId: o.poItemId,
    ...
    console.log(
      '🔧 DEBUG: activeMoldsResult keys:',
      activeMoldsResult ? Object.keys(activeMoldsResult) : 'null/undefined'
    );
    ...
    if (activeMoldsResult && (activeMoldsResult as any).rows) {
      ...
      console.error(
        '❌ Unexpected activeMoldsResult structure:',
        activeMoldsResult
      );
      activeMolds = [];
    }
    ...
    console.log(
      `🔧 Found ${activeMolds ? activeMolds.length : 0} active molds from database`
    );
    ...
    console.log(
      `🟢 OEM Priority scheduling starting on first work day: ${currentScheduleDate.toDateString()}`
    );
    ...
    const existingAssignments = new Set(
      existingScheduleRows.map((row: any) => {
        const dateStr =
          typeof row.scheduled_date === 'string'
            ? row.scheduled_date.split('T')[0]
            : row.scheduled_date.toISOString().split('T')[0];
    ...
    console.log(
      `🔍 Found ${existingScheduleRows.length} existing schedule assignments to avoid OEM collisions`
    );
    ...
    for (const order of oemOrdersToSchedule) {
      try {
        const matchingMolds = findExactMatchingMolds(
          order.modelId || order.stockModelId || ''
        );

        if (matchingMolds.length === 0) {
          console.warn(
            `⚠️ No matching molds found for order ${order.orderId} (model: ${order.modelId})`
          );
          continue;
        }
        ...
        console.log(
          `🔍 Checking collision for ${order.orderId} on ${scheduledDateStr}`
        );
        ...
        for (const mold of matchingMolds) {
          ...
          // FIXED: Real-time collision detection - check database for live conflicts
          console.log(
            `🔍 Collision check: mold=${mold.mold_id}, date=${scheduledDateStr}`
          );
          const liveCollisionCheck = await pool.query(
            `
            SELECT COUNT(*) as count 
          ...
          console.log(
            `🔍 Results: DB collision=${hasDbCollision}, Memory collision=${hasMemoryCollision}, Count=${collisionCount}, Rows=${rows.length}`
          );
          ...
          if (!hasDbCollision && !hasMemoryCollision) {
            ...
            console.log(
              `🟢 ✅ Found available mold for OEM: ${mold.mold_id} for ${order.orderId} on ${scheduledDateStr} (DB: ${hasDbCollision ? 'conflict' : 'free'}, Memory: ${hasMemoryCollision ? 'conflict' : 'free'})`
            );
            break;
          } else {
            console.log(
              `🟢 ❌ OEM Mold collision detected: ${mold.mold_id} already assigned on ${scheduledDateStr} (DB: ${hasDbCollision ? 'conflict' : 'free'}, Memory: ${hasMemoryCollision ? 'conflict' : 'free'})`
            );
          }
        }
        ...
        if (!selectedMold) {
          console.warn(
            `⚠️ No available molds for OEM order ${order.orderId} on ${scheduledDateStr} - trying next day`
          );
        ...
        console.log(
          `✅ Scheduled OEM order ${order.orderId} on ${orderScheduleDate.toDateString()}`
        );
        scheduledCount++;
        ...
        console.error(
          `❌ Failed to schedule OEM order ${order.orderId}:`,
          error
        );
      }
    }
    ...
    console.log(
      `🟢 OEM Priority scheduling complete: ${scheduledCount} orders scheduled`
    );
    ...
    res.json({
      ...
      debug: {
        ...
        productionOrders: p1QueueData.filter(
          (o) => o.source === 'production_order'
        ).length,
        stockItemIds,
...
router.post('/generate-algorithmic-schedule', async (req, res) => {
  try {
    ...
    console.log(
      `✅ Using work days from frontend settings: ${enforcedWorkDays.map((d: number) => ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][d]).join(', ')}`
    );
    console.log(
      `✅ Using daily capacity from frontend: ${maxOrdersPerDay} orders/day`
    );

    console.log(`🚀 Starting algorithmic scheduler over ${scheduleDays} days`);
    console.log(
      `📅 Work days ENFORCED: ${enforcedWorkDays.map((d: number) => ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][d]).join(', ')} (${enforcedWorkDays.join(', ')})`
    );
    ...
    console.log(
      `📋 Found ${p1QueueData.length} total orders in unified P1 production queue`
    );
    ...
    console.log(
      `👥 Found ${dbEmployees.length} layup employees with total capacity: ${totalDailyCapacity} parts/day`
    );
    dbEmployees.forEach((emp: any) => {
      const dailyCapacity = emp.rate * emp.hours;
      console.log(
        `  ${emp.employee_id}: ${emp.rate} parts/hr × ${emp.hours} hrs = ${dailyCapacity} parts/day`
      );
    });

    // Use capacity from frontend settings (calculated from UI employee settings)
    const actualDailyCapacity =
      maxOrdersPerDay || Math.floor(totalDailyCapacity) || 21;
    console.log(
      `🎯 Using frontend daily capacity: ${actualDailyCapacity} orders/day (passed from UI settings)`
    );
    ...
    const findExactMatchingMolds = (stockModelId: string) => {
      ...
      return activeMolds.filter((mold: any) => {
        ...
        if (
          normalizedStockModel.includes('mesa_universal') ||
        ...
        if (!hasMatch) {
          console.warn(
            `🚨 STRICT VALIDATION: No mold match found for stock model "${stockModelId}" (normalized: "${normalizedStockModel}"). Available molds for this model: ${moldStockModels.join(', ')}`
          );
        }
      ...
      return orders.sort((a, b) => {
        ...
        const calculatePriority = (order: any) => {
          const dueDate = new Date(
            order.dueDate || order.due_date || order.orderDate || '2099-12-31'
          );
          ...
          if (
            ...
            console.log(
              `🏭 P1 PO PRIORITY: Order ${order.orderId} gets +1000 priority (source: ${order.source})`
            );
          }
          ...
          if (
            ...
            console.log(
              `🏔️ MESA PRIORITY: Order ${order.orderId} gets +100 additional Mesa priority`
            );
          }

          // Due date urgency scoring (closer due dates = higher priority)
          if (daysDiff < 0)
            priority += 500; // Overdue orders
          else if (daysDiff <= 7)
            priority += 300; // Due within a week
          else if (daysDiff <= 14)
            priority += 200; // Due within 2 weeks
        ...
        const dueDateA = new Date(
          a.dueDate || a.due_date || a.orderDate || '2099-12-31'
        );
        const dueDateB = new Date(
          b.dueDate || b.due_date || b.orderDate || '2099-12-31'
        );
    ...
    console.log(
      `🎯 Sorted ${prioritizedOrders.length} orders by priority score and due date`
    );
    ...
    prioritizedOrders.slice(0, 10).forEach((order, index) => {
      ...
      console.log(
        `   ${index + 1}. ${order.orderId}: ${stockModelId}, Due ${dueDate.toDateString()}${priorityTags}`
      );
    });

    // Generate work dates based on configured work days
    const generateWorkDates = (
      startDate: Date,
      days: number,
      ...
      while (totalDays < days) {
        ...
        if (allowedWorkDays.includes(dayOfWeek)) {
          ...
          if (workDate.getDay() === 5 && !allowedWorkDays.includes(5)) {
            console.error(
              `❌ CRITICAL ERROR: Attempted to add Friday ${workDate.toDateString()} but Friday not in allowed work days: [${allowedWorkDays.join(', ')}]`
            );
            throw new Error(
              `Friday assignment prevented - not in configured work days`
            );
          }
          ...
          console.log(
            `✅ Added work date: ${workDate.toDateString()} (${dayName}, Day ${workDate.getDay()})`
          );
          totalDays++;
        }
      ...
      const allowedDayNames = allowedWorkDays.map(
        (d) => ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][d]
      );
      console.log(
        `📅 Generated ${workDates.length} work dates for: ${allowedDayNames.join(', ')}`
      );
      return workDates;
    };
    ...
    console.log(
      `📅 SCHEDULING WINDOW: Starting from ${startDate.toDateString()}, generating ${scheduleDays} work days`
    );
    const workDates = generateWorkDates(
      startDate,
      scheduleDays,
      enforcedWorkDays
    );
    ...
    prioritizedOrders.forEach((order: any) => {
      ...
      if (compatibleMolds.length === 0) {
        console.error(
          `🚨 CRITICAL VALIDATION FAILURE: Order ${order.orderId} with stock model "${stockModelId}" has NO compatible molds. SCHEDULING BLOCKED.`
        );
        invalidOrders.push({
          orderId: order.orderId,
    ...
    if (invalidOrders.length > 0) {
      console.error(
        `🚨 SCHEDULING BLOCKED: ${invalidOrders.length} orders have no compatible molds:`,
        invalidOrders
      );
      return res.status(400).json({
        ...
        message:
          'Under no circumstances will a stock model not match the mold. Fix mold configuration before scheduling.',
      });
    }

    console.log(
      '✅ STRICT VALIDATION PASSED: All orders have compatible molds'
    );
    ...
    for (const order of prioritizedOrders) {
      ...
      const materialPrefix = stockModelId.toLowerCase().startsWith('cf_')
        ? 'cf'
        : stockModelId.toLowerCase().startsWith('fg_')
          ? 'fg'
          : 'unknown';
        ...
        try {
          const features =
            typeof order.features === 'string'
              ...
              : order.features;
          heavyFill =
            features.heavyFill === true || features.heavyFill === 'true';
          lopAdjustment =
            features.lopAdjustment === true ||
            features.lopAdjustment === 'true';
        } catch (e) {
      ...
      console.log(
        `🎯 ORDER: ${order.orderId} → Stock: ${stockModelId} | Material: ${materialPrefix} |`
      );
      ...
      compatibleMolds.forEach((mold) => {
        console.log(
          `✅ EXACT MATCH: ${stockModelId} → ${mold.model_name} (stockModels: ${(mold.stock_models || []).join(', ')})`
        );
      });

      console.log(
        `🔍 EXACT MATCHING: ${stockModelId} → Found ${compatibleMolds.length} compatible molds`
      );
      console.log(
        `✅ Found ${compatibleMolds.length} compatible molds for ${stockModelId}: ${compatibleMolds.map((m) => m.model_name).join(', ')}`
      );
      ...
      for (const workDate of workDates) {
        ...
        if (!enforcedWorkDays.includes(dayOfWeek)) {
          console.log(
            `⚠️ SKIP: ${workDate.toDateString()} (${dayName}) not in allowed work days: [${enforcedWorkDays.join(', ')}]`
          );
          continue;
        }
        ...
        if (dayOfWeek === 5) {
          console.error(
            `🚨 FRIDAY PREVENTION: Attempted to schedule on Friday ${workDate.toDateString()} - BLOCKED!`
          );
          continue;
        }

        console.log(
          `🎯 ATTEMPTING: ${workDate.toDateString()} (${dayName}, Day ${dayOfWeek}) - Current count: ${currentDailyCount}/${actualDailyCapacity}`
        );

        // STRICT CAPACITY CHECK: Never exceed daily capacity
        if (currentDailyCount >= actualDailyCapacity) {
          console.log(
            `⏸️ CAPACITY FULL: ${dayName} already has ${currentDailyCount}/${actualDailyCapacity} orders - STRICT LIMIT ENFORCED`
          );
          continue;
        }
        ...
        if (currentDailyCount + 1 > actualDailyCapacity) {
          console.log(
            `⏸️ SAFETY CHECK: Adding this order would exceed capacity (${currentDailyCount + 1} > ${actualDailyCapacity})`
          );
          continue;
        }
        ...
        for (const mold of compatibleMolds) {
          ...
          if (currentUsage < moldCapacity) {
            ...
            const finalDailyCheck =
              (dailyAllocationCount.get(dailyKey) || 0) + 1;
            if (finalDailyCheck > actualDailyCapacity) {
              console.log(
                `🚫 FINAL CAPACITY CHECK FAILED: Would exceed daily limit (${finalDailyCheck} > ${actualDailyCapacity})`
              );
              continue;
            }
            ...
            if (!enforcedWorkDays.includes(scheduleDayOfWeek)) {
              console.error(
                `❌ CRITICAL: Attempted to schedule ${order.orderId} on ${scheduleDayName} ${scheduleDate.toDateString()}`
              );
              console.error(
                `   Allowed work days: [${enforcedWorkDays.map((d: number) => ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][d]).join(', ')}]`
              );
              throw new Error(
                `${scheduleDayName} assignment blocked - not in configured work days`
              );
            }
            ...
            console.log(
              `✅ Selected mold ${mold.model_name} for ${order.orderId} (${currentUsage + 1}/${mold.multiplier})`
            );
            scheduled = true;
            break;
          }
      ...
      if (!scheduled) {
        console.log(
          `❌ Could not allocate order ${order.orderId} - no mold capacity available in ${scheduleDays} work days (2 weeks limit)`
        );
      }
    }
    ...
    const successRate =
      totalProcessed > 0 ? (totalScheduled / totalProcessed) * 100 : 0;

    // CAPACITY VALIDATION: Verify we didn't exceed limits
    const dailyBreakdown = new Map<string, number>();
    allocations.forEach((allocation) => {
      const dateKey = new Date(allocation.scheduledDate)
        .toISOString()
        .split('T')[0];
    ...
    console.log(
      `👥 Employee daily capacity: ${actualDailyCapacity} orders/day (based on employee rates)`
    );
    ...
    dailyBreakdown.forEach((count, dateKey) => {
      ...
      console.log(
        `   ${date.toDateString()} (${dayName}): ${count}/${actualDailyCapacity} orders ${isOverCapacity ? '⚠️ OVER CAPACITY!' : '✅'}`
      );
    });

    if (totalCapacityViolations > 0) {
      console.error(
        `🚨 CAPACITY VIOLATIONS DETECTED: ${totalCapacityViolations} days exceed daily capacity of ${actualDailyCapacity} orders/day`
      );
    }

    const theoreticalMaxOrders = workDates.length * actualDailyCapacity;
    console.log(
      `🧮 CAPACITY MATH CHECK: ${workDates.length} work days × ${actualDailyCapacity} capacity = ${theoreticalMaxOrders} max possible orders`
    );

    if (totalScheduled > theoreticalMaxOrders) {
      console.error(
        `🚨 IMPOSSIBLE SCHEDULE DETECTED: Scheduled ${totalScheduled} orders but theoretical max is ${theoreticalMaxOrders}`
      );
    }
    ...
    if (unscheduledOrders.length > 0) {
      ...
      unscheduledOrders.slice(0, 10).forEach((order) => {
        console.log(
          `   - ${order.orderId}: ${order.stockModelId || order.modelId} (Due: ${new Date(order.dueDate || order.orderDate).toDateString()})`
        );
      });
      ...
      console.log(
        `   - Other capacity/timing issues: ${unscheduledOrders.length - noMoldsCount}`
      );
    }
    ...
    if (allocations.length > 0) {
      try {
        ...
        console.log(
          `🗑️ Clearing existing schedule from ${targetWeekStart.toISOString()} to ${targetWeekEnd.toISOString()}`
        );
        ...
        console.log(
          `📅 Saving ${allocations.length} algorithmic schedule entries to layup_schedule table`
        );
        ...
        for (const allocation of allocations) {
          ...
          const specificEmployeeAssignment = allocation.employeeId
            ? { [allocation.employeeId]: 1 }
            : {}; // Assign 1 unit of work to the specific employee
        ...
        console.log(
          `✅ Successfully saved algorithmic schedule to layup_schedule table`
        );
      } catch (saveError) {
        console.error(
          '⚠️ Error saving algorithmic schedule to database:',
          saveError
        );
    ...
    res.json({
      ...
      analytics: {
        ...
        materialBreakdown: {
          ...
          unknown: allocations.filter((a) => a.materialPrefix === 'unknown')
            .length,
        },
      },
    });
