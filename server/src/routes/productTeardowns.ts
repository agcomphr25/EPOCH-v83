...
router.get('/suggestions', asyncRoute(async (_req: any, res: any) => {
  const [names, assemblies, locations, parts] = await Promise.all([
  ...
  res.json({ names, assemblies, locations, partNumbers: parts });
}));
...
router.post('/:id/items', asyncRoute(async (req: any, res: any) => {
  ...
  const match = await inventoryMatch(body.itemName, body.enteredPartNumber);
  ...
  const rows = await pool.query(
    ...
    [req.params.id, body.itemName, body.enteredPartNumber || null, body.quantity, body.assemblyName || null,
      body.parentAssemblyName || null, body.physicalLocation || null, body.threadSize || null, body.length || null,
      body.headStyle || null, body.driveStyle || null, body.materialFinish || null, body.additionalDetails || null,
      body.notes || null, matched?.id ?? null, matched?.ag_part_number ?? null, match.state, match.state === 'found']
  );
...
router.get('/:id/bom-comparison', asyncRoute(async (req: any, res: any) => {
  ...
  const items = await pool.query(`SELECT id,item_name,inventory_part_number FROM product_teardown_items WHERE teardown_id=$1`, [req.params.id]);
