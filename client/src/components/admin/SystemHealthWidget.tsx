...
export default function SystemHealthWidget({ variant = 'default' }: Props) {
  ...
  if (isPremium) {
    return (
      <div className="depth-card flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="depth-card-icon bg-indigo-500/20">
              <ShieldCheck className="h-5 w-5 text-indigo-400" />
            </div>
            <span className="depth-card-title">Queue Integrity</span>
          </div>
          {data?.lastCheckTime ? (
            ...
            <span className="text-xs text-gray-600 italic">not yet run</span>
          )}
        </div>
          ...
          {isLoading || !data ? (
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-semibold text-green-400">No critical mismatches</p>
              </div>
              {data.warningCount > 0 && (
        ...
        <Link href="/admin/queue-integrity">
          <button className="quick-action-btn w-full flex items-center justify-center gap-1.5 text-xs mt-auto">
