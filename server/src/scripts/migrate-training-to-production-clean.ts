...
// Database URLs
const DEV_DATABASE_URL = 'postgresql://neondb_owner:npg_28YFPchwECLb@ep-sweet-smoke-adiyfj99.c-2.us-east-1.aws.neon.tech/neondb';
const PROD_DATABASE_URL = 'postgresql://neondb_owner:npg_8ybQvUYfuNm6@ep-wispy-sun-adm062ft.c-2.us-east-1.aws.neon.tech/neondb';

async function migrateTrainingData() {
  console.log('🚀 Starting clean training data migration from development to production...\n');
  ...
  try {
    ...
    // Reset sequences
    await prodDb.execute(sql`ALTER SEQUENCE training_modules_id_seq RESTART WITH 1`);
    await prodDb.execute(sql`ALTER SEQUENCE training_questions_id_seq RESTART WITH 1`);
    await prodDb.execute(sql`ALTER SEQUENCE training_question_options_id_seq RESTART WITH 1`);
