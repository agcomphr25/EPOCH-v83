...
async function checkUserMapping() {
  ...
  devUsers.forEach(user => {
    console.log(`Username: ${user.username.padEnd(20)} | Employee ID: ${String(user.employeeId || 'NULL').padEnd(8)} | Name: ${user.firstName} ${user.lastName}`);
  });
  ...
  for (const devUser of devUsers) {
    ...
    if (prodUser) {
      ...
      const status = canMap ? '✅' : '❌';
      console.log(`${status} ${devUser.username.padEnd(20)} | DEV EmpID: ${String(devUser.employeeId || 'NULL').padEnd(8)} | PROD EmpID: ${prodUser.employeeId || 'NULL'}`);
    } else {
