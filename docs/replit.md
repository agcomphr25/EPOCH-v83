...
- **February 17, 2026**: PRODUCTION FORECAST ENGINE (Level 1) - Read-only forecast system estimating department progression and ship dates for active orders. New tables: department_forecast_defaults (avg processing days per department), model_forecast_multiplier (per-model complexity multiplier). Engine in server/services/productionForecastEngine.ts with simulateOrderForecast, getExpectedDepartment, generateDashboardForecast functions. API routes: GET /api/forecast/order/:orderId, GET /api/forecast/dashboard. Frontend dashboard at /production-forecast with table view, status color coding (green=on track, yellow=early, red=late), filtering, and summary cards. Fully isolated - no modifications to existing production logic, layup scheduler, or order progression. Uses backlog-aware timing with business day calculations.
- **February 17, 2026**: CHECKLIST MANAGEMENT SYSTEM - Built admin interface for managing employee checklists. New database tables: checklist_templates, checklist_template_items, checklist_assignments, checklist_responses, checklist_response_items. Admin page at /admin/checklist-management with Templates, Assignments, and History tabs. Features: create checklist templates with configurable items (checkbox/text/number/dropdown), each item has its own frequency (daily/weekly/monthly), assign to specific employees, toggle required/optional per item, toggle clock-out enforcement per template. Updated DailyChecklistModal to use DB-driven items with fallback to legacy hard-coded department items. Updated TimeClock/TimeClockModal to use new enforcement-status endpoint. API routes at /api/checklist-management/* with server-side auth (admin-only for management, auth-required for employee-facing). Frequency is per-item, not per-template.
