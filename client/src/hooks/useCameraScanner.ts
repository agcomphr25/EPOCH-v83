...
export function useCameraScanner(
  ...
  useEffect(() => {
    ...
    if (!videoRef.current) {
      ...
      console.log(
        '📱 Mobile Camera Debug: Video element created with mobile-friendly attributes'
      );
    }
  ...
  const requestPermission = useCallback(async (): Promise<boolean> => {
    try {
      ...
      console.log(
        '📱 Mobile Camera Debug: Stream tracks:',
      ...
      stream.getTracks().forEach((track) => {
        console.log(
          '📱 Mobile Camera Debug: Stopping test track:',
          track.label
        );
        track.stop();
      });
      ...
      console.log(
        '📱 Mobile Camera Debug: Permission state updated successfully'
      );
      return true;
    } catch (error) {
  ...
  const startScanning = useCallback(async (): Promise<void> => {
    if (!readerRef.current || !videoRef.current) {
      ...
      setState((prev) => ({
        ...prev,
        error: 'Camera initialization failed - missing components',
      }));
      return;
    }
    ...
    try {
      ...
      console.log(
        '📱 Mobile Camera Debug: Requesting camera stream with constraints:',
        constraints
      );
      ...
      console.log(
        '📱 Mobile Camera Debug: Camera stream obtained successfully'
      );
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const onLoadedMetadata = () => {
          ...
          video
            ...
            .catch((playError) => {
              console.error(
                '📱 Mobile Camera Debug: Video play failed:',
                playError
              );
              reject(playError);
            });
      ...
      await readerRef.current.decodeFromVideoDevice(
        ...
        (result: Result | null, error?: Error) => {
          if (result) {
            ...
            console.log(
              '📱 Mobile Camera Debug: Barcode detected:',
              barcodeText
            );
          ...
          if (error && !(error.name === 'NotFoundException')) {
            console.error(
              '📱 Mobile Camera Debug: Barcode scanning error:',
              error
            );
          }
        }
      ...
      console.log(
        '📱 Mobile Camera Debug: Camera scanning started successfully'
      );
    } catch (error) {
      console.error(
        '📱 Mobile Camera Debug: Failed to start camera scanning:',
        error
      );
      console.error('📱 Mobile Camera Debug: Error details:', {
        name: error.name,
  ...
  return {
    ...
    requestPermission,
    getVideoRef,
  } as CameraScannerState &
    CameraScannerActions & {
      getVideoRef: () => React.MutableRefObject<HTMLVideoElement | null>;
    };
}
