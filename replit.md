...
- **Control Centers**: Unified interfaces for P2 Purchase Orders and Cutting Table with dashboards, wizards, and progress tracking. Includes P2 Control Center Shipping tab and Production Timer endpoints with badge authentication. P2 PO production orders auto-generate when serialized items are created (via p2LayupSchedules endpoint), with manual generation endpoint at `/api/p2/generate-production-orders/:poId`. Cutting Table material type derivation uses 3-tier fallback: 1) cutting_packet_bom_materials fabric_type, 2) item name keywords, 3) 2-level BOM child inventory name scan for carbon/fiberglass inference. P1 layup schedule JOIN uses customer name matching (orders.customer = customers.name). Fabric inventory ICN (Internal Control Number) auto-generates on creation (format: ICN-YYYYMMDD-seq-rand) if not provided; ICN is displayed in fabric inventory tables, FIFO suggestions, and fabric retrieval cards on the Cutting Operator Dashboard. P2 production orders sync to cutting table manufacturing queue via `POST /api/cutting-table-mfg-queue/sync-p2-demands` endpoint — groups p2_production_orders (department='Cutting Table') by PO+SKU, maps to inventory items, creates manufacturing_queue entries with packet BOM linking, and skips duplicates.
...
- **Executive Rundown System**: Glenn-only personal task management with daily rundown groups, priority tiers (CRITICAL/HIGH/NORMAL/LOW), carry-forward of overdue items, drag-reorder via @dnd-kit, weekly view, and brain dump rapid capture. Backend routes at `/api/executive/rundown` with `requireExecutiveAccess` middleware. Full page at `/executive`, header dropdown in Navigation.tsx. Schema: `executive_rundown_groups`, `executive_rundown_items` tables with `executive_priority` enum.
- **User Snapshot Resolver**: Reusable backend utility (`server/utils/userSnapshot.ts`) that resolves numeric user/employee IDs to display names via `resolveUserSnapshot()` (users table) and `resolveEmployeeSnapshot()` (employees table), with batch variants. Used by project routes to populate `performedByDisplayName` and `completedByDisplayName` text columns alongside legacy integer ID fields.
- **User Identity Layer**: Foundational identity abstraction (`server/identity/userIdentity.ts`) providing `createIdentitySnapshot()` for richer identity records (userId, displayName, firstName, lastName), `formatIdentity()` for centralized formatting (full/first/initials), and `buildIdentityInsertFields()` for preparing safe DB insert payloads. All new routes should use this layer instead of ad-hoc name resolution.
