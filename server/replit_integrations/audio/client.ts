...
export const AUDIO_INTEGRATION_UNAVAILABLE = "AUDIO_INTEGRATION_UNAVAILABLE";

export class AudioIntegrationUnavailableError extends Error {
  readonly code = AUDIO_INTEGRATION_UNAVAILABLE;
  readonly statusCode = 503;

  constructor() {
    super("Audio integration is unavailable");
    this.name = "AudioIntegrationUnavailableError";
  }
}

let openAIClient: OpenAI | undefined;

/** Lazily construct the optional audio client only when an audio operation runs. */
export function getOpenAIClient(): OpenAI {
  ...
  if (!apiKey || !baseURL) throw new AudioIntegrationUnavailableError();
