...
export default function DailyTagUpPage() {
  const [filters, setFilters] = useState(() => { const params = new URLSearchParams(window.location.search); const projectId = params.get('projectId')?.trim(); const customerPo = params.get('customerPo')?.trim(); return { projectId: projectId || 'all', customer: 'all', customerPo: customerPo || 'all', department: 'all', source: 'both', attentionDays: '14', status: 'all', search: '', problemsOnly: false }; });
