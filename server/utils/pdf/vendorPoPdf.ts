...
function drawPageFooter(state: DrawState) {
  const { page, fonts, footerText } = state;
  const { width } = page.getSize();
  page.drawLine({
    start: { x: PAGE.MARGIN, y: FOOTER_Y + 12 },
    end: { x: width - PAGE.MARGIN, y: FOOTER_Y + 12 },
    thickness: 0.5,
    color: COLOR.LIGHT_BORDER,
  });
  drawText(page, `${footerText} - Generated by EPOCH`, PAGE.MARGIN, FOOTER_Y, fonts.regular, FONT_SIZE.FOOTER, COLOR.MUTED_TEXT);
  drawRightText(page, formatDate(new Date()), width - PAGE.MARGIN, FOOTER_Y, fonts.regular, FONT_SIZE.FOOTER, COLOR.MUTED_TEXT);
}
