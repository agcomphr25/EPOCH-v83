...
router.get("/modules", async (req, res) => {
  try {
    const modules = await db.select().from(trainingModules).orderBy(desc(trainingModules.createdAt));
    res.json(modules);
  } catch (error: any) {
...
router.delete("/modules/:id", async (req, res) => {
  try {
    ...
    if (questionIds.length > 0) {
      await db.delete(trainingQuestionOptions).where(
        eq(trainingQuestionOptions.questionId, questionIds[0])
      );
      for (let i = 1; i < questionIds.length; i++) {
        await db.delete(trainingQuestionOptions).where(
          eq(trainingQuestionOptions.questionId, questionIds[i])
        );
      }
    }
...
router.patch("/questions/:id", async (req, res) => {
  try {
    ...
    const validatedData = insertTrainingQuestionSchema.partial().parse(req.body);
...
router.delete("/questions/:id", async (req, res) => {
  try {
    ...
    await db.delete(trainingQuestionOptions).where(eq(trainingQuestionOptions.questionId, questionId));
...
router.patch("/employee/records/:id", async (req, res) => {
  try {
    ...
    const validatedData = insertEmployeeTrainingRecordSchema.partial().parse(req.body);
