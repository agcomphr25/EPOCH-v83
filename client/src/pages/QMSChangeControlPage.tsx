...
export default function QMSChangeControlPage() {
  ...
  const [customerDecisionRequired, setCustomerDecisionRequired] =
    useState('ALL');
  ...
  const query = useMemo(() => {
    ...
    if (productionBlocked !== 'ALL')
      params.set('productionBlocked', productionBlocked);
    if (customerDecisionRequired !== 'ALL')
      params.set('customerDecisionRequired', customerDecisionRequired);
    if (nextActionCategory !== 'ALL')
      params.set('nextActionCategory', nextActionCategory);
  ...
  const load = async () => {
    ...
    try {
      const response = await fetch(
        `/api/change-control-dashboard${query ? `?${query}` : ''}`,
        {
          credentials: 'include',
        }
      );
      ...
      if (!response.ok)
        throw new Error(
          payload.message || payload.error || 'Unable to load Change Control'
        );
  ...
  const openDetails = async (id: string) => {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok)
      return setError(payload.message || 'Unable to load change details');
    setDetails(payload);
  };
  ...
  const importIndividual = async () => {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok)
      return setError(payload.message || 'Historical import failed');
  ...
  const previewBulk = async () => {
    ...
    const payload = await response.json().catch(() => []);
    if (!response.ok)
      return setError(payload.message || 'Import preview failed');
  ...
  const createNative = async () => {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok)
      return setError(payload.message || 'Unable to create native change');
    setNativeOpen(false);
    await load();
  };
  ...
  const createPcr = async () => {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok)
      return setError(payload.message || 'Unable to submit PCR');
    setPcrOpen(false);
    await load();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground">
            Quality control center referencing authoritative NCR, CAR, PCR, ECR,
            and ECN workflows. Recommendations support—not guarantee—AS9100
            process execution and require Quality confirmation.
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Register Filters</CardTitle>
        </CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-4 xl:grid-cols-6">
          <FilterSelect
            value={source}
            setValue={setSource}
            values={['IMPORTED_HISTORICAL', 'EPOCH_NATIVE']}
          ...
          <Input
            ...
            placeholder="Affected part/document"
          />
          <Input
            value={ownerUserId}
            onChange={(event) => setOwnerUserId(event.target.value)}
            placeholder="Owner / investigator user ID"
          />
          <FilterSelect
            value={severityRisk}
            setValue={setSeverityRisk}
            values={['LOW', 'NORMAL', 'MEDIUM', 'HIGH', 'CRITICAL']}
          ...
          <FilterSelect
            ...
            setValue={setOverdue}
            values={['true', 'false']}
            placeholder="Any due state"
          />
          <FilterSelect
            value={productionBlocked}
            setValue={setProductionBlocked}
            values={['true', 'false']}
            placeholder="Any production state"
          />
          <FilterSelect
            value={customerDecisionRequired}
            setValue={setCustomerDecisionRequired}
            values={['true', 'false']}
            placeholder="Any customer decision"
          />
          <FilterSelect
            value={nextActionCategory}
            setValue={setNextActionCategory}
            values={[
              'BLOCKING',
              'ADVISORY',
            ...
            <TableBody>
              {rows.map((row) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell>
                    {row.source === 'IMPORTED_HISTORICAL' ? (
                      <Badge variant="secondary">Historical / Imported</Badge>
                    ) : (
                      <Badge>Epoch Native</Badge>
                    )}
                  </TableCell>
                  <TableCell>{row.status}</TableCell>
                  <TableCell>{row.severity_risk || '—'}</TableCell>
                  <TableCell>
                    {row.product_safety_flag ? 'Impact' : '—'}
                  </TableCell>
                  <TableCell>
                    {row.customer_impact_flag || row.customer_decision_required
                      ? 'Review'
                      : '—'}
                  </TableCell>
                  <TableCell>
                    {row.production_blocked ? (
                  ...
                  <TableCell>
                    <div className="max-w-xs">
                      <Badge
                        variant={
                          row.next_action?.classification === 'BLOCKING'
                            ? 'destructive'
                            : 'outline'
                        }
                      >
                      ...
                      <div className="mt-1 text-xs text-muted-foreground">
                        {row.next_action?.statement}
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
                    {formatDueAge(
                      row.next_action?.dueDate || row.due_date,
                      row.updated_at,
                      row.status
                    )}
                  </TableCell>
              ...
              {!rows.length && (
                <TableRow>
                  ...
                  >
                    {loading
                      ? 'Loading…'
                      : 'No matching quality actions or changes'}
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
      ...
      <Dialog open={historicalOpen} onOpenChange={setHistoricalOpen}>
        <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Import Existing Historical Record</DialogTitle>
          </DialogHeader>
          <div className="rounded-md border bg-muted/30 p-3 text-sm">
            Transcribed names and dates are labeled historical approval evidence
            and never become EPOCH electronic signatures.
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            <Field
              label="Original change number"
              value={historical.originalRecordNumber}
              onChange={(value) =>
                setHistorical({ ...historical, originalRecordNumber: value })
              }
            />
            <TypeSelect
              value={historical.changeType}
              onChange={(value) =>
                setHistorical({ ...historical, changeType: value })
              }
            />
            <Field
              label="Title"
              ...
              onChange={(value) =>
                setHistorical({ ...historical, title: value })
              }
            />
            <Field
              label="Original source/system"
              value={historical.originalSystemOrSource}
              onChange={(value) =>
                setHistorical({ ...historical, originalSystemOrSource: value })
              }
            />
            <Field
              label="Original record date"
              type="date"
              value={historical.originalRecordDate}
              onChange={(value) =>
                setHistorical({ ...historical, originalRecordDate: value })
              }
            />
            <Field
              label="Original status"
              value={historical.originalStatus}
              onChange={(value) =>
                setHistorical({ ...historical, originalStatus: value })
              }
            />
            <Field
              label="Requested by"
              value={historical.requestedBy}
              onChange={(value) =>
                setHistorical({ ...historical, requestedBy: value })
              }
            />
            <Field
              label="Department"
              value={historical.department}
              onChange={(value) =>
                setHistorical({ ...historical, department: value })
              }
            />
          </div>
          ...
          <Textarea
            ...
            onChange={(event) =>
              setHistorical({ ...historical, description: event.target.value })
            }
          />
          ...
          />
          <Label>
            Reason evidence is unavailable (required only when no file is
            attached)
          </Label>
          <Textarea
            value={historical.evidenceUnavailableReason}
            onChange={(event) =>
              setHistorical({
                ...historical,
                evidenceUnavailableReason: event.target.value,
              })
            }
          />
          <DialogFooter>
            <Button onClick={() => void importIndividual()}>
      ...
      <Dialog open={bulkOpen} onOpenChange={setBulkOpen}>
        <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Bulk Register Import</DialogTitle>
          </DialogHeader>
          <a href="/api/change-control/import/template.csv">
          ...
          {!!preview.length && (
            <div className="space-y-2">
              <div className="text-sm">
                {preview.filter((row) => row.valid).length} valid ·{' '}
                {preview.filter((row) => !row.valid).length} rejected ·{' '}
                {preview.reduce((sum, row) => sum + row.warnings.length, 0)}{' '}
                warnings
              </div>
              {preview.map((row) => (
                <div
                  key={row.rowNumber}
                  className={`rounded border p-2 text-sm ${row.valid ? '' : 'border-destructive'}`}
                >
                  Row {row.rowNumber}:{' '}
          ...
          <DialogFooter>
            <Button
              ...
              disabled={!preview.length || preview.some((row) => !row.valid)}
            >
              Commit valid register transactionally
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={nativeOpen} onOpenChange={setNativeOpen}>
        <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Create New EPOCH Change</DialogTitle>
          </DialogHeader>
          <div className="rounded-md border bg-muted/30 p-3 text-sm">
            Creates an automatically numbered draft in the existing ECR
            workflow. Submission and approval use immutable revisions and
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <Field
              label="Priority"
              value={native.priority}
              onChange={(value) => setNative({ ...native, priority: value })}
            />
            <Field
              label="Title"
              value={native.title}
              onChange={(value) => setNative({ ...native, title: value })}
            />
            <Field
              label="Department"
              value={native.department}
              onChange={(value) => setNative({ ...native, department: value })}
            />
          </div>
          <Label>Description</Label>
          <Textarea
            ...
            onChange={(event) =>
              setNative({ ...native, description: event.target.value })
            }
          />
          <Label>Reason and business justification</Label>
          <Textarea
            value={native.reasonForChange}
            onChange={(event) =>
              setNative({ ...native, reasonForChange: event.target.value })
            }
          />
          <Label>Implementation plan</Label>
          <Textarea
            value={native.implementationPlan}
            onChange={(event) =>
              setNative({ ...native, implementationPlan: event.target.value })
            }
          />
          <DialogFooter>
            <Button
      ...
      <Dialog open={pcrOpen} onOpenChange={setPcrOpen}>
        <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Submit Process Change Request</DialogTitle>
          </DialogHeader>
          <div className="rounded-md border bg-muted/30 p-3 text-sm">
            Submission starts QMS screening. It does not authorize
            implementation or departure from released requirements.
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            <Field
              label="Change type"
              value={pcr.changeType}
              onChange={(value) => setPcr({ ...pcr, changeType: value })}
            />
            <Field
              label="Scope"
              value={pcr.scope}
              onChange={(value) => setPcr({ ...pcr, scope: value })}
            />
            <Field
              label="Affected part number"
              value={pcr.partNumber}
              onChange={(value) => setPcr({ ...pcr, partNumber: value })}
            />
            <Field
              label="Related record ID (order, traveler, operation, equipment, material, or document)"
              value={pcr.contextId}
              onChange={(value) => setPcr({ ...pcr, contextId: value })}
            />
            <div className="space-y-2">
              <Label>Related context type</Label>
              <Select
                value={pcr.contextType}
                onValueChange={(value) =>
                  setPcr({ ...pcr, contextType: value })
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div className="space-y-2">
              ...
              <Select
                value={pcr.requiresCustomerApproval ? 'YES' : 'NO'}
                onValueChange={(value) =>
                  setPcr({ ...pcr, requiresCustomerApproval: value === 'YES' })
                }
              >
                <SelectTrigger>
                  <SelectValue />
          ...
          <Textarea
            value={pcr.proposedChange}
            onChange={(event) =>
              setPcr({ ...pcr, proposedChange: event.target.value })
            }
          />
          <Label>Reason and business justification</Label>
          <Textarea
            value={pcr.reason}
            onChange={(event) => setPcr({ ...pcr, reason: event.target.value })}
          />
          <Label>Initial risk information</Label>
          <Textarea
            value={pcr.riskAssessment}
            onChange={(event) =>
              setPcr({ ...pcr, riskAssessment: event.target.value })
            }
          />
          <DialogFooter>
            <Button onClick={() => void createPcr()}>
        ...
        <DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto">
          {details && (
            <>
              <DialogHeader>
                <DialogTitle>
                  {details.change_number} — {details.title}
                </DialogTitle>
              </DialogHeader>
              {details.source === 'IMPORTED_HISTORICAL' && (
                <Badge variant="secondary" className="w-fit">
                  Historical / Imported — not originally controlled by EPOCH
                </Badge>
              )}
              {details.source === 'EPOCH_NATIVE' && (
                <a href="/qms/design-control" className="w-fit">
                  <Button variant="outline">
                    Open controlled ECR / ECN workflow
                  </Button>
                </a>
              )}
              <div className="grid gap-4 md:grid-cols-2">
                <DetailSection
                  title="Summary"
                  value={`${details.change_type} · ${details.status}\n${details.description || ''}`}
                />
                <DetailSection
                  title="Impact / Risk"
                  value={
                    [
                      ...
                      details.product_safety_impact,
                      details.regulatory_impact,
                      details.configuration_impact,
                    ]
                      .filter(Boolean)
                ...
                <DetailSection
                  ...
                  value={
                    (details.links || [])
                      .map(
                        (link: any) =>
                          `${link.link_type}: ${link.linked_record_number || link.linked_record_id}`
                      )
                      .join('\n') || 'None linked'
                  }
                />
                <DetailSection
                  ...
                  value={
                    details.source === 'IMPORTED_HISTORICAL'
                      ? `${(details.historicalApprovals || []).length} historical approval evidence entries. These are not EPOCH electronic approvals.`
                      : 'Native approvals remain bound to the ECR/ECN revision and checksum.'
                  }
                />
                <DetailSection
                ...
                <DetailSection
                  ...
                  value={
                    (details.evidence || [])
                      .map(
                        (item: any) =>
                          `${item.original_filename} · SHA-256 ${item.sha256_checksum}`
                      )
                ...
                <DetailSection
                  ...
                  value={
                    (details.links || [])
                      .filter(
                        (link: any) => link.link_type === 'RELATED_CHANGE'
                      )
                      .map(
                        (link: any) =>
                ...
                <DetailSection
                  ...
                  value={
                    details.next_action
                      ? `${details.next_action.classification}: ${details.next_action.statement}\nResponsible: ${details.next_action.responsibleRole}\nEvidence: ${(details.next_action.evidence || []).join('; ')}\nControl: ${details.next_action.controlReference || 'Not configured'}`
                      : 'Assessment required'
                  }
                />
                <DetailSection
                  title="AS9100 Workflow Assessment"
                  value={
                    (details.assessments || [])
                      .map(
                        (assessment: any) =>
                          `Version ${assessment.version} · ${assessment.lifecycle_status} · ${assessment.unresolved_recommendations} unresolved recommendations`
                      )
                      .join('\n') || 'No versioned assessment submitted'
                  }
                />
              </div>
              <QualityActionWorkspace
                details={details}
                can={can}
