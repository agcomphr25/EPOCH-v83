    ...
    try {
      ...
      if (!navigator.mediaDevices) {
        addResult(
          'Media Devices API',
          'fail',
          'navigator.mediaDevices not available - HTTPS required'
        );
        return;
      }
      ...
      try {
        ...
        addResult(
          'Camera Detection',
          videoDevices.length > 0 ? 'pass' : 'fail',
      ...
      try {
        ...
        addResult(
          'Camera Stream',
          'pass',
          `Stream active with ${tracks.length} track(s)`,
          tracks.map((t) => ({
            kind: t.kind,
            label: t.label,
            enabled: t.enabled,
      ...
      if (isMobile) {
        try {
          ...
          addResult(
            ...
            'pass',
            'Mobile-optimized constraints work'
          );
          ...
          addResult(
            'Mobile Camera Constraints',
            'warning',
            `Mobile constraints failed: ${error.message}`
          );
          ...
          try {
            ...
            addResult(
              'Fallback Camera',
              'fail',
              `All camera access failed: ${fallbackError.message}`
            );
          }
        }
      }
      ...
      try {
        ...
        addResult(
          'Video Element',
          'fail',
          `Video element creation failed: ${error.message}`
        );
      }
      ...
      addResult(
        ...
        window.isSecureContext
          ...
          : 'Insecure context - camera may not work'
      );
    } catch (error) {
      addResult(
        'Diagnostic Error',
