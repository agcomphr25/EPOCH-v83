...
export function BulkShippingActions({
  ...
  const firstOrderCustomerId =
    selectedOrders.length > 0
      ? shippingOrders.find((order) => order.orderId === selectedOrders[0])
          ?.customerId
      : null;
  ...
  const { data: customerAddresses = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `/api/customers/${firstOrderCustomerId}/addresses`
      );
  ...
  useEffect(() => {
    if (dialogOpen && customerAddresses.length > 0 && firstOrderCustomerId) {
      ...
      const customerInfo = customersList.find(
        (c: any) => c.id.toString() === firstOrderCustomerId.toString()
      );
  ...
  const sendCustomerNotification = async (
    ...
    try {
      ...
      const response = await axios.post(
        '/api/communications/send-notification',
        notificationData
      );
  ...
  const handleBulkShipping = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        '/api/shipping-pdf/bulk-shipping-labels',
        {
          ...
          <div className="flex gap-2">
            <Dialog open={dialogOpen} onOpenChange={handleDialogClose}>
              ...
              <DialogContent className="max-w-lg">
                <DialogHeader>
                  <DialogTitle>
                    Bulk Shipping for {selectedOrders.length} Orders
                  </DialogTitle>
                </DialogHeader>
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2 pt-4">
                    ...
                    >
                      {isProcessing
                        ? 'Processing...'
                        : 'Ship & Notify Customers'}
                    </Button>
                  </div>

                  <div className="text-xs text-gray-500 bg-blue-50 p-2 rounded">
                    <strong>Note:</strong> This will generate a single shipping
                    label for all selected orders and automatically send
                    tracking information to each customer via their preferred
                    communication method.
                  </div>
                </div>
              </DialogContent>
