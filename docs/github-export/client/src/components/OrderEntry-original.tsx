...
export default function OrderEntry() {
  ...
  const [payments, setPayments] = useState<
    { type: string; date: Date; amount: number }[]
  >([]);
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
            ...
            if (draftResponse.customerId) {
              try {
                const customerResponse = await apiRequest(
                  `/api/customers/${draftResponse.customerId}`
                );
        ...
        setModelOptions(
          stockModelsResponse.filter((model: any) => model.isActive)
        );

        // Load features from API (group paint options under one feature)
        const activeFeatures = featuresResponse.filter(
          (feature: any) => feature.isActive
        );
        ...
        const paintFeatures = activeFeatures.filter(
          (feature: any) => feature.category === 'paint_options'
        );
        const nonPaintFeatures = activeFeatures.filter(
          (feature: any) => feature.category !== 'paint_options'
        );
        ...
        const finalFeatures =
          paintFeatures.length > 0
            ? [...nonPaintFeatures, paintOptionsFeature]
            : nonPaintFeatures;

        setFeatureDefs(
          finalFeatures.map((feature: any) => ({
            id: feature.id,
  ...
  const calculateTotals = () => {
    ...
    if (paintOptionsValue) {
      ...
      if (paintFeature && paintFeature.subCategory) {
        ...
        const subCategory = subCategories.find(
          (sc) => sc.id === paintFeature.subCategory
        );
    ...
    Object.entries(features).forEach(([featureId, featureValue]) => {
      ...
      if (featureDefinition) {
        ...
        if (
          featureDefinition.type === 'dropdown' &&
          featureDefinition.options
        ) {
          const selectedOption = featureDefinition.options.find(
            (opt) => opt.value === featureValue
          );
        ...
        if (
          featureDefinition.type === 'checkbox' &&
  ...
  const { basePrice, featureCost, subtotal, total, discountAmount } =
    calculateTotals();
  ...
  const getFeaturePrice = (featureId: string, value: any) => {
    ...
    if (featureId === 'paint_options_combined') {
      ...
      const paintFeature = paintFeatures.find(
        (f) => f.id === originalFeatureId
      );
      if (paintFeature && paintFeature.subCategory) {
        const subCategory = subCategories.find(
          (sc) => sc.id === paintFeature.subCategory
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input value={orderId} readOnly className="bg-gray-50" />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div key={featureDef.id} className="space-y-2">
                          ...
                          <PopoverTrigger asChild>
                            ...
                            >
                              {features[featureDef.id]
                                ? featureDef.options?.find(
                                    (opt) =>
                                      opt.value === features[featureDef.id]
                                  )?.label || features[featureDef.id]
                                : 'Select or search...'}
                              <ChevronDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                            </Button>
                          </PopoverTrigger>
                      ...
                      {featureDef.type === 'textarea' && (
                        <textarea
                          ...
                          placeholder={
                            featureDef.placeholder || 'Enter details...'
                          }
                          value={features[featureDef.id] || ''}
                          onChange={(e) =>
                            setFeatures((prev) => ({
                      ...
                      {featureDef.type === 'text' && (
                        <Input
                          type="text"
                          placeholder={
                            featureDef.placeholder || 'Enter text...'
                          }
                          value={features[featureDef.id] || ''}
                          onChange={(e) =>
                            setFeatures((prev) => ({
                      ...
                      {featureDef.type === 'number' && (
                        <Input
                          type="number"
                          placeholder={
                            featureDef.placeholder || 'Enter number...'
                          }
                          value={features[featureDef.id] || ''}
                          onChange={(e) =>
                            setFeatures((prev) => ({
                      ...
                      {featureDef.type === 'checkbox' && (
                        <div className="space-y-2">
                          {featureDef.options?.map((option) => {
                            ...
                            const quantity =
                              featureQuantities[featureDef.id]?.[
                                option.value
                              ] || 1;
                              ...
                              <div key={option.value} className="space-y-2">
                                      ...
                                      if (e.target.checked) {
                                        ...
                                        setFeatureQuantities((prev) => {
                                          ...
                                          if (newQuantities[featureDef.id]) {
                                            delete newQuantities[featureDef.id][
                                              option.value
                                            ];
                                          }
                                ...
                                {isChecked && (
                                  <div className="ml-6 flex items-center space-x-2">
                                    <Label
                                      htmlFor={`${featureDef.id}-${option.value}-qty`}
                                ...
                                (selectedValue: string) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <div className="flex items-center space-x-2">
                                        <Label
                                          htmlFor={`${featureDef.id}-${selectedValue}-qty`}
                                          className="text-xs text-gray-600"
                                        >
                                          Qty:
                                        </Label>
                  ...
                  {process.env.NODE_ENV === 'development' && (
                    <div className="text-xs text-gray-500 bg-gray-100 p-2 rounded">
                      ...
                      {features.action === 'bartlein_#3b' &&
                        ' (Shank Length should be visible)'}
                    </div>
                  )}
                  ...
                  <div className="flex justify-between items-start text-sm">
                    ...
                    <div className="w-16 text-right ml-2">
                      <span className="font-medium text-blue-600">
                        {modelId
                          ? `$${(modelOptions.find((m) => m.id === modelId)?.price || 0).toFixed(2)}`
                          : '$0.00'}
                      </span>
                    </div>
                  </div>
                  ...
                  {featureDefs.map((feature) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 text-right">
                          <span
                            className={`${hasValue ? 'font-medium' : 'text-gray-400'}`}
                          >
                            {displayValue}
                          </span>
                        </div>
                  ...
                  <SelectContent>
                    ...
                    {persistentDiscounts.map((discount: any) => {
                      const customerType = customerTypes.find(
                        (ct: any) => ct.id === discount.customerTypeId
                      );
