...
router.delete('/:moldId', async (req: Request, res: Response) => {
  try {
    ...
    if (
      error.code === '23503' &&
      error.constraint === 'layup_schedule_mold_id_fkey'
    ) {
      res.status(400).json({
        error:
          'Cannot delete mold - it is currently used in the layup schedule. Please clear the schedule first or disable the mold instead.',
...
router.get('/:moldId/references', async (req: Request, res: Response) => {
  try {
    ...
    const references = schedule.filter(
      (entry: any) => entry.moldId === req.params.moldId
    );
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        success: true,
        message: 'Cleared all schedule references for this mold',
      });
    } catch (error) {
