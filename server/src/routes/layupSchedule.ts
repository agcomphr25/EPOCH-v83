...
router.get('/debug-molds', async (req: Request, res: Response) => {
  try {
    const result =
      await rawSql`SELECT mold_id, stock_models FROM molds WHERE mold_id = 'Mesa Universal-1' LIMIT 1`;
    const drizzleResult = await db
      .select({ moldId: molds.moldId, stockModels: molds.stockModels })
      .from(molds)
      .limit(3);
...
router.patch('/molds/bulk/by-model', async (req: Request, res: Response) => {
  try {
    ...
    if (!modelName) {
      return res
        .status(400)
        .json({ success: false, error: 'modelName is required' });
    }
    ...
    console.log(
      `✅ Bulk updated ${result.length} molds for model ${modelName}:`,
      updateData
    );
...
router.patch('/molds/:id', async (req: Request, res: Response) => {
  try {
    ...
    console.log(`🔧 Updating mold id=${id} with:`, {
      enabled,
      multiplier,
      isActive,
      stockModels: newStockModels,
    ...
    try {
      ...
      if (!Array.isArray(newStockModels)) {
        return res.status(400).json({
          success: false,
          error: 'stockModels must be an array of strings',
        });
      }
      ...
      if (result.length === 0) {
        return res
          .status(404)
          .json({ success: false, error: 'Mold not found' });
      }
...
router.post(
  '/molds/bulk-update-stock-models',
  async (req: Request, res: Response) => {
    try {
      ...
      if (!modelName || !Array.isArray(newStockModels)) {
        return res.status(400).json({
          success: false,
          error: 'modelName and stockModels array required',
        });
      }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    `;
      console.log(
        `🔍 Found ${checkResult.length} molds with model_name='${modelName}':`,
        checkResult.slice(0, 3)
      );
      ...
      if (checkResult.length === 0) {
        return res.json({
          ...
          updatedCount: 0,
          molds: [],
          message: 'No molds found with that model_name',
        });
      }
    ...
    `;
      ...
      res.json({
        success: true,
        updatedCount: checkResult.length,
        molds: verifyResult,
      });
    } catch (error: any) {
...
router.post(
  '/molds/auto-populate-stock-models',
  async (req: Request, res: Response) => {
    try {
      ...
      for (const [modelName, moldList] of moldsByModel) {
        ...
        if (matchingStockModels.length === 0) {
          ...
          for (const sm of stockModelsList) {
            ...
            if (
              smKey.includes(normalizedModelName) ||
              normalizedModelName.includes(smKey) ||
        ...
        if (matchingStockModels.length > 0) {
          ...
          updates.push({
            modelName,
            stockModels: matchingStockModels,
            count: moldList.length,
          });
        }
      }
...
router.post('/generate', async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `📊 Total items to schedule: ${totalItems} (${selectedOrderIds.length} regular orders, ${selectedPOItems.length} PO items)`
    );
    console.log(
      '📦 Selected PO Items received:',
      JSON.stringify(selectedPOItems.slice(0, 5), null, 2)
    );
    console.log(
      '📦 PO Items total quantity:',
      selectedPOItems.reduce((sum, item) => sum + item.quantity, 0)
    );
    ...
    const stockModelsList = await db
      .select({
    ...
    console.log(
      `📦 Loaded ${stockModelDisplayMap.size} stock models for material detection`
    );
    ...
    if (rawMolds.length > 0) {
      console.log(
        '🔍 RAW MOLD DATA (first 3):',
        rawMolds.slice(0, 3).map((m: any) => ({
          mold_id: m.mold_id,
    ...
    const activeMolds = rawMolds.map((m: any) => ({
      ...
      stockModels:
        typeof m.stock_models === 'string'
          ? m.stock_models
              .replace(/^\{|\}$/g, '')
              .split(',')
              .filter((s: string) => s.length > 0)
          : Array.isArray(m.stock_models)
            ? m.stock_models
            : [],
    ...
    if (activeMolds.length > 0) {
      console.log(
        '🔍 Sample molds:',
        activeMolds.slice(0, 3).map((m: any) => ({
          moldId: m.moldId,
    ...
    if (selectedOrderIds.length > 0) {
      console.log(
        `🔍 Fetching ${selectedOrderIds.length} regular orders:`,
        selectedOrderIds.slice(0, 5)
      );
      ...
      console.log(
        `📦 Found ${ordersResults.length} regular orders in database`
      );
      if (ordersResults.length > 0) {
        console.log('🔍 Sample order:', ordersResults[0]);
        console.log(
          '🔍 Regular order stock models:',
          ordersResults
            .slice(0, 5)
            .map((o) => ({ orderId: o.orderId, stockModel: o.stockModel }))
        );
      }
      ...
      regularOrders = ordersResults.map((order) => {
        ...
        const otherOptions = Array.isArray(features.other_options)
        ...
        let material = null;
        const displayName =
          stockModelDisplayMap.get(order.stockModel || '') || '';
        if (
          displayName.startsWith('CF ') ||
          displayName.includes(' CF ') ||
          displayName.toLowerCase().includes('carbon')
        ) {
          material = 'Carbon Fiber';
        } else if (
          displayName.startsWith('FG ') ||
          displayName.includes(' FG ') ||
          displayName.toLowerCase().includes('fiberglass')
        ) {
          material = 'Fiberglass';
        }
    ...
    if (selectedPOItems.length > 0) {
      console.log(
        `🔍 Preparing ${selectedPOItems.length} PO items for scheduling`
      );
      ...
      console.log(
        `📦 Fetched ${poProductsData.length} PO products with action length data`
      );
    ...
    console.log(
      `📦 Prepared ${allItems.length} items for scheduling (${regularOrders.length} regular + ${poItems.length} PO units)`
    );
    ...
    if (weekStart) {
      ...
      console.log(
        `📅 Using provided week start: ${format(weekStartDate, 'yyyy-MM-dd')}`
      );
    } else {
      const today = new Date();
      weekStartDate = startOfWeek(addDays(today, 7), { weekStartsOn: 1 });
      console.log(
        `📅 Using calculated next Monday: ${format(weekStartDate, 'yyyy-MM-dd')}`
      );
    }
    ...
    for (const item of allItems) {
      ...
      const compatibleMolds = activeMolds.filter((mold) => {
        ...
        if (mold.stockModels && mold.stockModels.length > 0) {
          ...
          if (Array.isArray(mold.stockModels)) {
            ...
            return (
              mold.stockModels.includes(item.stockModel) ||
              mold.stockModels.some(
                (m) => getCanonicalName(m) === canonicalStockModel
              )
            );
          }
          ...
          if (typeof mold.stockModels === 'string') {
            ...
            return (
              cleanedModels.includes(item.stockModel) ||
              cleanedModels.some(
                (m) => getCanonicalName(m) === canonicalStockModel
              )
            );
        ...
        return (
          normalizedMoldModel === normalizedStockModel ||
      ...
      if (compatibleMolds.length === 0) {
        console.log(
          `⚠️ No compatible molds for ${item.orderId} (stockModel: ${item.stockModel}, type: ${typeof item.stockModel})`
        );
        overflowItems.push({
          ...item,
      ...
      for (const day of rotatedDays) {
        ...
        for (const mold of compatibleMolds) {
          ...
          if (currentUsage < mold.multiplier) {
            ...
            scheduled = true;
            console.log(
              `✅ Scheduled ${item.orderId} → ${mold.moldId} on ${['', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri'][day]}`
            );
      ...
      if (!scheduled) {
        console.log(
          `❌ Cannot schedule ${item.orderId} - no capacity available`
        );
        overflowItems.push({
          ...item,
    ...
    console.log(
      `✅ Generated schedule: ${scheduledItems.length} scheduled, ${overflowItems.length} overflow`
    );
...
router.post('/save', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      console.log(
        `📅 Target layup days for this save: ${layupDays.join(', ')}`
      );
      ...
      if (existingPOCounts.size > 0) {
        ...
        for (const [key, count] of poItemEntries) {
          ...
          console.log(
            `📦 Decremented PO item ${itemId}: removed ${count} from order_count`
          );
        }
      }
      ...
      console.log(
        `🗑️ Cleared existing entries for days: ${layupDays.join(', ')} (preserving other days in the week)`
      );
      ...
      for (const entry of entries) {
        ...
        const layupDay =
          processedScheduledDate instanceof Date
          ...
          if (parts.length >= 3) {
            ...
            try {
              ...
              if (poItemResult.rows && poItemResult.rows.length > 0) {
                ...
                console.log(
                  `📦 Created/updated production_orders record for ${orderId} with stock model: ${stockModelForPO}`
                );
              }
            } catch (poError) {
              console.log(
                `⚠️ Could not create production_orders for ${orderId}:`,
                poError
              );
            }
          }
        ...
        console.log(`✅ Order ${orderId} scheduled for ${scheduledDate}`);
      }
      ...
      if (poItemCounts.size > 0) {
        ...
        for (const [key, count] of newPOItemEntries) {
          ...
          console.log(
            `📦 Updated PO item ${itemId}: added ${count} to order_count`
          );
      ...
      if (orderIds.length > 0) {
        ...
        console.log(
          `📦 Moved ${progressedCount} orders to Layup/Plugging department`
        );
      }
      ...
      if (productionOrderNumbers.size > 0) {
        ...
        for (const poNumber of poNumbersArray) {
          ...
          if (checkRows.length > 0) {
            ...
            console.log(
              `📊 PO ${poNumber}: ${completedItems}/${totalItems} items fully scheduled`
            );
        ...
        if (fullyScheduledPOs.length > 0) {
          ...
          console.log(
            `📦 Moved ${poProgressedCount} production orders to Layup/Plugging (all items complete): ${fullyScheduledPOs.join(', ')}`
          );
        } else {
          console.log(
            `📦 No production orders ready to move (items still pending)`
          );
        }
      }
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `📋 Found ${scheduleOrderIds.length} schedule entries for ${scheduleDate}`
      );
      ...
      console.log(
        `📦 Regular orders: ${regularOrderIds.length}, PO units: ${poUnitIds.length}`
      );
      ...
      if (poUnitIds.length > 0) {
        ...
        if (poMappings.length > 0) {
          const uniqueMappings = Array.from(
            new Map(
              poMappings.map((m) => [`${m.poNumber}-${m.poItemId}`, m])
            ).values()
          );
          ...
          console.log(
            `🔗 Mapped ${poUnitIds.length} PO units to ${productionOrderIds.size} production orders`
          );
        }
      }
      ...
      const allOrderIds = [
        ...regularOrderIds,
        ...Array.from(productionOrderIds),
      ];

      console.log(
        `✅ Total orders for barcode scan: ${allOrderIds.length} (${regularOrderIds.length} regular + ${productionOrderIds.size} production orders)`
      );
...
router.get('/week/:weekStart', async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `📋 SCHEDULE REPRINT: Fetching schedule for week starting ${weekStart}...`
    );
    ...
    console.log(
      `Regular orders: ${regularOrderIds.length}, PO orders: ${poOrderIds.length}`
    );
    ...
    const scheduledItems = scheduleEntries.map((entry: any) => {
      ...
      if (entry.order_id.startsWith('PO-')) {
        ...
        const poOrder = poOrders.find(
          (po: any) => po.order_id === entry.order_id
        );
        ...
        // Regular order
        const order = regularOrders.find(
          (o: any) => o.order_id === entry.order_id
        );
    ...
    console.log(
      `✅ Built ${scheduledItems.length} scheduled items with details`
    );
...
router.get('/weekly-summary', async (req: Request, res: Response) => {
  try {
    console.log(
      '📊 WEEKLY SUMMARY: Fetching packets needed from P1 schedule...'
    );
    ...
    const scheduleRows = Array.isArray(scheduleResult)
      ? scheduleResult
      : (scheduleResult as any).rows || [];
    scheduleRows.forEach((row: any) => {
      const stockModel = (row.stock_model || '').toLowerCase();
      if (
        stockModel.includes('_cf_') ||
        stockModel.includes('_cf') ||
        stockModel.includes('cf_')
      ) {
        carbonFiberCount++;
      } else if (
        stockModel.includes('_fg_') ||
        stockModel.includes('_fg') ||
        stockModel.includes('fg_')
      ) {
        fiberglassCount++;
      }
    });
    ...
    console.log(
      `✅ Weekly summary: ${carbonFiberCount} CF, ${fiberglassCount} FG from ${scheduleRows.length} scheduled items`
    );
    ...
    if (carbonFiberCount > 0) {
      summaryItems.push({
        stockModel: 'cf_packet',
        quantity: carbonFiberCount,
      });
    }
...
router.post(
  '/backfill-production-orders',
  async (req: Request, res: Response) => {
    try {
      console.log(
        '🔧 BACKFILL: Starting production_orders backfill for scheduled PO items...'
      );
      ...
      `);
        ...
        console.log(
          `📋 Found ${missingRows.length} missing production_orders records to backfill`
        );
        ...
        for (const row of missingRows) {
          ...
          `,
            ...
            if (!poItemResult.rows || poItemResult.rows.length === 0) {
              console.log(
                `⚠️ No matching PO item found for ${orderId} (poNumber=${poNumber}, itemId=${itemId})`
              );
              skippedCount++;
              continue;
            }
            ...
            let derivedStockModel =
              row.stock_model || poItem.stock_model_id || '';
            ...
            const stockModelForItem =
              derivedStockModel || poItem.item_name || '';
          ...
          `,
            ...
            console.log(
              `✅ Created production_orders for ${orderId} with stock model: ${stockModelForItem}`
            );
            createdCount++;
          } catch (itemError) {
        ...
        console.log(
          `✅ BACKFILL COMPLETE: Created ${createdCount}, Skipped ${skippedCount}, Errors ${errorCount}`
        );
