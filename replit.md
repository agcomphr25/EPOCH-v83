...
- **Cross-Platform Deployment**: PWA capabilities with Capacitor facilitate deployment to web and mobile (iOS/Android).
- **Dynamic Form Generation**: Includes a dynamic form builder with signature capture.
- **Authentication**: Hybrid JWT + Session authentication with capability-based access control and account lockout mechanisms.
- **Capability-Based Permissions**: A simplified 3-role system (ADMIN, EMPLOYEE, OWNER) with individual capability assignments for fine-grained access control.
- **Data Consistency**: The `features` object serves as the single source of truth for all feature data during order entry.
