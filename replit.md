...
## Recent Changes
- **Finish QC Completed Report** (Oct 24, 2025): Implemented comprehensive reporting system tracking orders progressed out of Finish QC department. Enhanced department progression tracking to record full audit trail in departmentHistory JSONB field (fromDepartment, toDepartment, timestamp, progressedBy, assignedTechnician). Created new report accessible from Forms & Reports menu showing last 7 days of completions grouped by technician, displaying order details and which user clicked the progression button. Critical fix: Uses strict departmentHistory filtering (fromDepartment === 'Finish QC') to avoid false positives from orders currently in Finish QC. Includes CSV export and summary statistics. Important: Neon serverless database returns query results as arrays directly, not wrapped in { rows: [...] } object.
