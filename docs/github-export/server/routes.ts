...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/persistent-discounts', async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: 'Failed to retrieve persistent discounts' });
    }
  });
  ...
  app.delete('/api/feature-categories/:id', async (req, res) => {
    try {
      ...
      res
        .status(400)
        .json({ error: error.message || 'Failed to delete feature category' });
    }
  });
  ...
  app.get('/api/feature-sub-categories', async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: 'Failed to retrieve feature sub-categories' });
    }
  });
  ...
  app.put('/api/orders/:orderId', async (req, res) => {
    try {
      ...
      if (existingDraft) {
        ...
        const newDraft = await storage.createOrderDraft({
          ...result,
          orderId,
        } as any);
        res.json(newDraft);
  ...
  app.put('/api/orders/draft/:orderId', async (req, res) => {
    try {
      ...
      if (!existingDraft) {
        ...
        return res
          .status(404)
          .json({ error: `Draft order with ID ${orderId} not found` });
      }
      ...
      try {
        ...
        if (parseError instanceof Error) {
          return res
            .status(400)
            .json({ error: `Validation error: ${parseError.message}` });
        }
  ...
  app.get('/api/form-submissions', async (req, res) => {
    try {
      const formId = req.query.formId
        ? parseInt(req.query.formId as string)
        : undefined;
  ...
  app.post('/api/timeclock', async (req, res) => {
    try {
      ...
      if (!employeeId || !action || !timestamp) {
        return res
          .status(400)
          .json({ error: 'Employee ID, action, and timestamp are required' });
      }
      ...
      if (action === 'IN') {
        ...
        return res
          .status(400)
          .json({ error: "Invalid action. Must be 'IN' or 'OUT'" });
      }
  ...
  app.put('/api/timeclock/entries/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const validatedData = insertTimeClockEntrySchema
        .partial()
        .parse(req.body);
  ...
  app.post('/api/checklist/complete', async (req, res) => {
    try {
      ...
      if (!employeeId || !date || !items) {
        return res
          .status(400)
          .json({ error: 'Employee ID, date, and items are required' });
      }
  ...
  app.get('/api/address/autocomplete', async (req, res) => {
    try {
      ...
      if (!authId || !authToken) {
        return res
          .status(500)
          .json({ error: 'SmartyStreets credentials not configured' });
      }
      ...
      try {
        ...
        console.log(
          'Autocomplete API not available, trying Street API approach'
        );
  ...
  app.post('/api/address/validate', async (req, res) => {
    try {
      ...
      if (!results || results.length === 0) {
        return res
          .status(400)
          .json({ error: 'Address could not be validated' });
      }
  ...
  app.get('/api/pdfs/order-confirmation', async (req, res) => {
    try {
      ...
      const mockPdfBuffer = Buffer.from(
        `Mock Order Confirmation PDF for Order: ${orderId}`
      );

      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="order-confirmation-${orderId}.pdf"`
      );
  ...
  app.get('/api/pdfs/packing-slip', async (req, res) => {
    try {
      ...
      // Mock PDF generation
      const mockPdfBuffer = Buffer.from(
        `Mock Packing Slip PDF for Order: ${orderId}`
      );
  ...
  app.get('/api/pdfs/invoice', async (req, res) => {
    try {
      ...
      // Mock PDF generation
      const mockPdfBuffer = Buffer.from(
        `Mock Invoice PDF for Order: ${orderId}`
      );
  ...
  app.post('/api/communications/order-confirmation', async (req, res) => {
    try {
      ...
      if (!orderId || !via) {
        return res
          .status(400)
          .json({ error: 'Order ID and communication method are required' });
      }
      ...
      const mockCategories = [
        {
          ...
          description: 'QC inspection forms',
          createdAt: new Date(),
          updatedAt: new Date(),
        },
        {
          id: 2,
          name: 'Maintenance',
          description: 'Equipment maintenance forms',
          createdAt: new Date(),
          updatedAt: new Date(),
        },
        {
          id: 3,
          name: 'Production',
          description: 'Production tracking forms',
          createdAt: new Date(),
          updatedAt: new Date(),
        },
      ];
    ...
    try {
      ...
      const mockTables = [
        ...
        },
        {
          ...
          description: 'Quality control submissions',
        },
        {
  ...
  app.get('/api/documentation', async (req, res) => {
    try {
      ...
      const docPath = path.join(
        process.cwd(),
        'EPOCH_v8_Complete_Architecture.md'
      );
