...
export async function seedOrderReferenceTables() {
  try {
    ...
    for (const dept of departments) {
      await db.insert(orderDepartmentTypes).values(dept).onConflictDoNothing();
    }
    ...
    for (const status of statuses) {
      await db.insert(orderStatusTypes).values(status).onConflictDoNothing();
    }
