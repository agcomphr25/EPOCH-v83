...
describe('Phase 8C integration safety contract', () => {
  ...
  const migration = fs.readFileSync(
    path.join(root, 'migrations/0211_project_preproduction_readiness.sql'),
    'utf8'
  );
  ...
  it('uses compatible composite identities and additive partial uniqueness', () => {
    expect(migration).toContain('UNIQUE (id, project_id)');
    ...
    expect(migration).toContain("WHERE status='COMPLETE'");
    expect(migration).toContain(
      'No existing project or production record is backfilled'
    );
