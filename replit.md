...
- **Process Runner Integration**: Minimal ingestion endpoint for external Timer/Process Runner app events. Accepts POST requests at `/api/integrations/process-runner/events` with program run data (programRunId, programName, eventType, timestamp, stepIndex, totalElapsedMinutes, metadata). Optional token-based authentication via `PROCESS_RUNNER_TOKEN` environment variable. Safe and non-disruptive - EPOCH functions identically if no events are sent. Health check available at `/api/integrations/process-runner/health`.
