...
import { enforceDesignControlProjectAssignment } from '../middleware/designControlProjectAccess';
import designControlStructuredRoutes from './qmsDesignControlStructured';
import {
...
const requiredStepKeys = workflowSteps
  .filter((step) => step.key !== '12')
  .map((step) => step.key);
const releaseGateSourceRequirements =
  canonicalManufacturingEvidenceRequirements.map((requirement) => ({
...
function valuesForWorkflowItem(
  values: Record<string, unknown>,
  stepKey: string,
  kind: 'fields' | 'checklist' | 'approvals',
  canonicalKey: string
) {
  const step = DESIGN_CONTROL_WORKFLOW.find(
    (candidate) => candidate.key === stepKey
  );
  const workflowItem = step?.[kind].find(
    (candidate) => candidate.key === canonicalKey
  );
  const lookupKeys = workflowItem
    ? workflowItemLookupKeys(workflowItem)
    : [canonicalKey];
...
function missingForStep(
  ...
  return {
    fields: step.requiredFields.filter(
      (field) =>
        String(
          valuesForWorkflowItem(formData, step.key, 'fields', field) ?? ''
        ).trim().length === 0
    ),
    checklist: includeChecklist
      ? step.requiredChecklist.filter(
          (entry) =>
            valuesForWorkflowItem(checklist, step.key, 'checklist', entry) !==
            true
        )
      : [],
    approvals: step.requiredApprovals.filter(
      (approval) =>
        valuesForWorkflowItem(approvals, step.key, 'approvals', approval) !==
        true
    ),
  };
}
...
function formatMissingItems(
  step: WorkflowStepDefinition,
  missing: StepMissingEvidence
) {
  return [
    ...missing.fields.map(
      (field) => `Step ${step.key} ${step.title} field missing: ${field}`
    ),
    ...missing.checklist.map((item) => {
      const sourceRequirement = releaseGateSourceRequirements.find(
        (requirement) => normalizeKey(requirement.item) === normalizeKey(item)
      );
    ...
    ...missing.approvals.map(
      (approval) =>
        `Step ${step.key} ${step.title} approval missing: ${approval}`
    ),
  ];
}

function formatStepRequirementError(
  step: WorkflowStepDefinition,
  missing: StepMissingEvidence
) {
  const missingSourceStatuses =
    step.key === '12'
      ? missing.checklist.map((item) => {
          const sourceRequirement = releaseGateSourceRequirements.find(
            (requirement) =>
              normalizeKey(requirement.item) === normalizeKey(item)
          );
          return {
            item,
...
async function getRecord(id: string) {
  const [record] = await db
    ...
    .where(eq(designControlRecords.id, id))
    .limit(1);
  return record;
}
...
function buildReadinessFromSteps(
  ...
  for (const step of workflowSteps) {
    ...
    if (step.key !== '12' && persisted.status !== 'approved') {
      missingItems.push(
        `Step ${step.key} ${step.title}: approval required before Engineering Release Gate`
      );
    }
  }
  ...
  if (!releaseStep) {
    ...
    missingItems.push(
      ...missing.fields.map(
        (field) =>
          `Step ${releaseDefinition.key} ${releaseDefinition.title} field missing: ${field}`
      ),
      ...missing.approvals.map(
        (approval) =>
          `Step ${releaseDefinition.key} ${releaseDefinition.title} approval missing: ${approval}`
      )
    );
  }
  ...
  if (manufacturingEvidence) {
    missingItems.push(
      ...manufacturingEvidence.missingItems.map(
        (item) => `Step 12 Engineering Release Gate source incomplete: ${item}`
      )
    );
  } else {
    missingItems.push(
      ...canonicalManufacturingEvidenceRequirements.map(
        (requirement) =>
  ...
  return {
    ...
    manufacturingSourceStatuses: (manufacturingEvidence?.sources ?? []).map(
      (source) => ({
...
async function buildReadiness(recordId: string) {
  ...
  const manufacturingEvidence = record
    ? await getDesignManufacturingEvidence({
        rdProjectId: record.rdProjectId,
...
function approvalActorFromRequest(req: Request): DesignControlApprovalActor {
  ...
  return {
    id: user.id,
    username:
      user.username ?? user.email ?? user.displayName ?? `user-${user.id}`,
...
router.get(
  ...
  async (_req: Request, res: Response) => {
    try {
      ...
      if (
        typeof _req.query.rdProjectId === 'string' &&
        _req.query.rdProjectId.trim()
      ) {
        conditions.push(
          eq(designControlRecords.rdProjectId, _req.query.rdProjectId.trim())
        );
      }
      if (
        typeof _req.query.projectId === 'string' &&
        _req.query.projectId.trim()
      ) {
        conditions.push(
          eq(designControlRecords.projectId, _req.query.projectId.trim())
        );
      }
      ...
      const records =
        conditions.length > 0
          ? await db
              ...
              .orderBy(
                desc(designControlRecords.updatedAt),
                desc(designControlRecords.createdAt)
              )
          : await db
              .select()
              .from(designControlRecords)
      ...
      const rdProjectId =
        typeof _req.query.rdProjectId === 'string'
          ? _req.query.rdProjectId.trim()
          : '';
      const authority = rdProjectId
        ? await resolveDesignControlAuthority(rdProjectId)
        : null;
      res.json({
        records,
      ...
      res
        .status(500)
        .json({ message: 'Failed to load design control records' });
    }
  }
);
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (authority)
        conditions.push(eq(designControlRecords.authorityStatus, authority));
      ...
      const recordIds = projects.flatMap((project) =>
        project.recordId ? [project.recordId] : []
      );
      const stepCounts = recordIds.length
        ? await db
            ...
            .from(designControlSteps)
            .where(inArray(designControlSteps.recordId, recordIds))
            .groupBy(designControlSteps.recordId)
        : [];
      ...
      res.json({
        ...
        projects: projects.map((project) => ({
          ...
          completedSteps: Number(
            counts.get(project.recordId || '')?.completed || 0
          ),
          blockedSteps: Number(
            counts.get(project.recordId || '')?.blocked || 0
          ),
      ...
      console.error(
        '[qms-design-control] Failed to load oversight projects',
        error
      );
      res
        .status(500)
        .json({ message: 'Failed to load Design Control oversight' });
    }
  }
);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!parsed.success) {
        res.status(400).json({
          message: 'Invalid design control record',
      ...
      if (parsed.data.rdProjectId) {
        ...
        if (result.status === 'conflict') {
          return res.status(409).json({
            message:
              'This R&D project already has Design Control records. Use the project authority reconciliation workflow.',
            authorityState: result.resolution.state,
            authoritativeRecordId:
              result.resolution.authoritativeRecord?.id ?? null,
          });
        }
      ...
      const record = await createDesignControlRecordWithInitialWorkflow(
        parsed.data
      );
      ...
      res
        .status(500)
        .json({ message: 'Failed to create design control record' });
    }
  }
);
      ...
      const result = await saveDesignControlStepDraft({
        ...
        expectedContentVersionId:
          ...
          (typeof req.body?.contentVersionId === 'string' ||
            req.body?.contentVersionId === null)
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        !['APPROVED', 'REJECTED', 'RETURNED_FOR_REVISION'].includes(decision)
      ) {
      ...
      if (
        typeof req.body?.contentVersionId !== 'string' ||
        typeof req.body?.approvalKey !== 'string'
      ) {
        res
          .status(400)
          .json({ error: 'CONTENT_VERSION_AND_APPROVAL_KEY_REQUIRED' });
        return;
      }
      ...
      sendApprovalError(
        res,
        error,
        'Failed to record Design Control approval decision'
      );
    }
  }
);
...
router.get(
  '/:id/steps/:stepKey/versions',
  authenticateToken,
  async (req: Request, res: Response) => {
    try {
      const state = await getDesignControlStepApprovalState(
        req.params.id,
        req.params.stepKey
      );
      res.json({
        currentContentVersion: state.currentContentVersion,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    res.status(410).json({
      ...
      message:
        'The legacy release submission path is retired. Submit Step 12 and record its authenticated approval decisions.',
    });
  }
);
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.error(
        '[qms-design-control] Failed to compute manufacturing evidence',
        error
      );
      res
        .status(500)
        .json({ message: 'Failed to compute design manufacturing evidence' });
    }
  }
);
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.error(
        '[qms-design-control] Failed to compute engineering release preview',
        error
      );
      res
        .status(500)
        .json({ message: 'Failed to compute engineering release preview' });
    }
  }
);
...
router.post(
  ...
  authenticateToken,
  requirePermission('design.release'),
  async (req: Request, res: Response) => {
    try {
      ...
      console.error(
        '[qms-design-control] Failed to submit engineering release',
        error
      );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ message: 'Failed to compute design control readiness' });
    }
  }
);
