...
export default function P2LayupScheduler() {
  ...
  const [p2OrderAssignments, setP2OrderAssignments] = useState<{
  ...
  useEffect(() => {
    if (
      existingP2Schedule &&
      Array.isArray(existingP2Schedule) &&
      existingP2Schedule.length > 0
    ) {
      const assignments: {
        [orderId: string]: { moldId: string; date: string };
      } = {};
      ...
      console.log(
        '📅 Loading existing P2 schedule assignments:',
  ...
  const [hasUnsavedP2ScheduleChanges, setHasUnsavedP2ScheduleChanges] =
    useState(false);
  ...
  const processedOrders = useMemo(() => {
    ...
    console.log('🔧 P2 LOP Scheduler auto-run:', {
      ...
      lopOrdersNeedingAdjustment: lopOrders.filter((o) => o.needsLOPAdjustment)
        .length,
    });
  ...
  useEffect(() => {
    ...
    if (p2Orders.length > 0) {
      ...
      console.log(
        '🏭 P2LayupScheduler: First 5 P2 orders:',
        p2Orders.slice(0, 5).map((o) => ({
          orderId: o.orderId,
          source: o.source,
  ...
  const generateP2AutoSchedule = useCallback(() => {
    ...
    console.log(
      '🚀 Generating P2 auto-schedule for',
      p2Orders.length,
      'P2 orders'
    );
    ...
    const getWorkDaysInWeek = (startDate: Date) => {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        if (workDay.getDay() === 5) {
          console.error(
            `❌ CRITICAL: P2 getWorkDaysInWeek attempted to include a Friday! Date: ${workDay.toDateString()}`
          );
      ...
      if (fridayCheck.length > 0) {
        console.error(
          `❌ CRITICAL: P2 Found ${fridayCheck.length} Friday dates in work days!`,
          fridayCheck.map((d) => d.toDateString())
        );
    ...
    const getCompatibleMolds = (order: any) => {
      ...
      console.log(
        `🎯 P2 Order ${order.orderId} (${modelId}) → ${compatibleMolds.length} compatible molds:`,
    ...
    console.log(
      `👥 P2 Employee capacity: ${maxOrdersPerDay} orders per day max`
    );
    ...
    sortedP2Orders.forEach((order) => {
      ...
      if (bestMold && bestDateIndex < allWorkDays.length) {
        ...
        assigned = true;
        console.log(
          `🏭 P2 ORDER ASSIGNED: ${order.orderId} to ${bestMold.moldId} on ${format(targetDate, 'MM/dd')} (${dailyAssignments[dateKey]}/${maxOrdersPerDay} daily capacity)`
        );
      }

      if (!assigned) {
        console.warn(
          `❌ Could not find available cell for P2 order: ${order.orderId}`
        );
      }
    });

    console.log(
      '📅 Generated P2 schedule assignments:',
      Object.keys(newP2Assignments).length,
      'P2 orders assigned'
    );
        ...
        <div className="px-6 pb-4">
          <div className="flex justify-between items-center p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
            ...
            <div className="flex items-center space-x-2">
              ...
              <Dialog
                open={showP2MoldSettings}
                onOpenChange={setShowP2MoldSettings}
              >
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setNewP2Mold((prev) => ({
                                ...prev,
                                instanceNumber: parseInt(e.target.value) || 1,
                              }))
                            }
                          />
                        </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        {molds
                          .filter(
                            (m) =>
                              m.moldId.includes('P2') ||
                              m.modelName.includes('P2')
                          )
                          .map((mold) => (
                            <div
                              key={mold.id}
                            ...
                            >
                              <div className="flex items-center space-x-4">
                                <Badge
                                  variant={
                                    mold.enabled ? 'default' : 'secondary'
                                  }
                                >
                                  {mold.moldId}
                                </Badge>
                                ...
                                <span className="text-xs text-gray-500">
                                  Instance: {mold.instanceNumber} | Capacity:{' '}
                                  {mold.multiplier}
                                </span>
                              </div>
                              <div className="flex items-center space-x-2">
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    handleDeleteP2Mold(mold.moldId)
                                  }
                                >
                                  Delete
                                </Button>
              ...
              <Dialog
                open={showP2EmployeeSettings}
                onOpenChange={setShowP2EmployeeSettings}
              >
                <DialogTrigger asChild>
                  <Button variant="outline" size="sm">
                ...
                <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <h3 className="font-semibold mb-3">
                        Existing P2 Employees
                      </h3>
                      <div className="grid grid-cols-1 gap-2">
                        {employees
                          .filter(
                            (emp) =>
                              emp.department?.includes('P2') ||
                              emp.employeeId.includes('P2')
                          )
                          .map((employee) => (
                            <div
                              key={employee.id}
                            ...
                            >
                              ...
                              <div className="flex items-center space-x-2">
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    toggleEmployeeStatus(employee.employeeId)
                                  }
                                >
                                  {employee.isActive
                                    ? 'Deactivate'
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    handleDeleteP2Employee(employee.employeeId)
                                  }
                                >
                                  Delete
                                </Button>
              ...
              >
                ...
                {(() => {
                  ...
                  const relevantMolds = molds.filter((m) => {
                    ...
                    const hasP2Assignments = Object.values(
                      p2OrderAssignments
                    ).some((assignment) => assignment.moldId === m.moldId);
                    const isCompatibleWithP2Queue = compatibleMoldIds.has(
                      m.moldId
                    );
                  ...
                  console.log(
                    `🏭 P2 Relevant molds: ${relevantMolds.length}/${molds.filter((m) => m.enabled).length}`
                  );
                  console.log(
                    `🏭 P2 Mold order counts:`,
                    sortedMolds.map(
                      ({ mold, orderCount }) =>
