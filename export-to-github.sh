...
# Copy core files
cp package.json github-export/
cp package-lock.json github-export/
cp tsconfig.json github-export/
...
cp -r client/ github-export/
cp -r server/ github-export/
cp -r shared/ github-export/
cp -r scripts/ github-export/

# Create .gitignore for GitHub
cat > github-export/.gitignore << EOF
node_modules
dist
.DS_Store
server/public
...
# Create .env.example
cat > github-export/.env.example << EOF
# Database Configuration
DATABASE_URL=postgresql://username:password@localhost:5432/epoch_erp
...
echo "✅ GitHub package created in 'github-export' directory"
