...
// Helper function to get text width for right-alignment
function getTextWidth(text: string, fontSize: number): number {
  // Approximate character width (Helvetica is roughly 0.5-0.6 of font size per char)
...
router.get('/sales-order/:orderId', async (req: Request, res: Response) => {
  try {
    ...
    // Define column positions for consistent alignment throughout the PDF
    const featureColX = margin + 8;
    const selectionColX = margin + 180;
    const priceColRightX = margin + printableWidth - 15; // Right edge of price column
    ...
    // Miscellaneous Items
    miscPrice = 0;
    let miscDisplay = 'None';
    
    if ((order as any).features?.miscItems && Array.isArray((order as any).features?.miscItems) && (order as any).features?.miscItems.length > 0) {
      const miscItems = (order as any).features?.miscItems;
      miscPrice = miscItems.reduce((sum: number, item: any) => sum + (item.total || 0), 0);
      miscDisplay = miscItems.map((item: any) => 
        `${item.description || 'Misc Item'} (Qty: ${item.quantity}) - $${(item.total || 0).toFixed(2)}`
      ).join(', ');
    }
    ...
    // Word wrap miscellaneous items text
    ...
    // Notes and Miscellaneous Items Section
    currentY -= 50;
    page.drawText('NOTES AND MISCELLANEOUS ITEMS', {
      x: margin,
      y: currentY,
      size: 12,
      font: boldFont,
    ...
    if (order.notes && order.notes.trim() !== '') {
      const noteLines = wrapText(order.notes, printableWidth - 10, 9, font);
