...
- **Central QR Code System**: Generates QR codes with public identifiers (qr_XXXXXXXX format), resolves them to entity-specific routes based on user role, logs all scan events via audit service, and provides admin CRUD UI using TanStack Table patterns. Supports 10 entity types (order, inventory_item, employee, mandrel, oven, timer_program, document, equipment, material_lot, custom). Enforces environment safety (dev/prod) and uses separate routers for public resolver (/qr/:code) and admin endpoints (/api/qr-codes). Tables: qr_codes, qr_code_scan_log. Routes: /qr/:code (public resolver), /admin/qr-codes (admin UI).
