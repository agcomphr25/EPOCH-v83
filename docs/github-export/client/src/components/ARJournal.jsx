 ...
 */
export default function ARJournal({ dateFrom, dateTo }) {
  const [, navigate] = useLocation();
  const [dateRange, setDateRange] = useState({ dateFrom, dateTo });
  const [termsFilter, setTermsFilter] = useState('all');
  const { data, loading, error, refresh } = useARTransactions(dateRange);
  ...
  // Navigate to detailed AR transaction view
  const handleRowClick = (transaction) => {
    navigate(`/ar/${transaction.id}`);
  };
                ...
                <tbody>
                  {filteredData.map((transaction) => (
                    ...
                    >
                      ...
                      <td className="p-4">
                        <Badge variant="outline">{transaction.terms}</Badge>
                      </td>
                      <td className="p-4">
