...
const workspace = fs.readFileSync(
  path.join(root, 'client/src/features/design-control/DesignControlWorkspace.tsx'),
  'utf8',
);
...
describe('Phase 11 unified Design Control workspace', () => {
  ...
  it('keeps authority on rd_projects and authoritative design control records', () => {
    ...
    expect(route).toContain("eq(designControlRecords.authorityStatus, 'authoritative')");
  ...
  it('provides bounded server pagination and filtering', () => {
    ...
    expect(qms).toContain('Filtering and pagination are performed by the server');
  });
  ...
  it('renders live requirements, risks, reviews, verification, and validation', () => {
    for (const key of ['detail.requirements', 'detail.risks', 'detail.reviews', 'detail.verification', 'detail.validation']) {
      expect(workspace).toContain(key);
    }
  });

  it('contains no former sample registers', () => {
    expect(qms).not.toMatch(/const\s+(inputs|outputs|reviews|risks|verification|validation|changes|releases)\s*=/);
    expect(qms.toLowerCase()).not.toContain('sample');
  });

  it('explains ECR, ECN, and release distinctions', () => {
    expect(workspace).toContain('ECR</strong> asks whether a change should be made');
  ...
  it('forces existing mutation panels into oversight mode for auditors', () => {
    ...
    expect(workspace.match(/oversightMode=\{readOnly\}/g)?.length).toBeGreaterThanOrEqual(6);
  ...
  it('provides loading, empty, error, retry, and permission messages', () => {
    for (const text of ['Loading bounded oversight results', 'No Design Projects match', 'Oversight could not be loaded', 'Retry', 'not authorized']) {
  ...
  it('does not add client-side release bypasses', () => {
    expect(workspace + qms).not.toMatch(/bypassRelease|forceRelease|skipReadiness/);
  });
