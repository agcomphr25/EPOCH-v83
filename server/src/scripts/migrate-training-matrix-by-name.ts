...
async function migrateEmployeeTrainingMatrixByName() {
  console.log(
    '🚀 Starting employee-specific training matrix migration (matching by name)...\n'
  );
  ...
  try {
    ...
    const devEmployees = await devDb
      .select({
    ...
    const prodEmployees = await prodDb
      .select({
        id: schema.employees.id,
        name: schema.employees.name,
        employeeCode: schema.employees.employeeCode,
    ...
    for (const devEmp of devEmployees) {
      ...
      let prodEmp = prodEmployees.find(
        (e) =>
          e.name?.toLowerCase().trim() === devEmp.name?.toLowerCase().trim()
      );

      // If no name match, try by employeeCode
      if (!prodEmp && devEmp.employeeCode) {
        prodEmp = prodEmployees.find(
          (e) => e.employeeCode === devEmp.employeeCode
        );
      }
      ...
      if (prodEmp) {
        ...
        console.log(
          `   ✅ Mapped "${devEmp.name}" (ID ${devEmp.id} → ${prodEmp.id})`
        );
      } else {
        console.log(
          `   ⚠️  Employee "${devEmp.name}" (ID ${devEmp.id}) not found in PROD`
        );
      }
    }
    ...
    const employeeMatrixEntries = devMatrix.filter(
      (entry) => entry.employeeId !== null
    );
    console.log(
      `   Found ${employeeMatrixEntries.length} employee-specific training matrix entries`
    );
    ...
    if (skippedCount > 0) {
      console.log(
        `   ⚠️  Skipped ${skippedCount} entries (employees not in production)`
      );
    }
    ...
    const prodEmployeeMatrixCount = prodMatrixCount.filter(
      (e) => e.employeeId !== null
    ).length;
    ...
    console.log(
      `Employee ID Mappings Created: ${Object.keys(employeeIdMap).length}`
    );
    console.log(
      `Employee-Specific Entries in DEV: ${employeeMatrixEntries.length}`
    );
    console.log(`Employee-Specific Entries Migrated: ${migratedCount}`);
    console.log(`Employee-Specific Entries Skipped: ${skippedCount}`);
    console.log(
      `Total Employee Entries in PROD Now: ${prodEmployeeMatrixCount}`
    );
    console.log('═'.repeat(70));
